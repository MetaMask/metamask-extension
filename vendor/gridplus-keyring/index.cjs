...
var types = require('@gridplus/types');
...
function parseHdPath(hdPath, insertIdx = 0) {
  ...
  for (const segment of segments) {
    ...
    const idx = (() => {
      if (normalized === "x") {
        usedX = true;
        return insertIdx + (isHardened ? types.HARDENED_OFFSET : 0);
      }
      ...
      return parsed + (isHardened ? types.HARDENED_OFFSET : 0);
    })();
    indices.push(idx);
  }
  if (!usedX) {
...
function formatSignerPath(signerPath) {
  const segments = signerPath.map((idx) => {
    if (idx >= types.HARDENED_OFFSET) {
      return `${idx - types.HARDENED_OFFSET}'`;
    }
    return idx.toString();
  });
...
var GridPlusKeyring = class extends eventemitter3.EventEmitter {
...
exports.ConnectApiError = ConnectApiError;
exports.GridPlusKeyring = GridPlusKeyring;
exports.INTERNAL_VAR_HD_PATH = INTERNAL_VAR_HD_PATH;
exports.LEGACY_HD_PATH = LEGACY_HD_PATH;
exports.PER_PAGE = PER_PAGE;
exports.STANDARD_HD_PATH = STANDARD_HD_PATH;
exports.default = GridPlusKeyring;
exports.findIterIdx = findIterIdx;
exports.formatSignerPath = formatSignerPath;
exports.getSignerPath = getSignerPath;
exports.hasInternalVarIdx = hasInternalVarIdx;
exports.isValidSignerPath = isValidSignerPath;
