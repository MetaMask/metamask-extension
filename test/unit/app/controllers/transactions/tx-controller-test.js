...
import { TRANSACTION_TYPE_CONTRACT_INTERACTION, TRANSACTION_TYPE_DEPLOY_CONTRACT, TRANSACTION_TYPE_SENT_ETHER, TRANSACTION_TYPE_TOKEN_METHOD_APPROVE, TRANSACTION_TYPE_TOKEN_METHOD_TRANSFER, TRANSACTION_TYPE_RETRY } from '../../../../../shared/constants/transaction'
...
describe('Transaction Controller', function () {
  ...
  describe('#_determineTransactionType', function () {
    it('should return a simple send type when to is truthy but data is falsy', async function () {
      ...
      assert.deepEqual(result, { type: TRANSACTION_TYPE_SENT_ETHER, getCodeResponse: null })
    })
    ...
    it('should return a token transfer type when data is for the respective method call', async function () {
      ...
      assert.deepEqual(result, { type: TRANSACTION_TYPE_TOKEN_METHOD_TRANSFER, getCodeResponse: undefined })
    })
    ...
    it('should return a token approve type when data is for the respective method call', async function () {
      ...
      assert.deepEqual(result, { type: TRANSACTION_TYPE_TOKEN_METHOD_APPROVE, getCodeResponse: undefined })
    })
    ...
    it('should return a contract deployment type when to is falsy and there is data', async function () {
      ...
      assert.deepEqual(result, { type: TRANSACTION_TYPE_DEPLOY_CONTRACT, getCodeResponse: undefined })
    })
    ...
    it('should return a simple send type with a 0x getCodeResponse when there is data and but the to address is not a contract address', async function () {
      ...
      assert.deepEqual(result, { type: TRANSACTION_TYPE_SENT_ETHER, getCodeResponse: '0x' })
    })
    ...
    it('should return a contract interaction type with the correct getCodeResponse when to is truthy and there is data and it is not a token transaction', async function () {
      ...
      assert.deepEqual(result, { type: TRANSACTION_TYPE_CONTRACT_INTERACTION, getCodeResponse: '0x0a' })
    })
