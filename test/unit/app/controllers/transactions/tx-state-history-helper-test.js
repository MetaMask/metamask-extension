...
describe('Transaction state history helper', function () {
  ...
  describe('#migrateFromSnapshotsToDiffs', function () {
    it('migrates history to diffs and can recover original values', function () {
      testVault.data.TransactionController.transactions.forEach((tx) => {
        const newHistory = txStateHistoryHelper.migrateFromSnapshotsToDiffs(
          tx.history
        )
        newHistory.forEach((newEntry, index) => {
          ...
          const reconstructedValue = txStateHistoryHelper.replayHistory(
            historySubset
          )
          assert.deepEqual(
            oldEntry,
  ...
  describe('#generateHistoryEntry', function () {
    function generateHistoryEntryTest (note) {
      ...
      const result = txStateHistoryHelper.generateHistoryEntry(
        prevState,
        nextState,
        note
      )
