...
async function runNavigateTxsFlowTest (assert, done) {
  ...
  // Verify transaction changed to num 3 and routed correctly
  ...
  assert.equal(summaryAction[0].innerText, 'UNKNOWN FUNCTION', 'correct route')

  // Click navigation button
  navigateTxButtons = await queryAsync($, '.confirm-page-container-navigation__arrow')
  navigateTxButtons[1].click()

  // Verify transation changed to num 2 and routed correctly
  trxNum = await queryAsync($, '.confirm-page-container-navigation')
  assert.equal(trxNum[0].innerText.includes('2'), true, 'changed transaction left')
  summaryAction = await queryAsync($, '.confirm-page-container-summary__action')
  ...
  // Verify transation changed to num 1 and routed correctly
  trxNum = await queryAsync($, '.confirm-page-container-navigation')
  assert.equal(trxNum[0].innerText.includes('1'), true, 'changed transaction left')
  summaryAction = await queryAsync($, '.confirm-page-container-summary__action')
  assert.equal(summaryAction[0].innerText, 'TRANSFER', 'correct route')

  // Verify right arrow is working correctly
  navigateTxButtons = await queryAsync($, '.confirm-page-container-navigation__arrow')
