...
const REGISTRY = {
  'wallet.unlocked': {
    ...
    check: async (callHandler, _params, context) => {
      ...
      if (!result.ok) {
        return { pass: false, hint: 'Extension not loaded or no active session. Call mm_launch first.' };
      }
  ...
  'ext.element_visible': {
    ...
    check: async (callHandler, params) => {
      ...
      const result = await callHandler('mm_wait_for', { testId, timeoutMs: 5000 });
      if (!result.ok) return { pass: false, hint: `Element [data-testid="${testId}"] not found on screen.` };
  ...
  'ext.on_screen': {
    ...
    check: async (callHandler, params) => {
      ...
      const result = await callHandler('mm_get_state', {});
      if (!result.ok) return { pass: false, hint: 'Cannot get state to check screen.' };
