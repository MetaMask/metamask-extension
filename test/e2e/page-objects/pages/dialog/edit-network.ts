...
class EditNetworkModal {
   ...
   */
  async check_chainIdInputFieldIsEnabled(shouldBeEnabled: boolean): Promise<void> {
    console.log(`Check that chain id input field is ${shouldBeEnabled ? 'enabled' : 'disabled'}`);
    const chainIdInput = await this.driver.findElement(
      this.chainIdInputField,
    );
