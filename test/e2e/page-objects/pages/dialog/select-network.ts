...
class SelectNetwork {
  ...
  async clickDiscoverButton(): Promise<void> {
    ...
    await this.driver.clickElement('[data-testid="network-list-item-options-discover"]');
  }

  async check_discoverButtonIsVisible(): Promise<void> {
    console.log('Check Discover button is visible in network options');
    await this.driver.waitForSelector('[data-testid="network-list-item-options-discover"]');
  }
}
