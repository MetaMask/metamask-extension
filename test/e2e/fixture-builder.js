...
function onboardingFixture() {
  try {
    // Prefer the artifact generated by the export job if present; otherwise fall back to the local fixture file
    // The artifact is downloaded to './onboarding-fixture.json' (root of workspace)
    const artifactPath = path.resolve(process.cwd(), 'onboarding-fixture.json');
  ...
  if (fs.existsSync(localFixturePath)) {
    try {
      const content = fs.readFileSync(localFixturePath, 'utf8');
      const parsed = JSON.parse(content);
      // eslint-disable-next-line no-console
      console.log(
      ...
      // eslint-disable-next-line no-console
      console.error('[FixtureBuilder] Error reading local fixture file:', e);
    }
  }
  ...
  // eslint-disable-next-line no-console
  console.error(
    '      Look for "e2e Wallet State Fixture File" comment in your PR\n',
  );
  // eslint-disable-next-line no-console
