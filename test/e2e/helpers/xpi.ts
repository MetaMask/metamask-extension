...
export async function getOrBuildXpi(extDir: string) {
  ...
  try {
    await using xpiFd = await open(xpiPath, 'r+');
    const { size } = await xpiFd.stat();
    const xpiHash = Buffer.allocUnsafe(HASH_SIZE);
    const hashOffset = size - HASH_SIZE;
    const { bytesRead } = await xpiFd.read(xpiHash, 0, HASH_SIZE, hashOffset);
    if (bytesRead === HASH_SIZE && xpiHash.toString() === manifestHash) {
      // no changes, no need to rebuild
      return xpiPath;
    }
    await patchManifest(xpiFd, hashOffset, manifest, manifestHash);
    return xpiPath;
  } catch {
...
export async function patchManifest(
  ...
  for (const [buffer, position] of [
    ...
    const writeRes = await xpiFd.write(buffer, 0, buffer.length, position);
    if (writeRes.bytesWritten !== buffer.length) {
      throw new Error(`Failed to write buffer to XPI at offset ${position}`);
    }
  }
}
