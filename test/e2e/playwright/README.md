...
- Shard counts come from `prep-e2e-chrome.yml` and `prep-e2e-firefox.yml` (`MATRIX_TOTAL`); the runner does time-based distribution internally, so there is no `--shard` in the test command.
