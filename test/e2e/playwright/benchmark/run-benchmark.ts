...
const browserLoads = args.find(arg => arg.startsWith('--browser-loads='))?.split('=')[1] || '10';
const pageLoads = args.find(arg => arg.startsWith('--page-loads='))?.split('=')[1] || '10';

console.log('🚀 Starting MetaMask Page Load Benchmark');
console.log(`📊 Configuration: ${browserLoads} browser loads, ${pageLoads} page loads per browser`);
...
try {
  ...
  execSync(
    `yarn playwright test --project=benchmark --reporter=list,html`,
    {
      stdio: 'inherit',
      env: {
        ...process.env,
