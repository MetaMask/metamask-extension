...
import MOCK_SWAP_QUOTES_ETH_MUSD from '../../tests/bridge/mocks/swap-quotes-eth-musd.json';
...
/**
 * Helper to add network delay to mock responses.
 * Simulates real-world latency based on Sentry production data
 * Source: https://metamask.sentry.io/explore/traces/ (http.client spans, 30d)
 *
 * @param delayMs - Delay in milliseconds
 * @param response - The response object
 * @returns A delayed callback function
 */
function delayedResponse<TResponse>(
  ...
  return async (req) => {
    console.log('Request mocked ============', req.url);
...
async function mockAuthAPICall(
  ...
  return requestRuleBuilder
    ...
    .thenCallback(async (request) => {
      ...
      // The response function takes requestBody, path, and an identifier function
      const json = (
...
export async function mockDelayedQuoteStream(
  server: Mockttp,
): Promise<MockedEndpoint> {
  return server
    .forGet(/getQuoteStream/u)
    .asPriority(200) // Higher priority to override base bridge mocks
    .always()
    .thenStream(
      200,
...
export async function mockPowerUserPrices(
  ...
  // Authentication APIs - nonce endpoint
  // https://metamask.sentry.io/explore/traces/?query=span.op%3Ahttp.client+http.url%3A*authentication.api.cx.metamask.io*
  const authNonceResponse = AuthMocks.getMockAuthNonceResponse();
  endpoints.push(await mockAuthAPICall(server, authNonceResponse, 1200));

  // Authentication APIs - login endpoint
  const authLoginResponse = AuthMocks.getMockAuthLoginResponse();
  endpoints.push(await mockAuthAPICall(server, authLoginResponse, 1200));

  // Authentication APIs - access token endpoint
  // https://metamask.sentry.io/explore/traces/?query=span.op%3Ahttp.client+AND+http.url%3A*oidc.api.cx.metamask.io*
  const authTokenResponse = AuthMocks.getMockAuthAccessTokenResponse();
  endpoints.push(await mockAuthAPICall(server, authTokenResponse, 3400));

  // User storage API - GET
  // https://metamask.sentry.io/explore/traces/?query=span.op%3Ahttp.client+http.url%3A*user-storage.api.cx.metamask.io*
  endpoints.push(
    await server
  ...
  // Linea Sepolia Infura RPC (testnet)
  endpoints.push(
    await server
  ...
  // Google accounts (Chrome browser integration)
  endpoints.push(
    await server
      .forAnyRequest()
  ...
  // Trigger API (notifications query)
  endpoints.push(
    await server
      .forAnyRequest()
  ...
  // Contentful CDN (product announcements)
  endpoints.push(
    await server
  ...
  // Price API - supportedVsCurrencies
  endpoints.push(
    await server
  ...
  // Solana spot prices - path-based (with pricePercentChange1d for UI rendering)
  endpoints.push(
    await server
  ...
  // Solana spot prices - query-based
  endpoints.push(
    await server
      .forGet(/price\.api\.cx\.metamask\.io\/v\d+\/spot-prices\?.*solana/u)
      .asPriority(103)
  ...
  // General spot prices (EVM tokens)
  endpoints.push(
    await server
  ...
  // Historical prices (for charts) - EVM chains
  endpoints.push(
    await server
      .forGet(
  ...
  // Historical prices (for charts) - Solana and non-EVM networks
  endpoints.push(
    await server
      .forGet(
  ...
  // Cryptocompare single price API
  endpoints.push(
    await server
  ...
  // Bridge getQuote endpoint (negative lookahead to not match getQuoteStream)
  endpoints.push(
    await server
  ...
  // Catch-all for bridge.api requests
  endpoints.push(
    await server
  ...
  // Bridge getQuoteStream endpoint (SSE) - uses same mock data as bridge E2E tests
  endpoints.push(
    await server
  ...
  // Client config feature flags endpoint
  endpoints.push(
    await server
