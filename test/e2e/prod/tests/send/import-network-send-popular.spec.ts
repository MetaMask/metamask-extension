...
/**
 * Production E2E Test: Add Popular Network, Import Account, and Send ETH
 * This test uses REAL network infrastructure with production RPC endpoints.
 */
describe('Production E2E: Import Account and Send on Mainnet', function (this: Suite) {
  this.timeout(300000); // 5 minutes for network operations and send

  it('imports account with private key and sends on Monad for GasSponsorship test', async function () {
    await withProductionFixtures(
      {
        fixtures: new FixtureBuilder()
          .withNetworkControllerOnBase() // Start with Base instead of localhost
          .build(),
        title:
          this.test?.fullTitle() ||
          'Import account and send on default fixture production test',
      ...
      async ({ driver }: { driver: Driver }) => {
        ...
        try {
          ...
          balance = (await balanceElement.getText()).replace(" ETH", "").trim();
        ...
        // Click on desired token
        await driver.clickElement({
          css: '[data-testid="token-asset-0x2105-ETH"]',
          text: symbol, // ETH
        });
        ...
        // Fill amount (sending 00005 ETH as a test)
        const sendAmount = '0.00001';
        console.log(`[PROD TEST] Filling amount: ${sendAmount} ETH`);
        ...
        await activityListPage.checkTransactionAmount(`-${sendAmount} ETH`);

        console.log('[PROD TEST] ✅ Transaction sent successfully!');
        console.log(`[PROD TEST] Sent ${sendAmount} ETH to ${recipientAddress} on Base network`);
        ...
        // Select Eth token
        console.log('[PROD TEST] Selecting ETH token...');
        ...
        console.log('[PROD TEST] ✅ Transaction sent successfully!');
        console.log(
          `[PROD TEST] Sent ${sendAmount} ETH to ${recipientAddress} on Base network`);

        console.log('[PROD TEST] ✅ All verifications passed!');
        console.log(
          `[PROD TEST] Successfully verified received transaction on Base network`);
      },
    );
  });
});
