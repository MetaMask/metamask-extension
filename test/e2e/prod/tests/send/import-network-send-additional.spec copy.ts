 ...
 */
describe('Production E2E: Monad Network, Import Account and Send MON', function (this: Suite) {
  this.timeout(300000); // 5 minutes for network operations and send

  it('adds Monad network, imports account with private key, and sends MON', async function () {
    await withProductionFixtures(
      ...
      async ({ driver }: { driver: Driver }) => {
        ...
        console.log(`[PROD TEST] Successfully verified received transaction on Monad network`);
      },
    );
  });
});
