...
import { type JavaTronPrivateNetworkPorts } from '../seeder/tron/java-tron-config';
...
async function runBitcoinSmoke(): Promise<void> {
  const [firstRpcPort, firstP2pPort, secondRpcPort, secondP2pPort] =
    await getAvailablePortsWithGaps(4);
  const nodes = [new BitcoinRegtestNode(), new BitcoinRegtestNode()] as const;
  const ports = [
    { p2pPort: firstP2pPort, rpcPort: firstRpcPort },
    { p2pPort: secondP2pPort, rpcPort: secondRpcPort },
  ] as const;
...
async function runTronSmoke(): Promise<void> {
  const portCount = JAVA_TRON_PORT_KEYS.length;
  const allocatedPorts = await getAvailablePortsWithGaps(portCount * 2);
  const ports = [
    toJavaTronPorts(allocatedPorts.slice(0, portCount)),
    toJavaTronPorts(allocatedPorts.slice(portCount, portCount * 2)),
  ] as const;
...
function toJavaTronPorts(ports: number[]): JavaTronPrivateNetworkPorts {
  return Object.fromEntries(
    JAVA_TRON_PORT_KEYS.map((key, index) => [key, ports[index]]),
  ) as JavaTronPrivateNetworkPorts;
}
