...
describe('MetaMask', function () {
  ...
  describe('sets permissions', () => {
    ...
    it('shows connected sites', async () => {
      const connectedSites = await findElement(
        driver,
      ...
      const domains = await findElements(
        driver,
        By.css('.connected-sites-list__domain')
      )
      assert.equal(domains.length, 1)

      const domainName = await findElement(
        driver,
      ...
      const permissionDescription = await findElement(
        driver,
    ...
    it('can get accounts within the dapp', async () => {
      ...
      const getAccountsButton = await findElement(
        driver,
        By.xpath(`//button[contains(text(), 'eth_accounts')]`)
      )
      await getAccountsButton.click()

      const getAccountsResult = await findElement(
        driver,
    ...
    it('can disconnect all accounts', async () => {
      ...
      const disconnectAllButton = await findElement(
        driver,
      ...
      const disconnectAllModalButton = await findElement(
        driver,
