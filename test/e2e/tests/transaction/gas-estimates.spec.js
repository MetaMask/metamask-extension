...
describe('Gas estimates generated by MetaMask', function () {
  const preLondonlocalNodeOptions = generateGanacheOptions({
    hardfork: 'berlin',
  });
  const postLondonlocalNodeOptions = generateGanacheOptions({
    hardfork: 'london',
  });
      ...
      await withFixtures(
        {
          ...
          localNodeOptions: postLondonlocalNodeOptions,
  ...
  describe('Send on a network that is not EIP-1559 compatible', function () {
    ...
    it('show expected gas defaults on a network not supported by legacy gas API', async function () {
      await withFixtures(
        {
          fixtures: new FixtureBuilder().build(),
          localNodeOptions: preLondonlocalNodeOptions,
          title: this.test.fullTitle(),
        },
        async ({ driver, localNodeServer }) => {
          await logInWithBalanceValidation(driver, localNodeServer);
