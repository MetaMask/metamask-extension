    ...
    it('should always follow the globally selected network after connecting and switching without refresh calls @no-mmi', async function () {
      ...
      await withFixtures(
        ...
        async ({ driver }) => {
          ...
          // TODO: check that the wallet network has changed too
          ...
          // after connecting the dapp should still be following the
          // globally selected network and therefore should have changed when
          // the globally selected network was manually changed via the wallet UI
          assert.equal(chainIdAfterManualSwitch, '0x1');
        },
      );
    });
  });
