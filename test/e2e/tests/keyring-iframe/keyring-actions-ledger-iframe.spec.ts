...
const mockLedgerIframeServerRedirect = async (mockServer: Mockttp) => {
  return [
    await mockServer
      .forGet('https://metamask.github.io/ledger-iframe-bridge/8.0.3/')
      .thenCallback(() => {
...
describe('Keyring Actions', function () {
  ...
  it('should be able to lock the wallet when the Ledger iframe is down', async function () {
    await withFixtures(
      {
        fixtures: new FixtureBuilder().build(),
        testSpecificMock: mockLedgerIframeServerDown,
  ...
  it('should be able to login when the Ledger iframe redirects', async function () {
    await withFixtures(
      {
        fixtures: new FixtureBuilder().build(),
        testSpecificMock: mockLedgerIframeServerRedirect,
        title: this.test?.fullTitle(),
      },
      async ({ driver }) => {
        await driver.navigate();
  ...
  it('should be able to create an account when the Ledger iframe redirects', async function () {
    await withFixtures(
      {
        fixtures: new FixtureBuilder().build(),
        testSpecificMock: mockLedgerIframeServerRedirect,
  ...
  it('should be able to lock the wallet when the Ledger iframe redirects', async function () {
    await withFixtures(
      {
        fixtures: new FixtureBuilder().build(),
        testSpecificMock: mockLedgerIframeServerRedirect,
