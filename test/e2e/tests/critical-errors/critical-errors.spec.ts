...
const EXTENDED_BACKGROUND_CONNECTION_TIMEOUT = 30_000;

const getBackgroundConnectionTimeout = () =>
...
describe('Critical errors', function (this: Suite) {
  it('shows critical error screen when background is unresponsive', async function () {
    const timeoutMs = getBackgroundConnectionTimeout();
  ...
  it('shows critical error screen when background takes longer than the liveness timeout to respond', async function () {
    const timeoutMs = getBackgroundConnectionTimeout();
    ...
    await withFixtures(
      {
        ...
        manifestFlags: {
          testing: {
            ...
            simulateDelayedBackgroundResponse: timeoutMs + 100,
          },
        },
        title: this.test?.fullTitle(),
      },
      async ({ driver }) => {
        ...
        // and ensure that the critical error screen remains in place even after the background
        // eventually responds.
        await driver.delay(timeoutMs + 1_000);
