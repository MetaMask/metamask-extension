...
export async function proxyTronBlockchainCalls(
  ...
  await proxyGetPath('/wallet/getchainparameters');
  ...
  await proxyPostPath('/wallet/getnextmaintenancetime');
  ...
  endpoints.push(
    ...
    await mockServer
      ...
      .always()
      .thenCallback(async () => {
        // TODO: This means most of the time we return a mocked response for those
        // so do we want to switch to a model where we seed the account with tokens
        // instead of mocking the response ?
        if (typeof localNode !== 'string') {
          return {
            statusCode: 200,
