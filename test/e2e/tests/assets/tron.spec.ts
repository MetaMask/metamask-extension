...
function buildTronAssetsFixture(): FixtureBuilderV2 {
  return new FixtureBuilderV2().withRemoteFeatureFlagController(TRON_ASSETS_REMOTE_FEATURE_FLAGS);
}
...
async function waitForVisibleTronToken(tokensTab: TokensTab, tokenName = "Tron"): Promise<void> {
  ...
  configureTronFixtureSession(
    ...
    ({ getDriver }) => {
      ...
      it("displays native TRX as the main balance for each asset account", async function () {
        ...
        console.log("Checking just created Tron account shows 0 TRX when native token is enabled");
        ...
        console.log("Checking funded account shows 106.072 TRX as the main balance");
        ...
        console.log("Checking portfolio account shows native TRX as the main balance");
      ...
      it("Low-value assets section hides tokens under $1 until expanded", async function () {
        ...
        await tokensTab.checkCollapsedTokenItemNumber(TRON_PORTFOLIO_MAIN_LIST_ASSET_NAMES.length);
