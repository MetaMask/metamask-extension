 ...
 * Generated at: 2025-11-21T18:27:44.080Z
 *
...
/**
 * Validates a "Transaction Finalized" event payload
 ...
 */
export function validateTransactionFinalized(
  event: EventPayload,
  ...
  if (errors.length > 0 && !assertFn) {
    ...
    throw new Error(`Validation failed for "Transaction Finalized" event:\n${errors.join('\n')}`);
  }

  if (assertFn) {
    // Check schema validation before running custom assertions
    if (errors.length > 0) {
      throw new Error(`Schema validation failed for "Transaction Finalized" event:\n${errors.join('\n')}`);
    }
