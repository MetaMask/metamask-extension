/**
 * Auto-generated validator for "Transaction Finalized Anon" event
...
/**
 * Validates a "Transaction Finalized Anon" event payload
 ...
 *                   Each function receives the event payload and array of validation errors.
 *                   If not provided, throws Error if there are validation errors.
 */
export function validateTransactionFinalizedAnon(
  event: EventPayload,
  ...
  if (event.properties.completion_time !== undefined && event.properties.completion_time !== null) {
    if (!(typeof event.properties.completion_time === 'string')) {
      errors.push('Property "completion_time" has invalid type. Expected: string');
    }
  }

  // Handle validation errors
  if (assertFn) {
    // Call custom assertion function(s) with event and errors
    ...
    throw new Error(`Validation failed for "Transaction Finalized Anon" event:\n${errors.join('\n')}`);
  }
}
