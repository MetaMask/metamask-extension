...
pwTest.describe('App Opened metric', () => {
  pwTest(
    'should send AppOpened metric when app is opened and metrics are enabled',
    async () => {
      await withFixtures(
        {
  ...
  pwTest(
    'should not send AppOpened metric when metrics are disabled',
    async () => {
      await withFixtures(
        {
          driverType: E2E_DRIVER.PLAYWRIGHT,
  ...
  pwTest('should send AppOpened metric when dapp opens MetaMask', async () => {
    await withFixtures(
      {
        driverType: E2E_DRIVER.PLAYWRIGHT,
