...
/**
 * Mocks the two Token API endpoints the spam cleanup depends on. Both are
 * registered with `.always()` so they keep answering for the whole run and win
 * over the global mocks in `mock-e2e.js`.
 *
