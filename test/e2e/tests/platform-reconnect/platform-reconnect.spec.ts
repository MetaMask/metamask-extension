  ...
  it('should maintain UI functionality after service worker is stopped', async function () {
    ...
    await withFixtures(
      ...
      async ({
        ...
        // Step 2: Log the extension ID
        console.log(`Extension ID: ${extensionId}`);

        // Step 3: Open chrome://serviceworker-internals/ in a new tab
        await driver.openNewPage('chrome://serviceworker-internals/');

        // Step 4: Find and stop the MetaMask service worker
        ...
        const stopServiceWorker = await driver.executeScript(`
          ...
          for (const item of serviceWorkerItems) {
            ...
            if (originText.includes('${serviceWorkerOrigin}')) {
              ...
              const stopButton = item.querySelector('button[data-command="stop"]');
              if (stopButton) {
                stopButton.click();
                return true;
              }
        ...
        // Step 5: Switch back to MetaMask and wait for reconnection
        await driver.switchToWindowWithTitle('MetaMask');

        // Wait 2 seconds for UI to initiate reconnection
        await driver.delay(2000);

        // Step 6: Verify UI is still working by adding an account
        ...
        // Add a new Ethereum account to verify functionality
        await accountListPage.addAccount({
          accountType: ACCOUNT_TYPE.Ethereum,
          accountName: 'Test Account After Reconnect',
        });

        // Verify the new account was created successfully
        await headerNavbar.check_accountLabel('Test Account After Reconnect');

        // Verify we can navigate back to the home page
        await accountListPage.closeAccountModal();
        await homePage.check_pageIsLoaded();
        await homePage.check_expectedBalanceIsDisplayed('0');

        console.log('Platform reconnect test completed successfully');
      },
    );
  });
});
