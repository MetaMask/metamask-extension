...
describe('Editing Confirm Transaction', function () {
  ...
  it('goes back from confirm page to edit eth value, gas price and gas limit - 1559', async function () {
    const ganacheOptions = {
      hardfork: 'london',
      accounts: [
        {
    ...
    await withFixtures(
      ...
      async ({ driver }) => {
        ...
        // enter max fee
        await driver.clickElement('[data-testid="base-fee-edit"]');
        await driver.delay(largeDelayMs);
        const maxBaseFee = await driver.findElement('[data-testid="base-fee-input"]');
        await maxBaseFee.clear();
        await maxBaseFee.sendKeys('8');
        await driver.delay(largeDelayMs);

        // enter priority fee
        const priorityFee = await driver.findElement('[data-testid="priority-fee-input"]');
        await priorityFee.clear();
        await priorityFee.sendKeys('8');
        await driver.delay(largeDelayMs);

        // edit gas limit
        const gasLimit = await driver.findElement('[data-testid="gas-limit-input"]');
