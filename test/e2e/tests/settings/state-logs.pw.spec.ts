...
pwTest.describe('State logs', () => {
  pwTest(
    'should download state logs for the account',
    async (
      // eslint-disable-next-line no-empty-pattern
      {},
      testInfo,
    ) => {
      // Chrome shows OS-level download prompt which can't be dismissed
      pwTest.skip(
        testInfo.project.name === 'chrome-e2e',
        'Chrome shows OS-level download prompt not dismissible in automation',
      );
      ...
      await withFixtures(
        ...
        async ({ driver }) => {
          ...
          const stateLogs = await getDownloadedStateLogs(
            driver,
            downloadsFolder,
          );
  ...
  pwTest(
    'state log file matches the expected state structure',
    async (
      // eslint-disable-next-line no-empty-pattern
      {},
      testInfo,
