...
pwTest.describe('Settings - Preferences and display', () => {
  pwTest(
    'validates language changes from preferences and display',
    async (
  ...
  pwTest(
    'validates "Dansk" localization across settings navigation',
    async (
      // eslint-disable-next-line no-empty-pattern
      {},
      testInfo,
  ...
  pwTest(
    'validates "Deutsch" localization in error messages',
    async (
      // eslint-disable-next-line no-empty-pattern
      {},
      testInfo,
      ...
      await withFixtures(
        ...
        async ({ driver }) => {
          ...
          await sendPage.checkRecipientValidationError(
            de.invalidAddress.message,
          );
        },
      );
    },
  ...
  pwTest(
    'validates "मानक हिन्दी" localization in tooltips',
    async (
      // eslint-disable-next-line no-empty-pattern
      {},
      testInfo,
  ...
  pwTest(
    'validates "العربية" localization in settings layout',
    async (
      // eslint-disable-next-line no-empty-pattern
      {},
      testInfo,
