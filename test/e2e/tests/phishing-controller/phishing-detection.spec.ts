...
describe('Phishing Detection', function (this: Suite) {
  ...
  const phishingRedirectBlockedHost = '127.0.0.1';
  const phishingRedirectDestination = 'https://metamask.github.io/test-dapp/';
  ...
  configureFixtureSession(
    'Phishing redirect protections',
    {
      ...
      testSpecificMock: async (mockServer: Mockttp) => {
        await setupPhishingDetectionMocks(mockServer, {
          statusCode: 200,
          blockProvider: BlockProvider.MetaMask,
          blocklist: [phishingRedirectBlockedHost],
          c2DomainBlocklist: [DEFAULT_BLOCKED_DOMAIN],
          blocklistPaths: [],
        });
      },
    ...
    ({ getDriver }) => {
      /**
       * Status codes 305 (via Location header) and 306 (Set-Proxy) header do
       * not automatically forward user agents to the new location, so we don't
       * test these status codes. Status code 304 (Not Modified) is not a
      ...
      const blocked = phishingRedirectBlockedHost;
      const destination = phishingRedirectDestination;
      ...
      before(
        'Set up redirect server and ensure extension is ready',
        async function () {
          ...
          await loginWithBalanceValidation(getDriver());
        },
      );
