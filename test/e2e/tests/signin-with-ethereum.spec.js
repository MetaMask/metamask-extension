  ...
  it('user should be able to confirm sign in with ethereum', async function () {
    ...
    await withFixtures(
      ...
      async ({ driver }) => {
        ...
        // Wait for signature request popup and check the displayed message
        ...
        const displayedMessage = await driver.findElement(
        ...
        assert.equal(
          `${await displayedMessage.getText()} ${await account.getText()}`,
          expectedSigninMessage,
        );
