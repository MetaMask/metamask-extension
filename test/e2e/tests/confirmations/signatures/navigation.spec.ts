  ...
  it('initiates and queues multiple signatures and confirms', async function () {
    await withRedesignConfirmationFixtures(
      ...
      async ({ driver, ganacheServer }: { driver: Driver, ganacheServer: Ganache }) => {
        ...
        await driver.clickElement('[data-testid="confirm-nav__previous-confirmation"]');
        ...
        //Confirm each message and verify
        await confirmAndVerifyResults(driver, publicAddress, "#signTypedDataVerify", "#signTypedDataVerifyResult");
        await confirmAndVerifyResults(driver, publicAddress, "#signTypedDataV3Verify", "#signTypedDataV3VerifyResult");
        await confirmAndVerifyResults(driver, publicAddress, "#signTypedDataV4Verify", "#signTypedDataV4VerifyResult");

      },
    );
  });

  it('initiates and rejects all', async function () {
    await withRedesignConfirmationFixtures(
      this.test?.fullTitle(),
      async ({ driver, ganacheServer }: { driver: Driver, ganacheServer: Ganache }) => {
        await unlockWallet(driver);
...
async function verifySignedTypeV3Confirmation(driver: Driver, origin: WebElement) {
...
async function verifySignedTypeV4Confirmation(driver: Driver, origin: WebElement) {
...
async function confirmAndVerifyResults(driver: Driver, publicAddress: string, verifyId: string, verifyResultId: string) {
  await driver.waitUntilXWindowHandles(3);
  await driver.switchToWindowWithTitle(WINDOW_TITLES.Dialog);
  await driver.delay(regularDelayMs);
