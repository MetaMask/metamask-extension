...
export function withTransactionEnvelopeTypeFixtures(
  ...
  const combinedMocks = async (
    ...
    const featureFlagsMocks =
      await mockRedesignedSendFlowFeatureFlag(mockServer);
    return [...baseMocks, ...[dialogSnapMocks], ...featureFlagsMocks];
  };
  return withFixtures(
    {
      dapp: true,
...
export async function mockEip7702FeatureFlag(mockServer: Mockttp) {
  return [
    await mockServer.forGet(FEATURE_FLAGS_URL).thenCallback(() => {
      return {
        ok: true,
        statusCode: 200,
        json: [
...
export async function mockRedesignedSendFlowFeatureFlag(mockServer: Mockttp) {
  return [
    await mockServer.forGet(FEATURE_FLAGS_URL).thenCallback(() => {
      return {
        ok: true,
