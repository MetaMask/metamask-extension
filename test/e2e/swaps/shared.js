...
const buildQuote = async (driver, options) => {
  await driver.clickElement('[data-testid="token-overview__button--swap"]');
  await driver.fill(
    'input[data-testid="prepare-swap-page__from-token-amount"]',
    options.amount,
  );
  await driver.delay(veryLargeDelayMs); // Need an extra delay after typing an amount.
  await driver.clickElement('[data-testid="prepare-swap-page__swap-to"]');
...
const reviewQuote = async (driver, options) => {
  ...
  await driver.findElement('[data-testid="review-quote__gas-fee-in-fiat"]');
...
const waitForTransactionToComplete = async (driver, options) => {
  await driver.waitForSelector({
    css: '[data-testid="awaiting-swap__header"]',
    text: 'Processing',
  });
  ...
  await driver.findElement({
    css: '[data-testid="awaiting-swap__main-description"]',
...
const checkActivityTransaction = async (driver, options) => {
  ...
  await driver.waitForSelector('[data-testid="list-item__title"]');

  const transactionList = await driver.findElements(
    '[data-testid="list-item__title"]',
  );
  ...
  await driver.findElement({
    css: '[data-testid="list-item__right-content"]',
  ...
  await driver.findElement({
    css: '[data-testid="transaction-list-item-details__tx-status"]',
    text: `Confirmed`,
  });

  await driver.findElement({
    css: '[data-testid="transaction-breakdown__value--amount"]',
    text: `-${options.amount} ${options.swapFrom}`,
  });
