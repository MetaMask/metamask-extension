  ...
  it('tests snap home page functionality', async function () {
    await withFixtures(
      ...
      async ({ driver }: { driver: Driver }) => {
        ...
        // Wait for the snap installation to be confirmed on the TestSnaps page.
        // This gives the AccountActivity WebSocket time to connect and process
        // its initial "chains up" notification before we switch to MetaMask,
        // preventing a race condition where that notification fires concurrently
        // with the global-menu drawer interaction and closes the drawer.
