...
const { loadBuildTypesConfig } = require("../development/lib/build-type") as {
  loadBuildTypesConfig: () => { features: { [x: string]: unknown }, buildTypes: { [x: string]: { features: string[] } } };
};
 ...
 * @returns
 */
function parseArrayOptionAsCsv<T extends string = string>(array: string[]): T[] {
  return array.flatMap<T>((v) => <T[]>v.split(','));
}
...
export const parseArgv = (argv: string[]) => {
  ...
  for (const item of defaults) excludeSet.has(item) || active.add(item);
  for (const item of include) excludeSet.has(item) || active.add(item);
 ...
 */
export function generateCli(argv: string[], buildTypes: string[], allFeatures: string[]) {
  const modeOptions = {
    group: "Build options:",
    alias: "m",
    description: "Enables/disables production optimizations or development hints.",
  ...
  function peekIsProduction(): boolean {
    if (typeof _isProduction !== "undefined") return _isProduction;
  ...
  const cli = yargs(argv)
    ...
    .completion("completion", "To enable bash/zsh completions, concat the script generated by running this command to your .bashrc or .bash_profile (or .zshrc for zsh).")
    ...
    .example("$0 --mode development --browser brave --browser chrome --zip", "Builds the extension in dev mode for Chrome & Brave; generates zip files for both.")
    .example("$0 completion", "Generates a bash completion script for the `yarn webpack` command.")
    ...
    .option("watch", {
      ...
      coerce: (value: "none" | "source-map" | "hidden-source-map") => {
      ...
      type: "number",
      default: <Manifest["manifest_version"]>2,
      choices: <Manifest["manifest_version"][]>[2, 3],
    }).option("type", {
      group: "Build options:",
      alias: "t",
      description: "Enables/disables feature flags based on the given type (main, beta, flask, etc).",
      type: "string",
      ...
      group: "Build options:",
      alias: "i",
      description: "Features you want enabled that are not already included in the selected build type.",
      array: true,
      type: "string",
      default: <typeof allFeatures>[],
      choices: allFeatures,
      coerce: parseArrayOptionAsCsv,
    }).option("exclude", {
      group: "Build options:",
      alias: "e",
      description: "Features you want disabled that are already included in the selected build type.",
      array: true,
      type: "string",
      default: <typeof allFeatures>[],
      choices: allFeatures,
