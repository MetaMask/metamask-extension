...
const { STATE_PROPERTIES_TO_CONTROLLER_MAP } = require('./transformMetamask');
...
function unflattenMetamaskCode(inputFilePath) {
  ...
  estraverse.replace(ast, {
    enter(node) {
      if (node.type === 'ObjectExpression' && node.properties.some(prop => prop.key && prop.key.name === 'metamask')) {
        const metamaskProperty = node.properties.find(prop => prop.key.name === 'metamask');
        if (metamaskProperty && metamaskProperty.value.type === 'ObjectExpression') {
          const unflattened = {};
          ...
          metamaskProperty.value.properties = Object.entries(unflattened).map(([controller, props]) => ({
            type: 'Property',
