...
/**
 * Generate warnings snapshot script for integration tests
 *
 * This script runs integration tests with a special flag that causes the console
 * capture system to automatically generate/update the integration tests snapshot file.
 *
 * Usage: yarn test:warnings:update:integration
 */
...
async function main(): Promise<void> {
  ...
  console.log('Running integration tests to capture warnings and errors...\n');

  // Check if we have existing temp files
  const tempDir = path.join(
    process.cwd(),
    'test',
    '.warnings-snapshot-temp-integration',
  );
  ...
  const env = {
    ...
    WARNINGS_SNAPSHOT_TYPE: 'integration',
  };

  try {
    const exitCode = await runTestCommand(
      ['test:integration', '--passWithNoTests'],
      env,
    );
    ...
    if (exitCode !== 0) {
      ...
      console.log('      yarn test:warnings:update:integration');
    ...
    console.log('   Check test/test-warnings-snapshot-integration.json');
