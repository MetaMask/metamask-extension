...
function createBuildConfiguration() {
  ...
  const bundlerOpts = {
    ...
    plugin: [],
    require: [],
    // fix resolver for exports field
    packageFilter: packageExportsCjsFix,
...
function composeIgnoreRegex(includeDependencies) {
  // create a regex to match everything other than the specified dependencies
  return new RegExp(
    `/node_modules/(?!${includeDependencies.map((d) => `(${d}/)`).join('|')})`,
    'u',
  );
}
