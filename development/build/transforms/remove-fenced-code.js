...
class RemoveFencedCodeTransform extends Transform {
    ...
    features = undefined;
    /** */
    shouldLintTransformedFiles = undefined;
    /** @private */
    _fileBuffers = undefined;
    /**
    ...
    /** @param {(error?: Error) => void} end
       * @returns {any}
       */
    _flush(end) {
        let fileContent, didModify;
        try {
            [fileContent, didModify] = removeFencedCode(this.filePath, this.features, Buffer.concat(this._fileBuffers).toString('utf8'));
        }
        catch (error) {
            return end(error);
        }
 ...
 * @param {Features} features - Features that are currently enabled.
 * @param {boolean} [shouldLintTransformedFiles=true] - Whether to lint transformed files.
 * @returns {(filePath: string) => Transform} The transform function.
 */
function createRemoveFencedCodeTransform(features, shouldLintTransformedFiles = true) {
    ...
    return function removeFencedCodeTransform(filePath) {
        ...
        return new RemoveFencedCodeTransform(filePath, features, shouldLintTransformedFiles);
    };
}
 ...
 * @returns {{ [x: string]: (params: string[] | undefined, filePath: string) => void; }} A mapping of command -> validator function.
 */
function CommandValidators({ features }) {
    return {
        [DirectiveCommands.ONLY_INCLUDE_IN]: (params, filePath) => {
            if (!params || params.length === 0) {
                throw new Error(getInvalidParamsMessage(filePath, DirectiveCommands.ONLY_INCLUDE_IN, `No params specified.`));
            }

            for (const param of params) {
                if (!features.all.has(param)) {
                    throw new Error(getInvalidParamsMessage(filePath, DirectiveCommands.ONLY_INCLUDE_IN, `"${param}" is not a declared build feature.`));
                }
            }
        },
    };
 ...
 * @returns {[string, boolean]} A tuple of the post-transform file contents and
...
function removeFencedCode(filePath, features, fileContent) {
    ...
    for (let i = 0; i < parsedDirectives.length; i++) {
        const { line, indices, terminus, command, parameters } = parsedDirectives[i];
        if (i % 2 === 0) {
            ...
            const blockIsActive = parameters.some((param) => features.active.has(param));
            ...
            // Forbid empty fences
            const { line: previousLine, indices: previousIndices } = parsedDirectives[i - 1];
...
function multiSplice(toSplice, splicingIndices) {
    ...
    if (splicingIndices.length > 2) {
        ...
        for (let i = 1; i < splicingIndices.length - 1; i += 2) {
            retainedSubstrings.push(toSplice.substring(splicingIndices[i], splicingIndices[i + 1]));
        }
    }

    // Get the last part to be included
    retainedSubstrings.push(toSplice.substring(splicingIndices[splicingIndices.length - 1]));
 ...
 * @property {ReadonlySet<string>} active Features that should be included in the output.
 * @property {ReadonlySet<string>} all All features that can be toggled.
 */
