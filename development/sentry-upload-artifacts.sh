...
function upload_bundle {
  ...
  printf 'Uploading %s\n' "${filename}"
  sentry-cli releases --org 'conflux-chain' --project 'portal' files "${release}" upload "${filepath}" "portal/${filename}"
}

function upload_sourcemaps {
  local release="${1}"; shift

  sentry-cli releases --org 'conflux-chain' --project 'portal' files "${release}" upload-sourcemaps ./dist/sourcemaps/ --url-prefix 'sourcemaps'
}
