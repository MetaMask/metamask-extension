...
export async function measureLoading(
  ...
  async function connect(target: Target) {
    ...
    const call = (method: string, params: Record<string, unknown> = {}) =>
      new Promise<Record<string, unknown>>((resolve, reject) => {
        ...
        if (socket.readyState !== WebSocket.OPEN) {
          reject(new Error(`CDP target disconnected: ${target.id}`));
          return;
        }
        const timer = setTimeout(() => {
          pending.delete(requestId);
          reject(new Error(`CDP timeout ${method} target ${target.id}`));
        }, 8000);
    ...
    socket.onclose = () => {
      for (const entry of pending.values()) {
        entry.reject(new Error(`CDP target closed: ${target.id}`));
      }
      pending.clear();
    };
  ...
  const observationSource = stripTypeScriptTypes(
    readFileSync(
      path.join(repo, 'development/perps/loading/market-observation.ts'),
      'utf8',
    ),
    { mode: 'strip' },
  ).replace(/^export /gmu, '');
  ...
  function verifyBuild(arm: string) {
    const runtimeProvenance = resolveMeasurementPath(
      root,
      `${arm}-runtime-provenance.json`,
    );
    const provenance = JSON.parse(
      readFileSync(
        existsSync(runtimeProvenance)
          ? runtimeProvenance
          : resolveMeasurementPath(root, `${arm}-build-provenance.json`),
        'utf8',
      ),
    ...
    for (const entry of provenance.files) {
      assert(
        createHash('sha256')
          .update(readFileSync(resolveMeasurementPath(buildRoot, entry.file)))
  ...
  async function sample(
    ...
    assert(
      !cold || existsSync(resolveMeasurementPath(out, 'prior-browser.pid')),
    ...
    verifyBuild(arm);
    writeFileSync(
      resolveMeasurementPath(out, 'lifecycle.json'),
      JSON.stringify(
        {
          beforePid,
          afterPid,
    ...
    try {
      ...
      const { result: globalObject } = await page.call('Runtime.evaluate', {
        expression: 'globalThis',
      });
      const { objectId } = globalObject as { objectId: string };
      const configured = await page.call('Runtime.callFunctionOn', {
        objectId,
        functionDeclaration:
          'function(mode, spans) { this.__perpsMeasure.mode = mode; this.__perpsMeasure.requiredSpans = spans; }',
        arguments: [
          { value: mode },
          { value: config.requiredSpans?.[arm]?.[mode] ?? [] },
        ],
      });
      assert(!configured.exceptionDetails, 'Measurement configuration failed');
      if (mode === 'account') {
        ...
        const configuredAccount = await page.call('Runtime.callFunctionOn', {
          objectId,
          functionDeclaration:
            "function(accountId) { const accounts = this.stateHooks.store.getState().metamask.internalAccounts; if (accounts.selectedAccount === accountId) throw new Error('Account-switch sample requires a different initial account'); const account = Object.values(accounts.accounts).find(account => account.id === accountId); if (!account) throw new Error('Expected existing account unavailable'); this.__perpsMeasure.expectedAddress = account.address; }",
          arguments: [{ value: accountId }],
        });
        assert(
          !configuredAccount.exceptionDetails,
          'Account-switch configuration failed',
        );
      }
      ...
      const helper = `${quote(process.execPath)} ${quote(path.join(repo, 'development/perps/loading/measure-loading.ts'))}`;
      ...
      const recipePath = resolveMeasurementPath(out, 'recipe.json');
      ...
      await run(
        ...
        resolveMeasurementPath(out, 'plan.json'),
      );
      await run(
        harness,
        [
          ...
          String(port),
          '--artifacts-dir',
          resolveMeasurementPath(out, 'recipe-run'),
          '--json',
        ],
        resolveMeasurementPath(out, 'recipe.log'),
      );
      ...
      assert(
        (await browserPid(port)) === afterPid,
      ...
      const finalWorkers = (await targets())
        ...
        .sort((left, right) => left.localeCompare(right));
      assert(
        finalWorkers.length === initialWorkers.length &&
          finalWorkers.every(
            (worker, workerIndex) => worker === initialWorkers[workerIndex],
          ),
        'Worker changed during measurement',
      );
      if (mode === 'background_resume') {
        const visibility = result.visibility as { state: string; at: number }[];
        const hidden = visibility.findIndex(
      ...
      const collectorSources = [
        ...
        sha256: createHash('sha256')
          .update(
            readFileSync(path.join(repo, 'development/perps/loading', file)),
          )
          .digest('hex'),
      }));
      const record = {
        ...
        formatterSha256: createHash('sha256')
          .update(
            readFileSync(path.join(repo, 'shared/lib/perps-formatters.ts')),
          )
          .digest('hex'),
      ...
      writeFileSync(
        resolveMeasurementPath(out, 'measurements.json'),
      ...
      writeFileSync(
        resolveMeasurementPath(out, 'failure.json'),
        JSON.stringify(
          {
            error: String(error),
  ...
  if (args[0] === 'resume') {
    ...
    try {
      ...
      assert((await cdp.evaluate('document.visibilityState')) === 'hidden');
      ...
      assert((await cdp.evaluate('document.visibilityState')) === 'visible');
    } finally {
      if (temporary) {
    ...
    assert(['before', 'after'].includes(args[0]), 'Specify before or after');
    assert(
    ...
    assert(/^[1-9]\d*$/u.test(args[2]), 'Specify a positive sample index');
    ...
    await sample(args[0], args[1], args[2], accountName);
  }
}
if (
  process.argv[1] &&
  path.resolve(process.argv[1]) ===
    path.join(process.cwd(), 'development/perps/loading/measure-loading.ts')
) {
  measureLoading().catch((error: unknown) => {
    console.error(error);
    process.exitCode = 1;
  });
}
