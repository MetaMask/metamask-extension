 ...
 * `scripts/load/init-sentry.ts`, `scripts/load/init-globals.ts`, and
 * `scripts/load/background.ts`) and the MV3 offscreen document (whose shared
 * bootstrap entries are `scripts/load/init-statehooks.ts` and
 * `scripts/load/init-globals.ts`).
 ...
 * forms are matched. `init-statehooks`, `init-sentry`, and `init-globals`
 * entries also exist for UI pages, but they share the same source files — and
 * those code paths run in the background page too, so changes to them
 * genuinely require an extension reload.
 */
const PRIVILEGED_PAGE_ENTRY_RE =
  /^(?:background|offscreen|init-statehooks|init-sentry|init-globals)(?:\.\d+)?$/u;
 ...
 * `init-globals` entries over the same modules) are only walked and hashed
 * once. A source change updates a module's hash under every runtime, so
 * deduplication doesn't lose change detection.
