 ...
 * MV2 background page (whose `<script>`s are
 * `scripts/load/init-state-hooks.ts` and `scripts/load/background.ts`) and the
 ...
 * forms are matched. UI pages also reference `init-state-hooks`, but that code
 * runs in the MV2 background page too, so changes to it genuinely require an
 * extension reload.
 */
const PRIVILEGED_PAGE_ENTRY_RE =
  /^(?:background|init-state-hooks|offscreen)(?:\.\d+)?$/u;
 ...
 * @param seen - Modules already visited; shared across entrypoints so modules
 * reachable from more than one privileged graph are only walked and hashed
