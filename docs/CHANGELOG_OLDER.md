...
- Removed the product tour from the Permissions Page (#28966)
...
- Enhanced MetaMask's browser support by updating code for better compatibility with older versions (#24907)
...
## [11.16.14]
### Fixed
- Fix bug that could result in failure of some swap transactions (#25488)

## [11.16.13]
### Changed
- Update Blockaid feature for latest security protections (#25442)

## [11.16.12]
### Added
- Add a marketing data collection consent toggle across Onboarding, Wallet Toast, and Settings, empowering users to control their participation in marketing analytics (#24605)

## [11.16.11]
### Fixed
- Ensure network requests for phishing lists are not sent if the Basic Functionality Toggle is off (#25306)

## [11.16.10]
### Fixed
- Capture Segment errors during initialization (#25253)

## [11.16.9]
### Fixed
- Fix an issue where Snaps would be unable to decrypt older state (#25172)

## [11.16.8]
### Changed
- Prepare for increasing the minimum Chromium version supported by MetaMask (#25142)
...
- [MMI] Updates MMI packages to latest versions (#24581)
...
input field and save it in that empty state. (#25109)
- Fix bug that can prevent updates to network data in connected dapps after manually switching the network in the wallet ui (#25127)
- [MMI] Fixed an issue that prevented MMI transactions from being sent correctly (#24947)
- [MMI] Better support for MMI Portfolio Dashboard with an update to the allow list  (#24992)
...
## [11.16.6]
### Added
- Add a Basic Functionality Toggle to settings, enabling users to opt-out of some features that send network requests to external services  (#23456)

### Changed
- Update MetaMask Chrome builds to Manifest V3 (#24746)
...
## [11.16.4]
### Changed
- Temporarily disable the opt-in modal (#25007)

## [11.16.3]
### Changed
- Update Trezor logo (#24343)

## [11.16.2]
### Fixed
- Fix gas fee displays on the Scroll network (#24854)

## [11.16.1]
### Changed
- Update Smart Transactions opt-in moda (#24771)

## [11.16.0]
### Added
- Revamped the UI for Snap installation and update processes (#23870)
- Added a 'Restore' onboarding option for seamless recovery from backup (#23739)
- Enabled ERC20 token detection on seven new networks (#24121)
- Activated token and NFT Auto-Detect by default for new mobile users (#23700)
- Introduced the Filecoin SVG logo (#24039)
- Enhanced transaction confirmations with a tooltip and icon for busy network status (#24104)
- Added a setting to experiment with redesigned confirmation pages through a feature flag (#24052)
- Introduced 'Developer Options' in settings for easier app state management and feature toggling during development (#22382)
- Automated Layer 1 gas fee inclusion for Scroll network transactions (#23991)
- Enhanced the Smart Transactions status page to display simulation details for native balance changes (#24075)
- Enhanced security by disabling paste functionality in the Snap removal confirmation field (#23993)
- Launched Transaction Insights V2 and Signature Insights to all users (#23630)

### Changed
- Updated the messaging for Blockaid warnings to provide clearer information to users during transactions (#24037)
- Optimized Smart Transactions by hiding the 'retry' button and displaying cancelled transactions only for Swaps in the Activity tab (#24176)
- Refreshed the Smart Transactions Opt In modal with updated UI and content, alongside a rebranding to 'Smart Transactions' (#24053)
- Eliminated the product tour from the home page (#24051)
- Refreshed the look of toggle buttons (#24090)

### Fixed
- Improved the redirect screen by caching dapp icons to prevent disappearance during loading (#23999)
- Fixed PPOM/Blockaid functionality in MV3 builds (#24115)
- Fixed offscreen document loading errors in MV3 (#24083)
- Improved account connection process by automatically filtering out duplicate or already connected accounts (#24057)
- Fixed an issue where the token list was cleared upon network switch (#24212)
- Improved how NFTs appear in MetaMask by addressing an issue with fetching details for certain NFTs (#24029)
- Resolved an issue where fiat prices for ERC20 tokens were not updating (#24021)
- Deprecated the Mumbai and Linea Goerli test networks (#23695)
- Fixed the issue where the checkmark icon was missing in the confirmation message after importing an ERC20 token (#24005)
- Resolved an issue where transactions triggered during a pending signature request caused an indefinite loading state (#24173)
- Implemented UI improvements on redesigned signature request pages based on feedback (#24135)
- Enhanced the 'scroll to bottom' button with improved hover effects (#24147)
- Enabled Ledger wallet users to sign signature requests seamlessly on the newly designed confirmation pages (#23780)
- Optimized gas estimation failure warnings to display a single alert (#23886)
- Improved the readability of footer button labels in the new-network component for both light and dark modes (#23988)
- Fixed synchronization issues with per-dapp selected network preferences across toggle changes (#24093)
- Fixed a problem where changing site permissions didn't immediately update the network used by the site (#23525)
- Corrected the display and spacing of the permissions tooltip within the connect accounts modal (#24098)
- Disabled the 'Connect more accounts' button when all accounts are already connected (#24084)
- Improved back navigation from Connected Accounts (#24085)
- Updated the wallet, permissions, and connections screens to align with the latest design improvements (#24023)
- Improved the permissions page to correctly display all permissions across networks (#23986)
- Enhanced the connections page to accurately display connected dApp information and status (#24022)
- Fixed an issue to ensure smooth disconnection of a single account from a dApp without breaking the page (#23935)

## [11.15.6]
### Changed
- Disable smart transaction opt-in modal (#24715)

## [11.15.5]
### Changed
- Update copy in Smart Transactions opt-in modal(#24461)

## [11.15.4]
### Changed
-  Update UI for the Smart Transactions Opt In modal (#24441)

## [11.15.3]
### Changed
- Smart transaction improvements (#24340)
...
### Fixed
- Add animation on the smart transaction status page (#24389)

## [11.15.2]
### Fixed
- Ensure smart transaction modal is shown for users upgrading from previous versions (#24377)
...
## [11.15.0]
### Added
- Expanded Smart Transactions (STX) functionality to include non-Swaps transactions on Ethereum Mainnet for users opted into STX (#23155)
- Added Base Mainnet and removed Gnosis and Celo networks from popular networks list (#23880)
- Enhanced balance fetching efficiency by supporting bulk balance checks on Linea, Aurora, Base, and ZkSync (#23436)
- Introduced a 'Learn more' link to the simulations toggle in privacy and onboarding settings (#23890)
- Introduced a confirmation step for approving higher token spending limits (#23560)
- Added a dismissible alert for users migrated from OpenSea to Blockaid on unsupported networks (#23743)
- Implemented support for signature requests with a redesigned interface (#23539)
- Introduced support for the new Linea Sepolia network and deprecated the Linea Goerli network (#23459)

### Changed
- Removed outdated announcements for 'Snaps Open Beta' and 'Buy & Sell' features (#23940)
- Enhanced Smart Transactions swaps with detailed simulation views (#23963)
- Updated the Arbitrum logo (#23969)
- Enhanced the appearance of links in Snap dialogs (#23840)
- Improved the layout of the security alert option in the settings page (#23718)
- Enhanced the appearance of security alerts by fine-tuning their spacing (#23900)
- Unified the color scheme for secondary titles in settings (#23764)
- Adjusted asset icon display and sizing in simulation details (#23760)
- Enhanced display for near-zero amounts and updated native token visuals for all chains (#23711)
- Enhanced default token name visibility by always using the remote token list for petnames (#23919)
- Improved transaction confirmation clarity by hiding totals for successful simulations (#23899)
- Updated transaction controller to display balance changes for wrapped ERC-20 tokens and legacy ERC-721 tokens (#23915)

### Fixed
- Enhanced the send flow by fine-tuning input fields to keep trailing zeros and decimals (#23808)
- Resolved token detection and import issues (#23798)
- Implemented a deprecation warning for users switching to the Mumbai network (#23846)
- Corrected the display of crypto balances in the presence of scam network warnings (#23645)
- Enhanced UI for simulation details by wrapping and adding tooltips to long asset names and amounts (#23768)
- Expanded the deprecation warning for OpenSea security alerts to include typed signature confirmations (#23743)
- Fixed an issue in Firefox where security alerts weren't displaying due to permission settings (#23958)
- Resolved an issue where the loading indicator overlapped with the UI in security alerts (#23927)
- Resolved an issue with an infinite loading spinner on blockaid alerts (#23480)
- Improved accessibility in the app's network selection by ensuring screen readers announce network names more clearly (#23842)
- Resolved an issue preventing QR code-based hardware wallet connections (#23903)
- Resolved an issue where the connected accounts modal would crash if a dApp with an installed Snap was accessed (#23928)

## [11.14.5]
### Fixed
- Prevent users from making fund loss errors while editing transactions by removing the edit button when on any confirmation screen for a transaction proposed by a dapp (#24322)
- Reduce failed simulations on NFT mint confirmations ([#24350]https://github.com/MetaMask/metamask-extension/pull/24350)

## [11.14.4]
### Fixed
- Fix bug that could cause safe-transfer-from transactions to be converted to transfer-from transactions, by removing the edit button on the safe-transfer-from confirmation screens (#24287)

## [11.14.3]
### Fixed
- Fixed getMinimumGasTotalInHexWei error by updating the "fee_market" gasEstimateType logic (#24287)

## [11.14.2]
### Changed
- Update Blockaid to ensure user's have the latest protections (#24171)

### Fixed
- Ensure gas estimates always reflect the correct network (#24253)

## [11.14.1]
### Fixed
- Fix crashes on transaction screens for some users with es_419, pt_BR, pt_PT, zh_CN, zh_TW locales (#24068)

## [11.14.0]
### Added
- Transaction Simulations
    - Integrated transaction simulation features into all builds, with the option to disable via privacy settings (#23651)
    - Added a toggle for transaction simulations under settings, with a feature highlight in the 'What's New' modal (#23604)
...
- Introduced a feature to toggle visibility of private keys during import (#23371)
- Enhanced network addition with a direct link to MetaMask Portfolio bridge (#23475)
- Added a MetaMask Bridges link in Swaps (#23437)
- Enhanced the display of watched NFTs with default petnames and icons (#23438)
- Introduced support for Base network in Blockaid validations (#23452)
- Enabled direct redirection to Snap confirmations from the home page (#23584)
- Introduced support for localized Snap manifests (#21909)
- Enabled Base network support for swaps (#23394)
- Introduced a 'What's New' notification for MetaMask Portfolio (#23359)
- Activated Snaps home pages in the stable release (#23383)
- Added a custom screen for `increaseAllowance` contract interactions (#23883)
- [MMI] Enhanced the custody view with a search bar (#23492)
- [MMI] Implemented message text verification for contract call operations (#23462)

### Changed
- Updated security settings by removing the OpenSea provider option (#23546)
- Transitioned users from the OpenSea security alert feature to Blockaid (#23460)
- Improved display of accounts within Snaps Permissions UI (#23454)
- Removed the banner from the account list menu (#23447)
- Updated the Fuse mainnet logo to its latest branding (#23542)
- Enhanced the per-dapp network selection feature to ensure subscribe and filter requests adhere to the selected network (#23386)
- Improved wallet setup by allowing onboarding and unlocking without needing Infura mainnet connectivity (#23388)
- Enhanced the hardware wallet import process with clearer instructions to switch networks if accounts appear empty (#23443)
- Implemented minor adjustments to Insights V2 (#23248)
- Updated snaps-registry to the latest version to address validation issues with certain signature lengths (#23380)
- Improved Snap layout by adopting gap for element spacing (#23369)
- [MMI] Updated MMI support URLs to direct users to the new support site (#23656)

### Fixed
- Refined the asset picker design (#23264)
- Improved token matching by making symbol checks case insensitive (#23637)
- Fixed an issue where very small transaction amounts were displayed in scientific notation (#23341)
- Resolved an issue where approving a transaction could inadvertently reset its status due to concurrent updates (#23647)
- Corrected the extension popup client connection count to support multiple open popups (#23583)
- Resolved text overflow in the Snaps list for long Snap IDs or names (#23517)
- Corrected an issue where domain resolution results persisted on the send screen after inputs were cleared (#23595)
- Resolved an issue causing a crash when attempting to connect to multiple Snaps simultaneously (#23646)
- Resolved an issue where Snap names were not displayed in the permission list due to a missing prop (#23627)
- Corrected the calculation of maximum gas displayed and the gas limit determination for swap transactions (#23545)
- [FLASK] Updated the send screen placeholder text in Flask to accurately reflect support for full domain resolution, not just ENS (#22192)

## [11.13.3]
### Fixed
- [MMI] Adds required variable to prod env (#23879)

## [11.13.2]
### Fixed
- [MMI] Updates MMI packages to latest versions (#23841)
- [MMI] Updates MMI RPC methods to use latest method (#23827)
- [MMI] Adds MMI Sentry DSN key (#23826)

## [11.13.1]

## [11.13.0]
### Added
- Added support for ERC-1155 tokens across multiple chains (#23297)
- Introduced a modal with a block explorer link for attempts to cancel already confirmed transactions (#22943)
- Added a message section to the personal sign page (#22766)
- Enabled Blockaid support for Sepolia (#23298)
- Added Snaps authorship modals (#22163)
- Added a prompt for users to connect their current account if it's not connected to a dApp (#22966)
- Implemented updated Linea gas fee estimates for more accurate transaction costs (#22918)

### Changed
- Removed Goerli network from settings and added warning modal for manual attempts to add it (#22995)
- Enhanced display names for first-party contracts (#23092)
- Added new footer buttons for easier account connections and management in the Connected Account view (#23241)
- Update Palm network logo with support for both light and dark modes (#23191)
- Improved account naming in transactions (#23046)

### Fixed
- Improved interface by disabling transaction buttons for accounts that can't sign (#23178)
- Fixed an issue where account permissions persisted after account removal (#23247)
- Fixed a glitch that prevented some NFTs from being listed due to a mix-up with token types (#22822)
- Fixed an issue with ERC1155 token transfers where decimal input amounts were incorrectly interpreted as hexadecimal (#23151)
- Corrected the display of suggested ticker symbols to only appear for new custom networks (#23169)
- Improved flexibility for dApp transactions, enabling them from any authorized account (#23269)
- Updated the Transaction Confirmation to clearly show estimated and maximum fees (#23203)
- Fixed missing Chain Name in the 'Report False Positives' (#23195)
- Updated network tracking to only include sites you've connected to (#23130)
- Excluded preinstalled Snaps from the Snaps list view (#23167)
- Updated Snap UI to ensure input labels are properly shown (#23153)
- Fixed an issue ensuring account avatars are displayed in both popup and full screen views (#23265)
- Improved send flow by ensuring pinned accounts are prominently displayed at the top and correctly labeled (#23176)
- Updated the Connections Page to include tabs and show connected accounts when an account is connected (#23125)
- Resolved text highlight color conflicts by reverting to hex code usage (#23188)
- [MMI] Simplified the home screen by removing the Buy, Sell, and NFT options (#23199)

## [11.12.4]
### Fixed
- Ensure native network balance is visible in home screen balance display (#23614)

## [11.12.3]
### Fixed
- [MMI] Fixes an error related with a missing code fence, when the MMI build didn't have the blockaid feature (#23516)
- [MMI] Fixes a bug for some custodians that don't send us the env property when connection to MMI (#23494)

## [11.12.2]
### Fixed
- Fix transaction confirmations so that they correctly show estimated fees instead of max possible fees
    - For non-layer 2 network, and non-token send, transactions(#23203)
    - For layer 2 networks and for erc20 transfers (#23511)

## [11.12.1]
### Changed
- Updated styling for missing petnames to prevent misinterpretation as malicious (#23458)

## [11.12.0]
### Added
- Introduced deprecation warnings for Arbitrum Goerli and OP Goerli test networks (#23071)
- Added origin details to personal sign page (#22763)
- Enhanced snap functionality with dynamic interfaces, customizable timeouts, and faster installations (#22828)
- Added a 'Connect Account' button in the account list menu for easier account connections (#22941)
- [FLASK] Added signature insights (#22485)
- [MMI] Enhanced error tracking in MMI controllers (#22994)
- [MMI] Updated MMI dependencies and refactored code (#22546)
...
- Enhanced Send Flow by displaying account names during search (#22824)
- Improved send flow by directly showing NFTs tab when sending an NFT (#23033)
- Simplified send flow by removing outdated gas options (#22951)
- [MMI] Improved custodian search by using environment names for unique identification (#23073)
- [MMI] Enhanced MMI keyring configuration for improved mv3 version compatibility (#22968)

### Fixed
- Improved accuracy of fee details for Optimism transactions (#22997)
- Ensured password prompt appears every time 'Show private key' is selected (#22867)
- Adjusted menu positioning ensuring visibility of all options (#22889)
- Corrected asset list to display native currency symbols instead of fiat on the home page (#22760)
- Fixed tokens being added to incorrect networks after switching networks (#22814)
- Fixed an issue where NFTs disappeared or couldn't be re-imported after account switching (#22856)
- Fixed an issue with saving very low default gas fees (#22790)
- Removed the subtle background behind setting icons (#22982)
- Improved 'See Details' in alerts to stop auto-scrolling (#22932)
- Improved security checks for transactions (#22978)
- Ensured gas fee editing remains accessible even when currency rate checks are disabled (#22890)
- Updated modal overlay for better visibility and focus on content in both light and dark modes (#23102)
- Fixed behavior for manually added networks to show a modal with the option to switch, rather than auto-switching (#22832)
- Corrected link alignment in Snap UI (#23045)
- Resolved an issue that prevented cancelling permission requests without crashing the extension (#22915)
- Fixed badge color in app-header to accurately reflect connection status (#23126)
- Fixed a glitch in the Product Tour where step numbers were not displaying correctly (#23100)
- Enhanced handling of long account names (#23096)
- Resolved issue with tabs overlapping content in the send flow (#23028)
- Ensured consistent balance display in Eth Overview and account list (#23059)
- [MMI] Corrected custody type determination to support various types (#22950)

## [11.11.4]
### Changed
- Enable Snaps home pages (#23383)

### Fixed
- Fix intermittent Snaps installation issues due to faulty validation (#23380)

## [11.11.3]

## [11.11.2]
### Changed
- Update the image in the Staking button What's New popup (#23330)

### Fixed
- [MMI] Fixed bug that prevents MMI users from submitting multiple Txs (#23342)
...
## [11.11.1]
### Added
- Adds a staking button to the mainnet Ethereum token list item  (#22347)

## [11.11.0]
### Added
- Added 'Pet Names' feature, allowing users to see preferred or suggested nicknames in the places of addesses
    - Added 'What's New' popup for Petnames feature (#22780)
    - Introduced toggle for Petnames in experimental settings (#22456)
    - UI enhancements for initial petnames release, including new "Recognized" category and visual refinements (#22772)
    - Enhanced token name display with pet names (#22734)
- Added title to Personal Sign component in confirmation design (#22749)
- Added progress indicator for scanning QR codes with hardware wallets (#20947)

### Changed
- Moved security alerts from Experimental to Security & Privacy settings (#22813)
- Updated BlockaidBannerAlert to support false positive reporting for failed types (#22742)
- Enhanced BlockaidBannerAlert functionality and display (#22625)
- Disabled smart swaps for Snap accounts (#22731)
- Disabled MetaMask on Battle.net to fix a 2FA login issue (#20396)
- Revised warning copy for mismatched chainID and currency symbol when adding custom networks (#22648)
- Improved UX to display multiple custom networks with the same ID but different RPC URLs in network selection (#22693)
- Updated the connections icon to display the connected dapp icon (#22634)
- Added title to Personal Sign page (#22749)
- Update padding in accounts details modal (#22775)
- [MMI] Refactored display of custodian deep link to improve efficiency and fix potential race conditions (#22825)
- [MMI] Hid the new buy & receive button under MMI build for a cleaner interface (#22384)

### Fixed
- Fixed cancel transaction signing from activity list (#22676)
- Fixed incorrect account name display in account details and receive list (#22844)
- Fixed IPFS NFTs fetching issue for manually imported NFTs (#22627)
- Fixed "send max" ETH calculation issue to adjust for gas changes (#22694)
- Fixed sign button color and updated deprecated components in SignatureRequestOriginalWarning for visual consistency (#22741)

## [11.10.1]
### Fixed
- Fix custom network editing, via Settings, for some networks (#23140)

## [11.10.0]
### Added
- Added preset network image avatars in the 'Select a network' pop-up (#22643)
- Expanded Blockaid banner support to include send requests from wallets (#22321)
- Expanded Blockaid banner support to include BNB chain, Polygon, Arbitrum, Optimism, Avalanche, and Linea networks (#22633)
- Added support for imToken Wallet using EIP-4527 (#21804)
- [FLASK] Introduced user operation support, enhancing transaction handling and alert display for user operations (#22469)
- [FLASK] Added support for the Lattice hardware wallet in MV3 (#22528)
- [FLASK] Added a 'You're sending to a contract' warning to the new send page (#22551)

### Changed
- Improved error messaging for Ledger connection issues (#21038)
- Updated 'What's New' and 'Settings' to communicate the extension's additional network coverage (#22618)
- Enhanced Token List layout for clearer token name and value display (#22601)
- Updated the date for the upcoming user survey displayed on the home screen (#22581)
- Improved QR code density for compatibility with the ZERO hardware wallet (#22135)
- Updated snaps packages to the latest versions (#22595)
- Updated the Gas API URL to ensure accurate gas fee calculations for Send and Swap transactions (#22544)

### Fixed
- Fixed an issue where clicking on the redirection link did not close the notification window (#22583)
- Improved Blockaid 'What's New' image display for users in OS dark mode (#22649)
- Added Blockaid dark mode support in the 'What's New' feature and introduced a new theme management utility (#22613)
- Ensured all security alert banners correctly trigger events (#22553)
- Updated 'What's New' Blockaid image to have a transparent background (#22539)
- Fixed unresponsiveness on the phishing warning page (#22645)
- Improved Trezor integration by updating to the correct version of the SDK (#22591)
- Improved account list links to display the correct explorer domain based on the selected network (#22483)
- Improved the reliability of snap installations by resolving an underlying technical issue (#22602)
...
## [11.9.4]
### Fixed
- Fix security advisory https://github.com/advisories/GHSA-78xj-cgh5-2h22 (#22910)
...
## [11.9.2]
### Fixed
- [MMI] Patches `@metamask/accounts-controller` to fix an issue with MMI where custodial keyrings could nto be found (#22730)
...
- Add ([EIP-6963](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-6963.md)) Provider (#19908)
...
## [11.3.0]
### Added
- Display total fiat balance on home screen (#20941)
- Give the user the ability to opt out of 3rd party network validation when adding a new network (#20816)
- Adds conversion buttons to buy, receive, and learn more about NFTs when an account is empty. (#21049)
- What's New modal for the latest Buy and Sell features (#20965)
- [Flask] Suggest names for Ethereum addresses in signature requests using installed snaps (#20959)

### Changed
- Move the token detection prompt to the top of the assets list (#20932)
- Adds address validation to the Import NFT modal (#21028)
- Add error message to the Import NFT modal if an NFT with that tokenId already exists (#20940)
- Show a warning when sending 0 tokens (#21091)
- Enable bridge button on the zkSync network, and for some tokens on other networks (#21085)
- Stop displaying a mismatched account warning on the Sign In With Ethereum page (#21107)
- Ensure "Enhanced Token Detection" is toggled off when opening "Advanced Configuration" during onboarding (#20960)
- Improve loading speed/performance upon opening MetaMask (#20843)
- Ensure logos are displayed on "Popular Networks" added from within MetaMask (#20895)
- Show more characters in the addresses in the Connected Sites account list (#21048)
- Replaces the "Add / Import / Hardware" links with a single button that takes the user to another quick modal to add, import, or connect a hardware wallet.(#21081)
- Disable the "Buy" button on the Sepolia network (#20839)
- Update the "Buy" button to a "Buy & Sell" button (#20891)
- Only display the Notification item in the dropdown menu if the user has installed a snap using the notifications permission (#20913)
- Update Snaps Settings screen (#21061)
- Site connection icon now indicates a connection if a dapp is connected to a snap (#20811)
- Update copy on the remove snap modal (#21065)
- Update the padding of the transaction insight dropdown (#21022)
- [Flask] Suggest names for Ethereum addresses in signature requests using services such as ENS and Etherscan (#20831)

### Fixed
- Fix to ensure contract address is pre-populated, and NFT is removed from token list, when converting a token to an NFT  (#20747)
- Fix autodetect tokens link so that the user is correctly taken to the right scroll position in settings (#20978)
- Ensure safe batch NFT transfers show display recipient address correctly (#21042)
- Fix bug that can prevent MetaMask from loading, and be stuck on the loading screen, on Firefox (#20992)
- Ensure correct network icon is displayed in the "You have switched to" modal when switching to "Popular Networks" (#21016)
- Ensure disconnecting a single dapp from a snap only disconnects that dapp (#20983)
- Fix the Snap npm link so that it leads to the stated version of the npm package (#20897)
- Fix to prevent crashes when switching network during a snaps confirmation (#21088)
- Fix issue that could cause crashes when attempting to render certain NFTs (#21418)
...
## [11.1.2]
### Fixed
- Prevent crashes for users that have NFTs without an image and/r limited image data (#21176)

## [11.1.1]
### Fixed
- Ensure NFT settings notice in the NFT import modal is shown and hidden correctly, and that the modal is hidden when clicking the link to settings (#21100)
- Modify settings toggle copy to more accurately describe behaviour (#21109)
...
- Improve error diagnostic information
  ...
  - Capture additional errors when state migrations fail (#20427)

### Fixed
- Fix bug where state was temporarily incomplete upon initial load (#20468)
...
## [10.34.4]
### Changed
- Updated snaps execution environment (#20420)

## [10.34.3]
### Fixed
- Ensure users phishing warning list is properly updated (#20381)
- Fix inaccurate info in swaps flow for zero-balance tokens (#20388)
- Fix 'Global Menu Explorer / Account Details' What's New notification display (#20371)

## [10.34.2]
### Added
- Add Address Details and View on Explorer to Global Menu (#20013)

## Changed
- Increase copy clipboard time (#20008)
...
- Scroll to selected account when opening account list menu (#20166)
...
- Improved UI for downloading state logs on Chromium-based browsers (#19872)
...
### Fixed
- Fixed bug that could cause loss of network or token data for users upgrading from old versions (#20276)
- Fix crash on open of MetaMask for some users that have old network data in state (#20345)

## [10.34.1]
### Fixed
- Fix bug that could cause a failure in the persistence of network related data (#20080)
- Fix possible crash when opening the network menu (#20181)
...
- Update trezor-connect to v9. Introduced trezor changes are documented at [trezor-suite](https://github.com/trezor/trezor-suite/blob/develop/packages/connect/CHANGELOG.md). (#18302)
...
- [FLASK] Add updated version of the Snaps settings UI ([#18438](https://github.com/MetaMask/metamask-extension/pull/18438), (#18775))
...
## [10.30.1]
### Fixed
- Disable Flask RPC test to fix failing build (#19011)
...
- Bump contract-metadata version, so that tokens added ([v2.3.0](https://github.com/MetaMask/contract-metadata/pull/1169)) and ([v2.3.1](https://github.com/MetaMask/contract-metadata/pull/1173)) are included in the default MetaMask token lists (#18589)
...
- [FLASK] Fix window overflow issues with snap UI text (#18169)
...
### Changed
- UX: Localize the avatar-favicon description text (#18132)
...
## [10.26.1]
### Fixed
- Fix main build by modifying desktop build steps (#18112)
...
## [10.25.0]
### Added
- Add new app translations (#15999)
- Add Celo to the popular custom network list (#16745)
- [FLASK] Add markdown formatting capabilities for Snaps UI (#16911)
- [FLASK] Add basic renderer for Snaps UI (#16605)

### Changed
- No longer displaying warning upon switching networks when there are no pending confirmations (#17179)
- Remove 'Verify contract details' link on Signature Request screen when there is no verifying contract (#17128)
- Show portfolio tooltip when "Protect your funds" popover is not on screen (#17084)
- Set default theme to dark when system preferred theme is set to dark (#15870)
- Require a username when adding a new contact to the address book (#17044)
- Update styles for "Transaction security check" toggle in settings (#16830)
- Display set block explorer in copy on transaction completion popup (#16300)
- Updating text colors for Sepolia and Goerli networks (#16928)
- Include L1 fees for quotes on Optimism (#16998)
- Factor in Optimism's L1 fees in to fee total for approval transactions (#16929)
- Update "Share your feedback" link location for Beta versions (#16853)
- Remove New Address Alert in send transaction flow (#14811)
- Allow submissions of transactions with a gas or priority fee of 0 (#16651)
- Disable eth_sign by default, allow users to toggle it back on (#17308)
- [FLASK] Use custom UI for transaction insights (#16912)
- [FLASK] Use custom UI dialogs (#16912)

### Fixed
- Fix tab redirect issue when a non-English language is set (#17155)
- Fix duplicate gas fee estimate displayed on testnets (#17207)
- Fix app-breaking error when an alphabetical character is entered in the spending cap field for token allowance (#17117)
- Fix alignment issues in contacts tab (#17171)
- Fix gas fee currency symbol in transaction details activity log (#16948)
- Fix function type display in sign typed message confirmation screen (#17077)
- Fix duplicate button issue on Ledger connectivity screen (#17130)
- Fix issue in settings search input when spaces are entered in-between terms (#17108)
- Fix incorrect transaction type when transaction data is not prefixed with '0x' (#17055)
- Fix account name collision issue (#16752)
- Fix caching issue with detected token data (#16866)
- Fix warning display on confirmation screens when a transaction is expected to fail (#17437)
- [FLASK] Fix race condition with transaction insights (#16956)
- [FLASK] Fix crash after Snap confirmation approval (#16864)
...
## [10.24.1]
### Added
- Ensure app name appears for Taiwanese language speakers in the extension stores (#17304)

## [10.24.0]
### Added
- Add NFT setApprovalForAll warning popover when approving the NFT Colleciton (#16195)
- Add "What's New" notification for Security & Privacy (#16783)
- Add informational message on Signature request screen as a user guidance (#16600)
- Add ability to opt out from getting balances as a batch request for all loaded accounts (#16746)
- Add Signature request warning modal (#16225)
- Add Improved Token Allowance experience toggle under Experimental Settings (#16291)
- Add "What's New" Notification for Improved token Allowance Experience (#16465)
- Add new IT translations (#15748)
- Add new zh_TW translations and apply a variety of fixes (#11212)
- Add ES translations for getting Ethereum Accounts permission message (#15660)
- Add "Verify contract details" link to SetApprovalForAll confirmation screens (#15756)
- Add Reject Transactions modal to be present in the footer of the Approve screen (#16832)
...
### Changed
- Update secondary copy and remove the Address component from SetApprovalForAll and NFT Approve screens (#16292)
- Update background color for picker network (#16466)
- Update network colors with design tokens (#16543)
- Update all background-default hovers colors (#16519)
...
- Migrate from deprecated `@metamask/controllers`to the new controller packages (#16547)
- Sanitize data ensuring we don't send any privacy sensitive information to Sentry errors (#16780)
- Track when token balance is changed to update the balance value on the Approval screen (#16964)
- Swaps: ensure 0% slippage on Arbitrum for wrapping/unwrapping ETH (ETH -> WETH or WETH -> ETH) (#16778)
- [Beta] Update `BETA` to sentence case (#16590)
- [Beta] Update text on the Welcome screen (#16489)
- [FLASK] **BREAKING:** Snaps are now required to request the `endowment:rpc` permission to receive RPC requests on `onRpcRequest` (#16673)
...
- [FLASK] **BREAKING:** Removed `wallet_enable` and `wallet_installSnaps` in favor of `wallet_requestSnaps` (#16525)
- [FLASK] **BREAKING:** The `wallet` global exposed to Snaps has been replaced with two new globals: `snap` and `ethereum` (#16525)
...
- [FLASK] **BREAKING:** Named parameters are now required in all Snaps RPC methods (#16525)
- Security and Privacy Settings Re-org (#16756)

### Removed
- [FLASK] **BREAKING:** Removed `snap_getAppKey` (#16525)
  - Snaps that need snap-specific entropy can use `snap_getEntropy` instead
- Remove ONBOARDING_V2 flag for release (#16865)

### Fixed
- Fix resolving to the Account nickname or contact name when scanning the QR code in the Send screen (#16204)
- Fix not being able to reject multiple Signature requests (#16199)
- Fix issue that prevents switching to localhost using the API (#16707)
- Fix unable to proceed with tx when there is "Insufficient funds for gas", by adding the estimated gas fee section on the Send screen (#14634)
- Fix Add Contact by disabling the Save button if the input fields are empty (#16233)
- Fix Token Detection displaying a token without balance, by updating the user state `tokensChainsCache` from array to object (#16535)
- Fix message not rendering properly on Sign Typed Data v4 screen, by supporting nested arrays and arrays with defined length (#16552)
- Fix German translation for `lightTheme` (#16517)
- Fix displaying Collectibles Approval screen instead of ERC20 Approval screen when the token standard property is undefined (#16765)
- Fix token balance precision on Confirm Token Approval page from Token Allowance flow (#16934)
- [FLASK] Clear notification state on restore (#16503)
- [FLASK] Fix a crash that happens after snap install (#16526)
- [FLASK] Fix usage of wrong `ethereum` global for `ethereum` endowment (#16932)
...
## [10.23.3]
### Removed
- Remove onramp provider Wyre (#17102)

## [10.23.2]
### Fixed
- Improve performance on signature request screens (#17052)
...
- Fix deadlock encountered while performing Swaps on unsupported testnets (#16511)
...
- Add Warning on a Send transaction request when user doesn't have funds (#16220)
...
## [10.21.1]
### Changed
- Improve rate limiting for metric events (#16308)
...
## [10.19.0]
### Added
- Add ENS wildcard and secure offchain resolution (ENSIP-10 & EIP3668) (#14675)
- Add "What's New" notification about the Merge (#15846)
- Add "What's New" notification about 3box deprecation (#15763)
- Add "What's New" notification for Add Popular Networks feature (#15121)
- Add "What's New" notification for Token Detection feature (#15807)
- Add Warning for Rinkeby, Ropsten and Kovan test networks deprecation (#15725)
- Add Network Info popup when changing selected Network (#13319)
- Add Goerli, BSC, Optimism, Polygon, Avalance, Fantom and Arbitrum networks to query balances using BalanceChecker ([#13846]
- Add Token detection V2 (#15138)
- Add Backup and Restore State functionalities for 3box replacement (#15243)(https://github.com/MetaMask/metamask-extension/pull/13846))
- Add a Warning on the Confirmation screen, when user is performing a SetApprovalForAll transaction (#15512)
- Add new translations for DE, EL, ES, FR, HI, ID, JA, KO, PT, RU, TL TR, VI and ZH (#14975)
- Add Italian translation for "Forgot password" message (#15477)
- Add link to Metalabs dapp into MetaMask home page (#15407)
- Add Sepolia as a default test network (#15787)
- Swaps: on the STX status page (#14995)
  - Add a link to the Swap tx
  - Add a link for "Create a new swap"
- Swaps: add a tx details view for cancelled Swaps on the Activity tab (#15273)
- [FLASK] Add snap update metrics (#15206)
- [FLASK] Add `snap_getBip44Entropy` RPC method and deprecate `snap_getBip44Entropy_*` (#15706)
- [FLASK] Add `snap_getBip32Entropy` JSON-RPC method (#15619)
- [FLASK] Add DApp triggered Snap updates (#15143)

### Changed
- Update Confirmation screen styling by adding a top margin and moving Site Icon next to the site URL (#15136)
- Remove 'ADD_POPULAR_NETWORKS' feature flag as the feature is enabled by default (#15229)
- Remove dark mode and transaction insights from the "What's New" announcements (#15279)
- Update design tokens package and consolidate shadow values with new tokens (#15264)
- Update Zendesk ticket request URLs for a facilitating the bug reporting process (#15458)
- Update Customize Nonce 'Learn More' link pointing to the correct Zendesk article (#15658)
- Update Avalanche RPC endpoint to point Infura, as it is now supported (#15720)
- Update the main text on SetApprovalForAll confirmation screen (#15724)
- Updated origin pill component to match the new design for Permissions and Switching Networks screens (#15603)
- Set "View full transaction details" as default option on SetApprovalForAll Confirmation screen (#15726)
- Update `gridplus-sdk` (#15711)
...
- Update title case to sentence case with some exceptions (names, companies, special terms and page navigation) (#15285)
- Swaps: show a block explorer URL and hide "Add token" prompt, if block explorer URL is not available on Build Quote screen (#15198)
- Swaps: Standardize the spelling of 'cancelled' (#15266)
- Swaps: Performance improvements for Swaps, especially on the Build Quote page (~7.2x faster) (#15359)
- Swaps: only check if a user has enough balance before calling STX. If "Not enough funds" error is thrown, show a notification, but keep STX active (#15218)
- [FLASK] Disable "Mark all as read button" when there are no notifications (#15333)

### Fixed
- Fix Amount set to 0 on Send page when editing an ERC20 tx by preserving its value (#15275)
- Fix limited scroll area in the Account Menu by adjusting the height of Account Menu nav items, expanding scroll area (#15302)
- Fix Actionable Message overlapping tabs on Confirm Transaction view (#15272)
- Fix Contacts breadcrumb when viewing Contact Details (#15663)
- Fix Blockies identicon alignment in General Settings (#15652)
- Fix extra space and period in Custom Token warning text (#15650)
- Fix Signature Request styles: overlapping text when using hardware wallet and icon shrinking with long URLs (#15621)
- Fix for randomly resets of updated values in network edit form (#14370)
- Fix Connected site popup styling, by aligning the rows, shrinking the site icon with long URLs and preventing size change on Disconnect button hover (#15409)
- Fix Blockies icon on Recipient Details, by ensuring it is used when the option is enabled (#15662)
- Fix casing in Import Token for Spanish locale (#15687)
- Fix approval warning condition for `setApprovalForAll` so it is not shown when revoking (#15806)
- Fix disabled Sign button on the Sign Message screen unless scrolled to the bottom (#15788)
- Swaps: fix displaying "No tokens available matching" when there are tokens available, by updating the `results` state if `initialResultsState` array length is different (#15270)
...
- [FLASK] Fix missing snap hook for `getAppKey`, the `getUnlockPromise` hook was missing, breaking some snap examples (#15354)
- [FLASK] Fix snaps authorship pill truncation, previously the authorship label would not be truncated and offset the UI (#15190)
- [FLASK] Fix Snaps key management permission copy (#14849)

## [10.18.4]
### Changed
- Update `eth-lattice-keyring` to v0.10.0 which itself updates `gridplus-sdk` (#15261)
...
- Update `eth-lattice-keyring` to v0.11.0 (#15490). See changes [GridPlus/eth-lattice-keyring@v0.10.0...v0.11.0](https://github.com/GridPlus/eth-lattice-keyring/compare/v0.10.0...v0.11.0)
- Improve ERC721 Send screen by parsing the `tokenId` and refactor `useAssetDetails` hook to avoid unnecessary network calls (#15304)
...
- **[FLASK] BREAKING:** Snaps [are now required](https://github.com/MetaMask/snaps-skunkworks/discussions/590) to export `onRpcRequest` to receive RPC requests (#14952)
  - All existing snaps will have to be updated to conform to this new standard.
- **[FLASK] BREAKING:** Snap state [is now encrypted](https://github.com/MetaMask/snaps-skunkworks/discussions/590) by default (#14952)
...
- **[FLASK]** Improve snap install screen ([#14803](https://github.com/MetaMask/metamask-extension/pull/14803), (#14752))
...
## [10.15.1]
### Fixed
- Fix Ledger connection failures that can occur after remove all hardware wallet accounts and reconnecting  (#14993)
- Fix bug that could cause MetaMask to crash in some cases when interacting with tokens or NFTs (#14962)
...
## [9.5.9]
### Added
- Re-added "Add Ledger Live Support" (#10293), which was reverted in the previous version
...
## [9.5.7]
### Fixed
- Revert "Add Ledger Live Support" (#10293), which introduced a UI crash for some locales
...
## [9.5.3]
### Fixed
- Fixes bug that made MetaMask unusable and displayed 'Minified React error #130' on certain networks and accounts (#11103)
- Prevent big number error when attempting to view transaction list (#11015)

## [9.5.2]
### Fixed
- Fixing address entry error when sending a transaction on a custom network (#11071)

## [9.5.1]
### Fixed
- Fixed icon on approval screen (#11048)
- Fixed broken app state for some users with Chinese, Portuguese or Spanish browser language settings. (#11036)

## [9.5.0] - 2021-04-28
### Added
- Adding popup display to show new MetaMask notifications (#10583)
- Add menu with "View on Etherscan" and "Account details" links to ETH asset page (#10938)
- Add view account details menu item to token page menu (#10932)
- Adding new links to contact MetaMask support (#10895)
...
### Changed
- Use custom token icons in the send flow token dropdown (#10939)
- Remove "My Wallet Account" section in Settings > Contact (#10680)
- Harden contract address validation for token swaps (#10912)
- Show the custom network name in swaps network fee tooltip (#10882)
- Only check whether the swaps feature is live after entering the feature (#10859)
- Update swaps metadata every 5 minutes as opposed to an hour (#10871)
- Increase default slippage from 2% to 3% in swaps and show Advanced Options by default (#10842)
- Prevent tokens without addresses from being added to token list (#10593)
- Add New Zealand Dollar to currency options (#10746)
- Allow 11 characters in symbol for custom RPCs (#10670)
- Hide the suggested token pane when not on Mainnet or test network (#10702)
- Prevents autocomplete text from displaying in the Add Token input (#10700)
- Removing hard references to 12 word seed phrases in copy (#10704)
- Add MetaMask to list of BIP44 HD path examples (#10703)
- Change 'Send ETH' title to 'Send' in the send flow (#10651)
- Don't render faucet row in deposit modal for custom chains (#10674)

### Fixed
- Prevent overflow of hostname on confirmation page (#10935)
...
- Prevent network dropdown label highlighting (#10643)
...
- Ensure MetaMask works correctly when on a custom network that shares a chain id with a default Infura network (#10594)
- Fixed bug that prevented speeding up cancelled transactions (#10579)
- Fixes hidden token bug when zero balance preference is on (#10630)
- Removing double click bug from delete custom network modal (#10628)

## [9.4.0] - 2021-04-15
### Added
- Notify users when MetaMask is unable to connect to the blockchain host (#10883)

## [9.3.0] - 2021-04-02
### Added
- Swaps support for the Binance network (#10721)
- Swaps support for forked Mainnet on localhost (#10658)

### Fixed
- Display BNB token image for default currency on BSC network home screen (#10777)
- Fix: ETH now only appears once in the swaps "to" and "from" dropdowns. (#10650)

## [9.2.1] - 2021-03-26
### Fixed
- Prevent UI crash when a 'wallet_requestPermissions" confirmation is queued behind a "wallet_addEthereumChain" confirmation (#10692)
- Fix infinite spinner when request for token symbol fails while attempting an approve transaction (#10712)

## [9.2.0] - 2021-03-15
### Added
- Add a warning when sending a token to its own contract address (#10546)
- Adding warnings for excessive custom gas input (#10582)
- Add support for multiple Ledger & Trezor hardware accounts (#10505)
- Add setting to hide zero balance tokens (#10486)

### Changed
- Update references to MetaMask support (#10563)
- Update Italian translation (#10126)

### Fixed
- Fix mobile sync of ERC20 tokens (#10591)
- Fix activity title text truncation (#10601)
- Remove 'Ethereum' from custom RPC endpoint warning (#10598)
- Show loading screen while fetching token data for approve screen (#10606)
- Show correct block explorer for custom RPC endpoints for built-in networks (#10587)

## [9.1.1] - 2021-03-03
### Fixed
- Fix ENS resolution related crashes when switching networks on send screen (#10560)
- Fix crash when speeding up an attempt to cancel a transaction on custom networks (#10561)

## [9.1.0] - 2021-02-01
### Uncategorized
- Update Japanese translations. (#10265)
- Update Chinese(Simplified) translations. (#9388)
- Update Vietnamese translations. (#10270)
- Update Spanish and Spanish(Latin American and Caribbean) translations. (#10258)
- Update Russian translations. (#10268)
- Update Tagalog localized messages. (#10269)
- Fix 'imported' translation use case for Dutch. (#10448)
- Use translated transaction category for confirmations. (#10391)
- Cancel unapproved confirmations on network change (#10357)
- Use native currency in asset row. (#10413)
- Fix color indicator size on connected site indicator. (#10421)
- Fix multiple notification window prompts. (#10423)
- Fix icons on token options menu. (#10424)
- Fix token fiat conversion rates when switching from certain custom networks. (#10414)
- Disable BUY button from home screen when not on Ethereum Mainnet. (#10453)
- Fixes gas selection check mark on the notification view. (#10465)
- Fix confirm page header with from/to addresses in fullscreen for tx confirmations. (#10467)
- Hide links to etherscan when no block explorer is specified for a custom network for notifications. (#10455)
- Fix swap insufficient balance error message. (#10456)
- Fix encypt/decrypt tx queueing. (#10350)
- Improve autofocus in the add network form. (#10473)
- Use eth_gasprice for tx gas price estimation on non-Mainnet networks. (#10444)
- Fix accountsChanged event not triggering when manually connecting. (#10477)
- Fix navigation from jumping vertically when clicking into token. (#10471)
...
- Fixes signTypedData message overflow. (#10485)
- Update swaps failure message to include a support link. (#10525)
...
- Fixes ENS IPFS resolution on custom networks with the chainID of 1. (#10507)

## [9.0.5] - 2021-02-09
### Uncategorized
- Allow editing transaction amount after clicking max (#10278)
- Standardize size, shape and color of network color indicators (#10214)
- Use network primary currency instead of always defaulting to ETH in the confirm approve screen (#10298)
...
- Require click of "Continue" button to interact with swap screen if there is a price impact warning for present swap (#10347)
- Change copy of submit button on swaps screen (#10373)
- Swaps token sources/verification messaging update (#10346)
- Stop showing the window.web3 in-app popup if the dapp is just using web3.currentProvider (#10378)
- Throw error when attempting to get an encryption key via eth_getEncryptionPublicKey when connected to Ledger HW (#10326)
- Make action buttons on message components in swaps flow accessible (#10386)

## [9.0.4] - 2021-01-27
### Uncategorized
- Update @metamask/contract-metadata from v1.21.0 to 1.22.0 (#10285)
- Update `hi` localized messages (#10264)
- Move fox to bottom of 'About' page (#10174)
- Fix hardware account selection (#10198)
- Add a timeout to all network requests (#10101)
- Fix displayed balance of tokens with 0 decimals in swaps flow (#10212)
- Prevent accidentally submitting a swap twice (#10162)
- Improve chain ID validation (#10224)
- Increase minimum Firefox version to v68 (#10195)
- Update TrezorConnect to v8 (#10192)
- Fix back button on swaps loading page (#10166)
- Do not publish swaps transaction if the estimateGas call made when adding the transaction fails. (#9947)

## [9.0.3] - 2021-01-22
### Uncategorized
- Fix site metadata handling (#10243)
...
## [9.0.1] - 2021-01-13
### Uncategorized
- Improved detection of contract methods with array parameters (#10169)
- Only warn of injected web3 usage once per page (#10178)
- Restore support for @metamask/inpage provider@"< 8.0.0" (#10179)
- Fix UI crash when domain metadata is missing on public encryption key confirmation page (#10180)

## [9.0.0] - 2021-01-12
### Uncategorized
- Remove window.web3 injection (#9156)
- Add web3 shim usage notification (#10039)
- Implement breaking window.ethereum API changes (#8640)
- Fix `eth_chainId` return values for Infura networks (#8629)
- Increase Chrome minimum version to v63 (#10019)
- Fix error where a swap only completed the token approval transaction (#10135)
- Remove unnecessary swaps footer space when in dropdown mode (#10100)
- Redesign view quote screens (#9905)
- Prevent hidden tokens from reappearing (#9320)
- Use consistent font size for modal top right Close links (#10000)
- Improve home screen notification appearance (#10046)
- Always roll back to the previously selected network when unable to connect to a newly selected network (#10093)
- Fix network settings Kovan block explorer link (#10117)
- Prevent malformed next nonce warning (#10143)
- Update @metamask/contract-metadata from v1.20.0 to 1.21.0 (#10142)
- Fix French "Block Explorer URL" translations (#10160)
- Automatically detect tokens on custom Mainnet RPC endpoints (#10157)
- Improve zh_CN translation (#9772)
- Fix bug where swaps button was disabled on Mainnet if the user hadn't switched networks in a long time (#10170)

## [8.1.11] - 2021-01-07
### Uncategorized
- Disable swaps when the current network's chainId does not match the mainnet chain ID, instead of disabling based on network ID (#10155)

## [8.1.10] - 2021-01-04
### Uncategorized
- Set last provider when switching to a customRPC (#10084)
- Update `@metamask/controllers` to v5.1.0 (#10096)
- Prevent stuck loading screen in some situations (#10103)
- Bump @metamask/contract-metadata from 1.19.0 to 1.20.0 (#10104)
- Fix frozen loading screen on Firefox when strict Enhanced Tracking Protection is enabled (#10110)
...
- Add eth_getProof (#10070)
- Improve swaps maximum gas estimation (#10043)
- Fetch swap quote refresh time from API (#10069)
- Disable console in contentscript to reduce noise (#10040)

## [8.1.8] - 2020-12-09
### Uncategorized
- Improve transaction params validation (#9992)
- Don't allow more than 15% slippage (#9991)
- Prevent unwanted 'no quotes available' message when going back to build quote screen while having insufficient funds (#9994)
- Fix missing contacts upon restart (#9999)
...
## [8.1.6] - 2020-12-04
### Uncategorized
- Fix QR code scans interpretting payment requests as token addresses (#9916)
- Add alt text for images in list items (#9847)
- Ensure watchAsset returns errors for invalid token symbols (#9960)
- Adds tokens from v1.19.0 of metamask/contract-metadata to add token lists (#9968)
- Etherscan links support Goerli network (#9970)
- Show price impact warnings on swaps quote screen (#9899)
- Replace use of ethgasstation (#9867)
- Show correct gas estimates when users don't have sufficient balance for contract transaction (#9984)
- Add 48x48 MetaMask icon for use by browsers (#9993)

## [8.1.5] - 2020-11-19
### Uncategorized
- Show send text upon hover in main asset list (#9871)
- Make edit icon and account name in account details modal focusable (#9855)
- Provide alternative text for images where appropriate (#9853)
- Remove CoinSwitch from the Deposit modal (#9869)
- Move add contact button in fullscreen/expanded view of settings lower to expose the close button. (#9883)
...
- Ensure "Known contract address" warning is shown on send screen even when changing asset (#9907)
- Fix display of Ledger connection error (#9911)
- Fix missing icon in asset page dropdown and in advanced gas modal button group (#9918)

## [8.1.4] - 2020-11-16
### Uncategorized
- Allow speeding up of underpriced transactions (#9687)
...
- Shorten unit input width and use ellipses for overflow (#9778)
- Make the login screen's Restore and Import links accessible (#9746)
- Display decimal chain ID in network form (#9780)
- Use MetaSwap API for gas price estimation in swaps (#9599)
- Make all UI tabs accessible via keyboard (#9518)
- Always allow overwriting invalid custom RPC chain ID (#9808)
- Fix send header cancel button alignment (#9812)
- Do not check popupIsOpen on Vivaldi (#9271)
- Fix UI crash when dapp submits negative gas price (#9306)
- Add sort and search to AddRecipient accounts list (#9257)
- Move `externally_connectable` from base to Chrome manifest (#9824)
- Add support for custom network RPC URL with basic auth (#9815)
- Make QR code button focusable (#9822)
- Warn instead of throw on duplicate web3 (#9832)
- @metamask/controllers@4.0.0 (#9838)
- Prevent user from getting stuck on opt in page (#9856)
- Show a 'send eth' button on home screen in full screen mode (#9845)
- Show send text upon hover in main asset list (#9871)
- Properly detect U2F errors in hardware wallet (#9880)

## [8.1.3] - 2020-10-29
### Uncategorized
- Prevent excessive overflow from swap dropdowns (#9642)
- Fix sorting Quote Source column of quote sort list (#9658)
- Fix adding contact with QR code (#9667)
- Fix ENS resolution of `.eth` URLs with query strings (#9674)
- Bump @metamask/inpage-provider from 6.1.0 to 6.3.0 (#9691)
- Provide image sizing so there's no jump when opening the swaps token search (#9700)
- Add ses lockdown to build system (#9568)
- Prevent memory leak from selected account copy tooltip (#9705)
- Prevent old fetches from polluting the swap state (#9671)
- Keyboard navigation for swaps dropdowns (#9702)
- Switch from Matomo to Segment (#9646)
- Fix fetching swaps when initial network not Mainnet (#9745)
- Include aggregator fee as part of displayed network fees (#9621)
- Bump eth-contract-metadata from 1.16.0 to 1.17.0 (#9736)
- Fix "+-" prefix on swap token amount (#9743)
- Focus on wallet address in buy workflow (#9715)

## [8.1.2] - 2020-10-20
### Uncategorized
- Ensure QR code scanner works (#9608)
- Help users avoid insufficient gas prices in swaps (#9624)
- Update swaps network fee tooltip (#9614)
- Prevent reducing the gas limit for swaps (#9623)
- Fix UI crash when trying to render estimated time remaining of non-submitted transaction (#9630)
- Update View Quote page to better represent the MetaMask fee (#9633)
...
## [8.1.0] - 2020-10-13
### Uncategorized
- Ensure address book entries are shared between networks with the same chain ID (#9565)
- Fix `eth_signTypedData_v4` chain ID validation for non-default networks (#9552)
- Allow the "Localhost 8545" network to be edited, and require a chain ID to be specified for it (#9551)
- Validate custom network chain IDs against endpoint `eth_chainId` return values (#9491)
- Require chain IDs to be specified for custom networks (#9487)
- Add MetaMask Swaps 🌻 (#9482)
- Fix data backup feature (#9422)
- Improve gas input UI by using tooltip instead of a modal to communicate gas data (#9434)
- Improve visual style and layout of the basic tab of the customize gas modal (#9433)
- Fix UI bug in token approval confirmation notifications (#9415)
- Update Wyre purchase URL (#9414)
- Rename 'Ethereum Main Network' in network selector to 'Etherum Mainnet' (#9411)
- Fix info tooltip on the alert settings screen when used in firefox (#9409)
- Fix UI bug in customize gas modal: shwo left border when the first button is selected (#9406)
- Correctly save new Contact Book addressed after editing them in 'Settings > Contact' (#9395)
- Improve Italian translations (#9293)
- Ensure the extension can be unlocked without network/internet access (#9295)
- Add messages to Ledger connection process (#9344)
- Hide seedphrase by default when restoring vault, and provide option for it to be shown (#9329)
- Ensure names of token symbols are shown when token amounts in the token list are long (#9333)
- Warn users when sending tokens to the token address (#9321)
- Fix bug that caused the accounts list to be empty after entering an incorrect password when attempting to export private key (#9288)
- Improve/fix error text for when ENS names are not found, on mainnet (#9314)
- Improve 'Contact Us' copy in settings (#9307)
- Fix capitalization of copy on MetaMetrics opt-in page (#9283)
- Add lock icon to default networks in the Settings network page, to indicate they are not editable (#9269)
- Hide gas price/speed estimate button, and link to advanced gas modal, in send flow on non-main network (#9189)
- Improve visual styling of back button in account modal (#9184)
- Fix vertical align of the network name in network dropdown button (#9152)
- Use new Euclid font throughout MetaMask (#9073)

## [8.0.10] - 2020-09-16
### Uncategorized
- Update default phishing list (#9423)
- Fix fetching a new phishing list on Firefox (#9416)

## [8.0.9] - 2020-08-19
### Uncategorized
- Move transaction confirmation footer buttons to scrollable area (#9228)
- Handle non-String web3 property access (#9256)
- Use @metamask/controllers@2.0.5 (#9266)
- Hide ETH Gas Station estimates on non-main network (#9189)

## [8.0.8] - 2020-08-14
### Uncategorized
- Fix Etherscan redirect on notification click (#9211)
- Reduce volume of web3 usage metrics (#9237)
- Permit all-caps addresses (#9227)

## [8.0.7] - 2020-08-10
### Uncategorized
- Change title of "Reveal Seed Words" page to "Reveal Seed Phrase" (#9065)
- Add tooltip to copy button for contacts and seed phrase (#8974)
- Fix broken UI upon failed password validation (#9063)
- Fix shifted popup notification when browser is in fullscreen on macOS (#9075)
- Support longer text in network dropdown (#9085)
- Fix onboarding bug where user can be asked to verify seed phrase twice (#8873)
- Replace "Email us" button with "Contact us" button (#9104)
- Fix bug where `accountsChanged` events stop after a dapp connection is closed. (#9137)
- Fix network name alignment (#9152)
- Add web3 usage metrics and prepare for web3 removal (#9144)

## [8.0.6] - 2020-07-23
### Uncategorized
- Hide "delete" button when editing contact of wallet account (#9030)
- Fix crash upon removing contact (#9031)
- Do not show spend limit for approvals (#9032)
- Update @metamask/inpage-provider@6.1.0 (#9046)
- Skip attempts to resolve 0x contract prefix (#9048)
- Use content-hash@2.5.2 (#9051)
- Display at least one significant digit of small non-zero token balances (#9056)
...
## [8.0.4] - 2020-07-08
### Uncategorized
- Fix transaction activity on custom networks (#8934)
- Fix account tracker optimization (#8936)

## [8.0.3] - 2020-07-06
### Uncategorized
- Restore missing 'data' provider event, and fix 'notification' event (#8921)
- Normalize the 'from' parameter for `eth_sendTransaction` (#8923)
- Fix handling of multiple `eth_requestAccount` messages from the same domain (#8924)
- Update Italian translations (#8917)
...
## [8.0.1] - 2020-07-02
### Uncategorized
- Fx overflow behaviour of add token list (#8874)
- Show `origin` in connect flow rather than site name (#8885)
- Allow setting a custom nonce of zero (#8883)
...
- Update eth-keyring-controller to fix erasure of imported/hardware account names (#8897)
- Include relative time polyfill locale data (#8896)
- Replace percentage opacity value (#8898)

## [8.0.0] - 2020-07-01
### Uncategorized
- Add permission system (#7004)
- Search accounts by name (#7261)
- Buffer 3 blocks before dropping a transaction (#7483)
- Handle one specific permissions request per tab (#7620)
- Add description to Reset Account in settings (#7686)
- Allow custom IPFS gateway and use more secure default gateway (#7362)
- Adjust colour of Reset Account button to reflect danger (#7696)
- Support new onboarding library (#7602)
- Update custom token symbol length restriction message (#7672)
- Handle 'Enter' keypress on restore from seed screen (#7747)
- Remove padding around advanced gas info icon (#7810)
...
- Sort seed phrase confirmation buttons alphabetically (#7933)
- Add support for 24 word seed phrases (#7987)
- Use contact name instead of address during send flow (#7971)
- Add title attribute to transaction title (#8050)
- Implement encrypt/decrypt feature (#7831)
- Add setting for disabling Eth Phishing Detection (#8125)
...
- Allow selecting multiple accounts during connect flow (#8078)
...
- Add popup explaining connection indicator to existing users (#8293)
- Correctly detect changes to background state (#8435)
- Disable import button for empty string/file (#7912)
- Make seed phrase import case-insensitive (#8246)
- Alert user upon switching to unconnected account (#8312)
- Only updating pending transactions upon block update (#8445)
- Fix firefox popup location (#8467)
- Prevent race condition where transaction value set in UI is overwritten (#8486)
- Fix default gas race condition (#8490)
- Update tokens after importing account (#8491)
- Enable disconnecting a single account or all accounts (#8496)
- Add support for IPFS address resolution (#8502)
...
- Improve Spanish localized message (#8567)
- Add switch to connected account alert (#8532)
- Stop polling for recent blocks on custom networks when UI is closed (#8575)
...
- Hide seed phrase during Account Import (#8730)
...
- Update method data when cached method data is empty (#8836)
- Improve error handling when signature requested without a keyholder address (#8833)
- Stop upper-casing exported private key (#8850)
- Include imported accounts in mobile sync (#8631)

## [7.7.9] - 2020-05-04
### Uncategorized
- Fix popup not opening (#8446)
- Skip adding history entry for empty txMeta diffs (#8449)
- Delete Dai/Sai migration notification (#8447)
- Update deposit copy for Wyre (#8460)
- Snapshot txMeta without cloning history (#8458)
- Fix method registry initialization (#8459)
- Add Dai/Sai to currency display (#8455)
- Prevent network switch upon close of network timeout overlay (#8461)
- Add INR currency option (#8457)
- Fix display of Kovan and Rinkeby chain IDs (#8462)
- Use ethereum-ens-network-map for network support (#8465)
- Update deprecated Etherscam link (#8463)
- Only update pending transactions upon block update (#8474)
- Update eth-contract-metadata (#8476)
- Fix Tohen Typo (#8509)
...
## [7.7.7] - 2020-03-04
### Uncategorized
- Remove invalid Ledger accounts (#8162)
- Fix account index check (#8163)

## [7.7.6] - 2020-03-03
### Uncategorized
- Prevent signing from incorrect Ledger account (#8154)
...
- Use eth-contract-metadata@1.12.1 (#7901)
- Fixing broken JSON import help link (#7910)

## [7.7.2] - 2020-01-13
### Uncategorized
- Fix gas estimate for tokens (#7753)
- Fix transaction order on transaction confirmation screen (#7473)

## [7.7.1] - 2019-12-09
### Uncategorized
- Fix text overlap when expanding transaction (#7488)
- Update gas when asset is changed on send screen (#7491)
...
- Prevent Maker migration dismissal timeout state from being overwritten (#7579)
- Persist Maker migration dismissal timeout (#7581)
- Ensure transactions are shown in the order they are received (#7484)
- Process URL fragment for ens-ipfs redirects (#7604)
- Fix typo that resulted in degrated account menu performance (#7628)
...
- Fixed link on root README.md (#7480)
- Update Wyre ETH purchase url (#7482)
- Ensure transactions are shown in the order they are received (#7484)
- Update gas when token is changed on the send screen (#7491)
- Fix accessibility of first-time-flow terms checkboxes (#7501)
- Fix chainId for non standard networks (#7502)
- Fix timing of DAI migration notifications after dismissal (#7579)
- Fixing hardware connect error display (#7519)
- Use localized messages for NotificationModal buttons (#7558)
- Fix text overlap when expanding transaction (#7488)
...
## [7.6.1] - 2019-11-19
### Uncategorized
- Add 'Remind Me Later' to the Maker notification (#7475)
...
- Fix sourcemaps for Sentry (#7410)
...
## [7.5.1] - 2019-11-13
### Uncategorized
- Fix regression for signed types data screens (#7402)
- Update json-rpc-engine (#7390)
- Reject connection request on window close (#7401)

## [7.5.0] - 2019-11-12
### Uncategorized
- ignore known transactions on first broadcast and continue with normal flow (#7328)
- eth_getTransactionByHash will now check metamask's local history for pending transactions (#7327)
...
- Fix transaction list item status spacing issue (#7319)
- Add hostname and extensionId to site metadata (#7218)
- Fix contact deletion (#7324)
- Fix edit contact details (#7326)
- Update eth-json-rpc-filters to fix memory leak (#7325)
- Add web3 deprecation warning (#7334)
...
- Use translated string for state log (#7255)
- fix issue of xyz ens not resolving (#7266)
...
## [7.2.3] - 2019-10-08
### Uncategorized
- Fix gas limit when sending tx without data to a contract (#7252)
...
## [7.2.1] - 2019-09-17
### Uncategorized
- Add `appName` message to each locale (#7180)

## [7.2.0] - 2019-09-17
### Uncategorized
- Update localization from Transifex Brave (#7099)
- Fix validation of empty block explorer url's in custom network form (#7137)
- Support for eth_signTypedData_v4 (#7128)
- Adds `chaindIdChanged` event to the ethereum provider (#7110)
- Improve browser performance issues caused by missing locale errors (#7091)
- Prevent ineffectual speed ups of pending transactions that don't have the lowest nonce (#7085)
...
- Allow dismissing privacy mode notification from popup (#7093)
...
- Right-to-left CSS (#7072)
- Persian translation (#6878)
- Added missed phrases to RU locale (#7012)
...
## [7.0.1] - 2019-08-08
### Uncategorized
- Ensure seed phrase backup notification only shows up for new users (#6975)

## [7.0.0] - 2019-08-07
### Uncategorized
- Capitalized speed up label to match rest of UI (#6828)
- Allows skipping of seed phrase challenge during onboarding, and completing it at a later time (#6874)
- Prevent opening of asset dropdown if no tokens in account (#6900)
- Set privacy mode as default (#6904)
- Adds Address Book feature (#6914)
- Disable Copy Tx ID and block explorer link for transactions without hash (#6928)
- Fix mobile sync (#6967)

## [6.7.3] - 2019-07-19
### Uncategorized
- Fix bug with resubmitting unsigned transactions. (#6888)

## [6.7.2] - 2019-07-03
### Uncategorized
- Normalize and Validate txParams in TransactionStateManager.addTx too (#6713)
- Update to Node.js v10 (#6759)
- Fixes #6694 (#6694)
- Add tests for ImportWithSeedPhrase#parseSeedPhrase (#6743)
- Fixes #6740 (#6740)
- Fixes #6741 (#6741)
- Fixes #6760, correct PropTypes for nextRoute (#6761)
- Use inline source maps in development (#6754)
- Document hotfix protocol (#6589)
- Add codeowner for package-lock-old.json package-lock.json package.json packagelock-old.json files (#6738)
- Add loading view to notification.html (#6648)
- Add brave as a platform type for MetaMask (#6731)

## [6.7.1] - 2019-07-28
### Uncategorized
- Fix display of token amount on confirm transaction screen (#6764)

## [6.7.0] - 2019-07-26
### Uncategorized
- Improve contract method data fetching (#6623)
- Adds 4byte registry fallback to getMethodData() (#6551)
- Add delete to custom RPC form (#6718)
- Fix styles on 'import account' page, update help link (#6700)
- Wrap smaller custom block explorer url text (#6714)
- Pin ethereumjs-tx (#6706)
- Fix styles on 'import account' page, update help link (#6700)
- Started adding visual documentation of MetaMask plugin components with the account menu component first (#6775)

## [6.6.2] - 2019-07-17
### Uncategorized
- Update dependencies, re-enable npm audit CI job (#6690)
- Fix styles on 'import account' page, update help link (#6700)

## [6.6.1] - 2019-06-06
### Uncategorized
- Revert "Improve ENS Address Input" to fix bugs on input field on non-main networks. (#6691)

## [6.6.0] - 2019-06-04
### Uncategorized
- Enable Ledger hardware wallet support on Firefox (#6659)
...
- Transactions/pending - check nonce against the network and mark as dropped if not included in a block (#6388)
- Improve ENS Address Input (#6606)
- Adds e2e test for removing imported accounts. (#6615)

## [6.5.2] - 2019-05-15
### Uncategorized
- Hardware Wallet Fix (#6613)

## [6.5.1] - 2019-05-14
### Uncategorized
- Fix bug where approve method would show a warning. #6602
- Fix wording of autoLogoutTimeLimitDescription (#6593)

## [6.5.0] - 2019-05-13
### Uncategorized
- feature: integrate gaba/PhishingController (#6568)
- Redesign custom RPC form (#6490)
- Adds auto logout with customizable time frame (#6558)
- Fixes ability to send to token contract addresses (#6578)
- Adds drag and drop functionality to seed phrase entry. (#6557)
- Include token checksum address in prices lookup for token rates (#6526)
- Add subheader to all settings subviews (#6502)
- Improve confirm screen loading performance by fixing home screen rendering bug (#6501)

## [6.4.1] - 2019-04-26
### Uncategorized
- Revert "Adds 4byte registry fallback to getMethodData()" to fix stalling bug. (#6521)

## [6.4.0] - 2019-04-18
### Uncategorized
- Move send to pages/ (#6445)
- update publishing.md with dev diagram (#6470)
- Update to eth-method-registry@1.2.0 (#6403)
- Fix switcher height when Custom RPC is selected or loading (#6468)
- feature: add Goerli support (#6459)
- Fixes #6321 & #6421 - Add Localhost 8545 for network dropdown names (#6444)
- Bump eth-contract-metadata (#6454)
...
- Fix links to MetamaskInpageProvider in porting_to_new_environment.md (#6420)
- Remove broken image walkthrough from metamaskbot comment (#6362)
...
### Uncategorized
- Fix display of gas chart on ethereum networks (#6389)
- Fixes for signing methods for ledger and trezor devices (#6395)
- Fix Wyre link (#6397)

## [6.3.1] - 2019-03-29
### Uncategorized
- Open restore vault in full screen when clicked from popup (#6353)
- Prevents duplicates of account addresses from showing in send screen "To" dropdown (#6372)
- Ensures users are placed on correct confirm screens even when registry service fails (#6374)

## [6.3.0] - 2019-03-26
### Uncategorized
- Gas chart hidden on custom networks (#6300)
...
- Redesign of the settings screen (#6333)
- Cancel transactions and signature requests on the closing of notification windows (#6340)
- Disable transaction "Cancel" button when balance is insufficient (#6341)
- Enable privacy mode by default for first time users (#6347)
...
## [6.2.0] - 2019-03-05
### Uncategorized
- Improves design and UX of onboarding flow (#6192)
- Fixes gas estimation when sending to contracts (#6195)
- Fixes display of notification windows when metamask is active in a tab (#6223)
- Adds MetaMetrics usage analytics system (#6171)
...
## [6.0.1] - 2019-02-12
### Uncategorized
- Fix advanced gas controls on the confirm screen (#6139)
- Trim whitespace from seed phrase during import (#6134)
...
- Add setting for inputting gas price with a text field for advanced users. (#6114)
- Add Swap feature to CurrencyInput (#6091)
- Change gas labels to Slow/Average/Fast (#6090)
- Extract advanced gas input controls to their own component (#6112)
- Update design of phishing warning screen (#5929)
- Add class to sign footer button (#6120)
- Fix locale codes contains underscore never being preferred (#6116)
...
## [5.3.4] - 2019-01-31
### Uncategorized
- fix - migration 30 (#6079)

## [5.3.3] - 2019-01-30
### Uncategorized
- Update privacy notice (#6006)
- Improved Spanish translations (#6072)
- Add visual indicator when displaying a cached balance. (#5854)
- Fix bug that interferred with using multiple custom networks. (#6044)

## [5.3.2] - 2019-01-28
### Uncategorized
- Order shapeshift transactions by time within the transactions list (#6021)
- Add and use cached method signatures to reduce provider requests (#6052)
- Refactor BalanceComponent to jsx (#6048)
- Prevent invalid chainIds when adding custom rpcs (#6026)
- Fix grammar error in Current Conversion (#6029)
- Disable account dropdown on signing screens (#6024)

## [5.3.1] - 2019-01-16
### Uncategorized
- Update Slovenian translation (#5966)
- Set auto conversion off for token/eth conversion (#6005)
- Fix confirm screen for sending ether tx with hex data (#6008)
- Refine app description (#5999)
- Harden Drizzle test runner script (#5997)
- Fix bug where MetaMask user calls non-standard ERC20 methods such as `mint`, `tokenData` will be `undefined` and an uncaught error will break the UI (#5995)
- Fixed a word in french translation (#5970)
- Fix Component#componentDidUpdate usage (#5977)
- Add scrolling button to account list (#5992)
- fix typo in phishing.html title (#5989)

## [5.3.0] - 2019-01-02
### Uncategorized
- Fix etherscan links on notifications (#5978)
- Fix drizzle tests (#5980)
...
- transactions - throw an error if a transaction is generated while the network is loading (#5924)
- Add loading network screen (#5893)

## [5.2.2] - 2018-12-13
### Uncategorized
- Fix speed up button not showing for transactions with the lowest nonce (#5925)
- Update the Phishing Warning notice text to not use inline URLs (#5923)
- Fix some styling and translations in the gas customization modal (#5919)

## [5.2.1] - 2018-12-12
### Uncategorized
- bugfix: Ensures that advanced tab gas limit reflects tx gas limit (#5917)

## [5.2.0] - 2018-12-11
### Uncategorized
- Implements new gas customization features for sending, confirming and speeding up transactions (#5704)
- Groups transactions - speed up, cancel and original - by nonce in the transaction history list (#5886)
- bugfix: eliminates infinite spinner issues caused by switching quickly from a loading network that ultimately fails to resolve (#5892)
- bugfix: provider crashes caused caching issues in `json-rpc-engine`. (#5902)
  - Fixed in (https://github.com/MetaMask/json-rpc-engine/commit/6de511afbd03ccef4550ea43ff4010b7d7a84039)

## [5.1.0] - 2018-12-03
### Uncategorized
- Fixed an infinite spinner bug. (#5860)
- Update phishing warning copy (#5875)
- bugfix: normalize contract addresss when fetching exchange rates (#5863)
- Use selector for state.metamask.accounts in all cases. (#5843)

## [5.0.4] - 2018-11-29
### Uncategorized
- Formats 32-length byte strings passed to personal_sign as hex, rather than UTF8. (#5878)
...
- Soften accusatory language on phishing warning (#5848)
- Open full-screen UI on install (#5835)
...
- transactions - autofill gasPrice for retry attempts with either the recomened gasprice or a %10 bump (#5786)
...
- Show Connect Requests count in extension badge (#5832)
- Increase Token Symbol length to twelve (#5816)
- With the EIP 1102 updates, MetaMask _does_ now open itself when visiting some websites. Changed the wording here to clarify that MetaMask will not open itself to ask you for your seed phrase. (#5819)
- Bump Node version to 8.13 (#5810)
...
- Localize language names in translation select list (#5559)
- Fix bug when eth.getCode() called with no contract (#5283)
...
- 100% coverage in French locale, fixed the procedure to verify proposed locale. (#5470)
- Added rudimentary support for the subscription API to support web3 1.0 and Truffle's Drizzle.
- Update Italian translation. (#5502)
...
## [4.10.0] - 2018-09-18
### Uncategorized
- Implement EIP-712: Sign typed data, but continue to support v1. (#4803)
- Restore multiple consecutive accounts with balances. (#4898)
- New BlockTracker and Json-Rpc-Engine based Provider. (#4279)
...
- Clear old seed words when importing new seed words. (#5229)
- Improve click area for adjustment arrows buttons. (#5264)
- Add new metamask_watchAsset method. (#4606)
- Fix bug where Ropsten loading message is shown when connecting to Kovan. (#5189)
- Add mock EIP-1102 support (#5256)

## [4.9.3] - 2018-08-16
### Uncategorized
- QR code scan for recipient addresses. (#4897)
- Add a download seed phrase link. (#4961)
- Fix bug where gas was not updating properly. (#5060)

## [4.9.2] - 2018-08-10
### Uncategorized
- Fix bug in migration #28 (#5020)

## [4.9.1] - 2018-08-09
### Uncategorized
- Allow to have tokens per account and network. (#4884)
- Continue to use original signedTypedData. (#4989)
- Fix ENS resolution issues. (#5010)
- Show error while allowing confirmation of tx where simulation fails. (#5000)
- Shows retry button on dApp initialized transactions. (#4995)

## [4.9.0] - 2018-08-07
### Uncategorized
- Show retry button on the latest tx of the earliest nonce. (#4926)
- Suggest using the new user interface. (#4888)
- Prevent sending multiple transasctions on multiple confirm clicks. (#4947)
- Add new tokens auto detection. (#4844)
- Remove rejected transactions from transaction history. (#4667)
- Add Trezor Support. (#4625)
...
- Redesign of the Confirm Transaction Screen. (#4691)
- Now shows notifications when transactions are completed. (#4840)
- Allow the use of HTTP prefix for custom rpc urls. (#4855)
- network.js: convert rpc protocol to lower case. (#4855)
- Restore multiple consecutive accounts with balances. (#4898)

## [4.8.0] - 2018-06-18
### Uncategorized
- Attempting to import an empty private key will now show a clear error. (#4513)
- Fix bug where metamask data would stop being written to disk after prolonged use. (#4570)
- Fix bug where account reset did not work with custom RPC providers. (#4523)
- Fix for Brave i18n getAcceptLanguages. (#4524)
- Fix bug where nonce mutex was never released. (#4557)
- Add phishing notice. (#4566)
- Allow Copying Token Addresses and link to Token on Etherscan. (#4591)
...
- Allow other extensions to make access our ethereum provider API (#3997)
