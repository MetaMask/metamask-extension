...
- Prefer integration (and unit) tests over new E2E when an existing integration suite already covers the scenario (e.g. confirmation banners via `dapp-swap-comparison-banner.test.tsx` with `integrationTestRender` and `mockMetaMaskState`). Assert UI copy via `messages.<key>.message`, not hardcoded strings.
...
- Native asset CAIP IDs in fixture `nativeAssetIdentifiers` can differ from the ID the Tokens tab requests. Catalog mocks must prefix-match `eip155:<chainId>/` (see `uiNativeAssetId` and `test/e2e/CONTEXT.md`). `matchingAssets` picks the first `idPrefixes` match, so a native whole-chain prefix can steal ERC-20 lookups — register token-specific entries first or more specifically. For unsupported-cryptocurrency coverage, use the `unsupportedPrice` scenario — quoted price mocks hide that path.
...
- Tokens tab `checkTokenExistsInList` matches `multichain-token-list-item-token-name` (asset `name` from the catalog, often `network.name`). Do not wait on `nativeSymbol` when that string differs.
