...
- When reporting potential bugs, user provides detailed code-path analysis and expects the agent to verify the issue exists by reading code before applying any fix
- User creates implementation plans in `.cursor/plans/` and expects agents to execute them without modifying the plan file itself
- User prefers defense-in-depth: always add frontend safety guards (e.g., HW wallet checks) even when request-time or backend gating should prevent the issue
- MetaMask mobile PRs (metamask-mobile repo) are used as canonical reference implementations when the extension needs to match behavior

## Learned Workspace Facts

- Bridge gas sponsorship has three mechanisms: `gasIncluded` (STX path, works for HW wallets), `gasIncluded7702` (EIP-7702 relay, does NOT work for HW wallets), and `gasSponsored` (backend flag gated by request params)
- `useGasIncluded7702` serves as the request-time HW wallet gate; it returns `false` for hardware wallets, so the backend won't return `gasSponsored=true` for HW accounts
- `shouldShowGasSponsored` in `multichain-bridge-quote-card.tsx` has two independent code paths: the quote-level `gasSponsored` flag and the network-level `insufficientBal + isCurrentNetworkGasSponsored` workaround — both must be gated for HW wallets
- `gasIncluded` (STX path) should show the "gas fees included" UI for HW wallets in the quote card; only `gasSponsored`/`gasIncluded7702` UI display paths are gated by `isHardwareWalletAccount`
- Monad (`CHAIN_IDS.MONAD`) requires a minimum 10 MON native reserve for 7702-sponsored transactions; the reserve check in `_getBaseValidationErrors` only applies when `quoteRequest.gasIncluded7702` is true
