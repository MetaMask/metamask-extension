...
- Verify the issue exists before fixing; provide evidence (reproduce CI failure locally or trace logic), then fix, then verify again with tests/lint
- Execute attached plans end-to-end without editing the plan file; drive existing todos (in_progress → done) in order
- Do not commit, stage, or push unless explicitly asked; when asked, use Conventional Commits (`fix(scope): …`)
- Keep explanations concise; prefer short summaries with file/line references over lengthy narratives
- When addressing PR reviewer comments, tackle all comments in one pass and verify each fix
- Match existing code patterns (error handling style, feature-flag plumbing, eslint-disable comments) rather than inventing new conventions
- Files with JSX must use `.tsx` extension (not `.ts`); new test files must be TypeScript (`.test.ts`/`.test.tsx`), never `.test.js`
- `.cursor/hooks/state/` is local session metadata — always gitignore it and never commit it
- `.worktrees/` (local git worktrees directory) should be gitignored
- Continue working after interrupted assistant output without restarting the task
- For continual-learning updates: update AGENTS.md entries in place, cap 12 bullets per section, plain bullets only, no metadata annotations
- When the user provides a GitHub PR URL with "address comments", read and address all reviewer comments systematically

## Learned Workspace Facts

- Shell environment in this workspace has recurring `_encode:25` / zsh encoding issues; use `env -i HOME=$HOME PATH=$PATH /bin/bash -c '...'` or `env -i` wrappers for reliable `yarn`/`jest`/`gh` invocations
- Node version per `.nvmrc` may be v24.13+; use `nvm use` before builds
- CI "unit tests" can exit 1 with all assertions passing due to **Console Baseline Reporter** violations; fix by updating `test/jest/console-baseline-unit.json`
- CI "fitness functions" (`preventJavaScriptFileAdditions`) fails when new `.js`/`.jsx` files are added under `app/`, `shared/`, or `ui/`
- Restricted import paths: UI tests cannot import directly from `app/_locales/...`; use `test/lib/i18n-helpers` instead
- Bridge/gas sponsorship involves multiple layers: quote-level flags (`gasSponsored`, `gasIncluded`, `gasIncluded7702`), network-level remote flags (`gasFeesSponsoredNetwork`), and `insufficientBal` workaround paths — each may need independent hardware-wallet gating
- `getBridgeQuotes` selector strips `gasIncluded7702`/`gasSponsored` for HW wallets but leaves `gasIncluded` (STX path); UI code with non-quote-based sponsorship paths still needs explicit HW checks
- `redux-mock-store` thunk `mock.calls` contain `[dispatch, getState]`, not action arguments; spy on the action creator separately to assert device/args
- For `ControllerFlatState` mocks in tests, prefer minimal shape + `as unknown as ReturnType<typeof getFlatState>` rather than hand-building every property
- When a keyring type is unknown, default `accountSupports7702` to `false` (safe default) rather than assuming support
- `yarn test:unit` invocation: `node --expose-gc ./node_modules/jest/bin/jest.js` (not plain `yarn test:unit` which may hit shell encoding issues)
- Prettier requires trailing commas on multiline function signatures; long signatures must be broken across lines
