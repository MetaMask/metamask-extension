...
- `unifiedEvmAccountsApiBalances.mainnetAdditionalBalances` only seeds ERC-20 balances for chain 1 (mainnet). For non-mainnet ERC-20 balances, pass `unifiedEvmAccountsApiBalances.additionalBalances` (chain-matched by CAIP-2 prefix) so Accounts API v5 refresh cannot wipe fixture-seeded `AssetsController` amounts. Accounts API v5 `balance` strings are human amounts, not raw token units.
