...
const { STATE_PROPERTIES_TO_CONTROLLER_MAP } = require('./shared/constants/metamask');
            ...
            metamaskProperty.initializer.properties.forEach((prop) => {
              if (
                ...
                const controllerName =
                  STATE_PROPERTIES_TO_CONTROLLER_MAP[prop.name.text as keyof typeof STATE_PROPERTIES_TO_CONTROLLER_MAP];
                if (controllerName) {
                  ...
                  if (existingController && ts.isPropertyAssignment(existingController)) {
                    if (ts.isObjectLiteralExpression(existingController.initializer)) {
                    ...
                    newProperties.push(
                      ts.factory.createPropertyAssignment(
                        ...
                        ts.factory.createObjectLiteralExpression([prop], true)
                      )
                    );
                  }
                } else {
