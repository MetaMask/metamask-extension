...
function extractComponentName(content) {
  ...
  const importMatches = content.match(
    /import\s+([A-Za-z][A-Za-z0-9]*)\s+from/g,
  );
...
function generateTitleFromPath(filePath, content) {
  ...
  let finalTitle;
  if (
    lastDirOriginal === filename ||
    lastDirPart.toLowerCase() === componentName.toLowerCase()
  ) {
...
function updateStoryTitle(filePath) {
  try {
    ...
    const exportDefaultRegex =
      /(export\s+default\s*\{[^}]*?title:\s*['"`])([^'"`]+)(['"`])/s;

    // Pattern 2: const meta: Meta<...> = { title: '...' }
    const metaRegex =
      /(const\s+meta[^=]*=\s*\{[^}]*?title:\s*['"`])([^'"`]+)(['"`])/s;

    // Pattern 3: const SomeStory = { title: '...' }
    const constStoryRegex =
      /(const\s+[A-Za-z][A-Za-z0-9]*\s*=\s*\{[^}]*?title:\s*['"`])([^'"`]+)(['"`])/s;
    ...
    if (!titleFound) {
      console.log(
        `⚠️  Could not find title property in main story config: ${filePath}`,
      );
      return false;
    }
...
function main() {
  ...
  if (args.length > 0) {
    ...
    storybookFiles = args.filter(
      (file) => file.match(/\.stories\.(js|tsx)$/) && fs.existsSync(file),
    );
