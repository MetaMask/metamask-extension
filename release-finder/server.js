...
router.post('/find-commits', async (ctx) => {
  ...
  for (const commitSHA of commitSHAList) {
    ...
    if (releaseTags.length > 1) {
      ...
      commitDetails.earliestReleaseTag = await findReleaseCandidateTag(commitSHA);
    } else {
...
async function findReleaseCandidateTag(commitSHA) {
  ...
  const tagList = execSync(`git tag --list`).toString().split('\n').filter(tag => {
    if (tag && /^v(\d+)\.(\d+)\.(\d+)$/.exec(tag)) {
      return true
    }
    return false;
  })
  ...
  const sortedTagList = [...tagList].sort((aTag, bTag) => {
    const [, aMajor, aMinor, aPatch] = /^v(\d+)\.(\d+)\.(\d+)$/.exec(aTag);
    const [, bMajor, bMinor, bPatch] = /^v(\d+)\.(\d+)\.(\d+)$/.exec(bTag);
  ...
  const [, major, minor, patch] = /^v(\d+)\.(\d+)\.(\d+)$/.exec(highestVersionedTag);
  ...
  const branchesContainingCommit = execSync(`git branch -r --contains ${commitSHA}`).toString().trim();

  for (const branchName of potentialBranches) {
    const branchExistsAndCommitOnBranch = branchesContainingCommit.includes(`origin/Version-${branchName}`)
  ...
  const nextMinorVersionExists = remoteBranches.includes(`origin/Version-${`v${major}.${nextMinorVersion}.0`}`)
  const possibleNextNextMinorVersion = nextMinorVersionExists ? getNextVersion(nextMinorVersion, 'MINOR') : nextMinorVersion
