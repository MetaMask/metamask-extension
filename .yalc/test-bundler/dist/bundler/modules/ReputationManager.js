...
class ReputationManager {
    ...
    checkStake(title, info) {
        ...
        (0, utils_1.requireCond)(this.getStatus(info.addr) !== ReputationStatus.BANNED, `${title} ${info.addr} is banned`, utils_1.ValidationErrors.Reputation, { [title]: info.addr });
        (0, utils_1.requireCond)(ethers_1.BigNumber.from(info.stake).gte(this.minStake), `${title} ${info.addr} ${(0, utils_1.tostr)(info.stake) === '0' ? 'is unstaked' : `stake ${(0, utils_1.tostr)(info.stake)} is too low (min=${(0, utils_1.tostr)(this.minStake)})`}`, utils_1.ValidationErrors.InsufficientStake);
