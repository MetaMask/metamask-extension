...
class BundleManager {
    ...
    async sendBundle(userOps, beneficiary, storageMap) {
        ...
        try {
            ...
            if (reasonStr.startsWith('AA3')) {
                this.reputationManager.crashedHandleOps((0, src_1.getAddr)(userOp.paymasterAndData));
            }
            else if (reasonStr.startsWith('AA2')) {
                this.reputationManager.crashedHandleOps(userOp.sender);
            }
            else if (reasonStr.startsWith('AA1')) {
                this.reputationManager.crashedHandleOps((0, src_1.getAddr)(userOp.initCode));
            }
            else {
    ...
    async createBundle() {
        ...
        mainLoop: for (const entry of entries) {
            const paymaster = (0, src_1.getAddr)(entry.userOp.paymasterAndData);
            const factory = (0, src_1.getAddr)(entry.userOp.initCode);
            ...
            (0, src_1.mergeStorageMap)(storageMap, validationResult.storageMap);
    ...
    async getUserOpHashes(userOps) {
        const { userOpHashes } = await (0, src_1.runContractScript)(this.entryPoint.provider, new types_1.GetUserOpHashes__factory(), [this.entryPoint.address, userOps]);
        return userOpHashes;
    }
}
