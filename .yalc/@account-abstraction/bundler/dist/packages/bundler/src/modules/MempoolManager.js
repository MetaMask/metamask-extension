...
class MempoolManager {
    ...
    addUserOp(userOp, userOpHash, prefund, referencedContracts, senderInfo, paymasterInfo, factoryInfo, aggregatorInfo) {
        ...
        if (index !== -1) {
            ...
            const paymaster = (0, src_1.getAddr)(userOp.paymasterAndData);
            if (paymaster != null) {
                this.incrementEntryCount(paymaster);
            }
            const factory = (0, src_1.getAddr)(userOp.initCode);
            if (factory != null) {
    ...
    updateSeenStatus(aggregator, userOp, senderInfo) {
        try {
            ...
            if (!(e instanceof src_1.RpcError))
                throw e;
        }
        this.reputationManager.updateSeenStatus(aggregator);
        this.reputationManager.updateSeenStatus((0, src_1.getAddr)(userOp.paymasterAndData));
        this.reputationManager.updateSeenStatus((0, src_1.getAddr)(userOp.initCode));
    }
    ...
    checkMultipleRolesViolation(userOp) {
        ...
        (0, src_1.requireCond)(!knownEntities.includes(userOp.sender.toLowerCase()), `The sender address "${userOp.sender}" is used as a different entity in another UserOperation currently in mempool`, src_1.ValidationErrors.OpcodeValidation);
        const knownSenders = this.getKnownSenders();
        const paymaster = (_a = (0, src_1.getAddr)(userOp.paymasterAndData)) === null || _a === void 0 ? void 0 : _a.toLowerCase();
        const factory = (_b = (0, src_1.getAddr)(userOp.initCode)) === null || _b === void 0 ? void 0 : _b.toLowerCase();
        ...
        (0, src_1.requireCond)(!isPaymasterSenderViolation, `A Paymaster at ${paymaster} in this UserOperation is used as a sender entity in another UserOperation currently in mempool.`, src_1.ValidationErrors.OpcodeValidation);
        (0, src_1.requireCond)(!isFactorySenderViolation, `A Factory at ${factory} in this UserOperation is used as a sender entity in another UserOperation currently in mempool.`, src_1.ValidationErrors.OpcodeValidation);
    }
    ...
    checkReplaceUserOp(oldEntry, entry) {
        ...
        (0, src_1.requireCond)(newMaxPriorityFeePerGas >= oldMaxPriorityFeePerGas * 1.1, `Replacement UserOperation must have higher maxPriorityFeePerGas (old=${oldMaxPriorityFeePerGas} new=${newMaxPriorityFeePerGas}) `, src_1.ValidationErrors.InvalidFields);
        (0, src_1.requireCond)(newMaxFeePerGas >= oldMaxFeePerGas * 1.1, `Replacement UserOperation must have higher maxFeePerGas (old=${oldMaxFeePerGas} new=${newMaxFeePerGas}) `, src_1.ValidationErrors.InvalidFields);
    }
    ...
    removeUserOp(userOpOrHash) {
        ...
        if (index !== -1) {
            ...
            this.decrementEntryCount((0, src_1.getAddr)(userOp.paymasterAndData));
            this.decrementEntryCount((0, src_1.getAddr)(userOp.initCode));
    ...
    getKnownEntities() {
        ...
        res.push(...userOps.map(it => {
            return (0, src_1.getAddr)(it.userOp.paymasterAndData);
        }));
        res.push(...userOps.map(it => {
            return (0, src_1.getAddr)(it.userOp.initCode);
        }));
