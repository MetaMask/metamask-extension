...
class ReputationManager {
    ...
    async getStakeStatus(address, entryPointAddress) {
        const sm = types_1.IStakeManager__factory.connect(entryPointAddress, this.provider);
    ...
    checkBanned(title, info) {
        (0, src_1.requireCond)(this.getStatus(info.addr) !== ReputationStatus.BANNED, `${title} ${info.addr} is banned`, src_1.ValidationErrors.Reputation, { [title]: info.addr });
    }
    /**
    ...
    checkThrottled(title, info) {
        (0, src_1.requireCond)(this.getStatus(info.addr) !== ReputationStatus.THROTTLED, `${title} ${info.addr} is throttled`, src_1.ValidationErrors.Reputation, { [title]: info.addr });
    }
    /**
    ...
    checkStake(title, info) {
        ...
        (0, src_1.requireCond)(this.getStatus(info.addr) !== ReputationStatus.BANNED, `${title} ${info.addr} is banned`, src_1.ValidationErrors.Reputation, { [title]: info.addr });
        (0, src_1.requireCond)(ethers_1.BigNumber.from(info.stake).gte(this.minStake), `${title} ${info.addr} ${(0, src_1.tostr)(info.stake) === '0' ? 'is unstaked' : `stake ${(0, src_1.tostr)(info.stake)} is too low (min=${(0, src_1.tostr)(this.minStake)})`}`, src_1.ValidationErrors.InsufficientStake);
        (0, src_1.requireCond)(ethers_1.BigNumber.from(info.unstakeDelaySec).gte(this.minUnstakeDelay), `${title} ${info.addr} unstake delay ${(0, src_1.tostr)(info.unstakeDelaySec)} is too low (min=${(0, src_1.tostr)(this.minUnstakeDelay)})`, src_1.ValidationErrors.InsufficientStake);
    }
    /**
