...
class BundlerServer {
    ...
    async _preflightCheck() {
        ...
        const emptyUserOp = {
            sender: src_1.AddressZero,
            callData: '0x',
            initCode: src_1.AddressZero,
            paymasterAndData: '0x',
            nonce: 0,
    ...
    intro(req, res) {
        res.send(`Account-Abstraction Bundler v.${src_1.erc4337RuntimeVersion}. please use "/rpc"`);
    }
    async rpc(req, res) {
        let resContent;
    ...
    async handleRpc(reqItem) {
        ...
        try {
            const result = (0, src_1.deepHexlify)(await this.handleMethod(method, params));
    ...
    async handleMethod(method, params) {
        ...
        switch (method) {
            ...
            case 'debug_bundler_getStakeStatus':
                ...
                break;
            default:
                throw new src_1.RpcError(`Method ${method} is not supported`, -32601);
        }
        return result;
    }
}
