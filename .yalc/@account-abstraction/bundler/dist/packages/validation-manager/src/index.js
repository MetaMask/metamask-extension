...
async function supportsDebugTraceCall(provider) {
    ...
    const ret = await (0, GethTracer_1.debug_traceCall)(provider, { from: src_1.AddressZero, to: src_1.AddressZero, data: '0x' }, { tracer: BundlerCollectorTracer_1.bundlerCollectorTracer }).catch(e => e);
...
async function checkRulesViolations(provider, userOperation, entryPointAddress) {
    ...
    const entryPoint = src_1.IEntryPoint__factory.connect(entryPointAddress, provider);
