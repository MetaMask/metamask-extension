...
class ValidationManager {
    ...
    _parseErrorResult(userOp, errorResult) {
        ...
        if (!((_a = errorResult === null || errorResult === void 0 ? void 0 : errorResult.errorName) === null || _a === void 0 ? void 0 : _a.startsWith('ValidationResult'))) {
            ...
            if (paymaster === src_1.AddressZero) {
                paymaster = undefined;
            }
            // eslint-disable-next-line
            const msg = (_c = (_b = errorResult.errorArgs) === null || _b === void 0 ? void 0 : _b.reason) !== null && _c !== void 0 ? _c : errorResult.toString();
            if (paymaster == null) {
                throw new src_1.RpcError(`account validation failed: ${msg}`, src_1.ValidationErrors.SimulateValidation);
            }
            else {
                throw new src_1.RpcError(`paymaster validation failed: ${msg}`, src_1.ValidationErrors.SimulatePaymasterValidation, { paymaster });
            }
        }
        ...
        function fillEntity(data, info) {
            const addr = (0, src_1.getAddr)(data);
            return addr == null
                ? undefined
    ...
    async _geth_traceCall_SimulateValidation(userOp) {
        ...
        try {
            ...
            debug('==dump tree=', JSON.stringify(tracerResult, null, 2)
                ...
                .replace(new RegExp((_a = (0, src_1.getAddr)(userOp.paymasterAndData)) !== null && _a !== void 0 ? _a : '--no-paymaster--'), '{paymaster}')
                .replace(new RegExp((_b = (0, src_1.getAddr)(userOp.initCode)) !== null && _b !== void 0 ? _b : '--no-initcode--'), '{factory}'));
            ...
            const err = (0, src_1.decodeErrorReason)(data);
            throw new src_1.RpcError(err != null ? err.message : data, 111);
        }
    }
    /**
    ...
    async validateUserOp(userOp, previousCodeHashes, checkStakes = true) {
        if (previousCodeHashes != null && previousCodeHashes.addresses.length > 0) {
            ...
            // [COD-010]
            (0, src_1.requireCond)(codeHashes === previousCodeHashes.hash, 'modified code after first validation', src_1.ValidationErrors.OpcodeValidation);
        }
        let res;
        let codeHashes = {
            addresses: [],
        ...
        (0, src_1.requireCond)(!res.returnInfo.sigFailed, 'Invalid UserOp signature or paymaster signature', src_1.ValidationErrors.InvalidSignature);
        const now = Math.floor(Date.now() / 1000);
        (0, src_1.requireCond)(res.returnInfo.validAfter <= now, 'time-range in the future time', src_1.ValidationErrors.NotInTimeRange);
        console.log('until', res.returnInfo.validUntil, 'now=', now);
        (0, src_1.requireCond)(res.returnInfo.validUntil == null || res.returnInfo.validUntil >= now, 'already expired', src_1.ValidationErrors.NotInTimeRange);
        (0, src_1.requireCond)(res.returnInfo.validUntil == null || res.returnInfo.validUntil > now + VALID_UNTIL_FUTURE_SECONDS, 'expires too soon', src_1.ValidationErrors.NotInTimeRange);
        (0, src_1.requireCond)(res.aggregatorInfo == null, 'Currently not supporting aggregator', src_1.ValidationErrors.UnsupportedSignatureAggregator);
        return Object.assign(Object.assign({}, res), { referencedContracts: codeHashes, storageMap });
    }
    async getCodeHashes(addresses) {
        const { hash } = await (0, src_1.runContractScript)(this.entryPoint.provider, new src_1.CodeHashGetter__factory(), [addresses]);
        return {
            hash,
            addresses
        };
    ...
    validateInputParameters(userOp, entryPointInput, requireSignature = true, requireGasParams = true) {
        (0, src_1.requireCond)(entryPointInput != null, 'No entryPoint param', src_1.ValidationErrors.InvalidFields);
        (0, src_1.requireCond)(entryPointInput.toLowerCase() === this.entryPoint.address.toLowerCase(), `The EntryPoint at "${entryPointInput}" is not supported. This bundler uses ${this.entryPoint.address}`, src_1.ValidationErrors.InvalidFields);
        // minimal sanity check: userOp exists, and all members are hex
        (0, src_1.requireCond)(userOp != null, 'No UserOperation param', src_1.ValidationErrors.InvalidFields);
        ...
        fields.forEach(key => {
            ...
            (0, src_1.requireCond)(value != null, 'Missing userOp field: ' + key + ' ' + JSON.stringify(userOp), src_1.ValidationErrors.InvalidFields);
            (0, src_1.requireCond)(value.match(HEX_REGEX) != null, `Invalid hex value for property ${key}:${value} in UserOp`, src_1.ValidationErrors.InvalidFields);
        });
        (0, src_1.requireCond)(userOp.paymasterAndData.length === 2 || userOp.paymasterAndData.length >= 42, 'paymasterAndData: must contain at least an address', src_1.ValidationErrors.InvalidFields);
        ...
        (0, src_1.requireCond)(userOp.initCode.length === 2 || userOp.initCode.length >= 42, 'initCode: must contain at least an address', src_1.ValidationErrors.InvalidFields);
        const calcPreVerificationGas1 = (0, sdk_1.calcPreVerificationGas)(userOp);
        (0, src_1.requireCond)(userOp.preVerificationGas >= calcPreVerificationGas1, `preVerificationGas too low: expected at least ${calcPreVerificationGas1}`, src_1.ValidationErrors.InvalidFields);
    }
}
