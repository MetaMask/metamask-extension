 ...
 * http(s).
 */
function hasOpaqueOrigin(sender?: OriginBearingSender): boolean {
...
/**
 * Whether a connection must be refused because the browser withheld the
 * sender's security origin.
 *
 ...
 * principal from `sender.url` would hand such a frame the URL origin's wallet
 * session — which is exactly the isolation the sandbox was asked for. Refuse
 * the connection rather than authorize it as the URL origin.
 *
 * Website subjects only. Extension senders keep the existing derivation; that
 * is a separate case with its own assessment.
 *
 * @param subjectType - The resolved subject type for this connection.
 * @param sender - The message sender to inspect.
 * @returns True when the connection should be refused.
 */
export function isOpaqueWebsiteSender(
  subjectType: SubjectType | undefined,
  sender?: OriginBearingSender,
): boolean {
  return subjectType === SubjectType.Website && hasOpaqueOrigin(sender);
}
