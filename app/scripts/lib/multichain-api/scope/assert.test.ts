  ...
  describe('assertScopeSupported', () => {
    ...
    describe('scopeString', () => {
      it('checks if the scopeString is supported', () => {
        ...
        expect(MockSupported.isSupportedScopeString).toHaveBeenCalledWith('scopeString', findNetworkClientIdByChainId)
      })
      ...
      it('throws an error if the scopeString is not supported', () => {
        ...
        }).toThrow(new EthereumRpcError(5100, 'Requested chains are not supported'))
      })
    })
    ...
    describe('scopeObject', () => {
      ...
      it('throws an error if there are methods missing from the OpenRPC Document', () => {
        expect(() => {
          assertScopeSupported('scopeString', {
            ...validScopeObject,
        ...
        }).toThrow(new EthereumRpcError(5101, 'Requested methods are not supported'))
      })
      ...
      it('checks if the notifications are supported', () => {
        ...
        expect(MockSupported.isSupportedNotification).toHaveBeenCalledWith('chainChanged')
      })
      ...
      it('throws an error if there are unsupported notifications', () => {
        ...
        }).toThrow(new EthereumRpcError(5102, 'Requested notifications are not supported'))
      })
      ...
      it('checks if the accounts are supported', () => {
        ...
        expect(MockSupported.isSupportedAccount).toHaveBeenCalledWith('eip155:1:0xdeadbeef', getInternalAccounts)
      })
      ...
      it('throws an error if there are unsupported accounts', () => {
        ...
        }).toThrow(new EthereumRpcError(5103, 'Requested accounts are not supported'))
      })
