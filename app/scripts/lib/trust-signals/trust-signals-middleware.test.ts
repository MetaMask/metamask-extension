...
describe('createTrustSignalsMiddleware', () => {
  ...
  describe('wallet_sendCalls', () => {
    ...
    it('scans all to addresses in the batch', async () => {
      ...
      const calls = [
        ...
        { to: TEST_ADDRESSES.SPENDER, data: '0x', value: '0x0' },
      ];

      const req = createMockRequest(
        MESSAGE_TYPE.WALLET_SEND_CALLS,
        createSendCallsParams(calls),
      );
    ...
    it('skips calls without a to address', async () => {
      ...
      const req = createMockRequest(
        MESSAGE_TYPE.WALLET_SEND_CALLS,
        createSendCallsParams(calls as { to?: string }[]),
      );
    ...
    it('does not scan if params are invalid', async () => {
      const { middleware } = createMiddleware();
    ...
    it('does not scan if chain is unsupported', async () => {
      const { middleware } = createMiddleware({ chainId: '0xdeadbeef' as Hex });
      scanAddressMockAndAddToCache.mockResolvedValue(
        MOCK_SCAN_RESPONSES.BENIGN,
      );

      const calls = [{ to: TEST_ADDRESSES.TO, data: '0x', value: '0x0' }];
    ...
    it('scans the origin URL', async () => {
      ...
      phishingController.scanUrl.mockResolvedValue(undefined);
    ...
    it('logs error and continues when scan fails', async () => {
      const { middleware } = createMiddleware();
