...
export async function readCriticalErrorRepairSession(
  browserApi: typeof browser,
): Promise<RestoringTabHandoff | null> {
  try {
    // storage.local survives runtime.reload(); storage.session does not
    const data = await browserApi.storage.local.get(CRITICAL_ERROR_REPAIR_KEY);
    const session = data[CRITICAL_ERROR_REPAIR_KEY];

    if (!session || typeof session !== 'object') {
      return null;
    }

    const { tabUrl, tabId, repairAction } = session as Record<string, unknown>;
    ...
    return {
      ...
      tabUrl,
      repairAction: Object.values(CriticalErrorRepairAction).includes(
        repairAction as CriticalErrorRepairAction,
      )
        ? (repairAction as CriticalErrorRepairAction)
    ...
    // even when the startup is a default init (not a critical-error repair).
...
/**
 * Removes the repair session key. Best-effort: if `storage.local.remove` fails,
 ...
 */
export async function clearCriticalErrorRepairSession(
  browserApi: typeof browser,
): Promise<void> {
  try {
    await browserApi.storage.local.remove(CRITICAL_ERROR_REPAIR_KEY);
  } catch (error) {
    // Do not rethrow: throwing would block the critical-error repair path and could
...
export async function openRestoringTabAndReload(
  ...
  repairAction: CriticalErrorRepairAction = CriticalErrorRepairAction.Recover,
): Promise<void> {
  ...
  await browser.storage.local.set({ [CRITICAL_ERROR_REPAIR_KEY]: value });
 ...
 * possible, opens the extension UI in a new tab so repair can still continue.
 * @param platform
 * @param handoff
 */
