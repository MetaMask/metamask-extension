 ...
 * string. It is important for the background process to never handle the
...
export function convertMnemonicToWordlistIndices(
  ...
  for (let i = 0; i < mnemonic.length; i++) {
    if (mnemonic[i] === 0x20) {
      // Space character
      if (i > start) {
        const index = getIndexFromTrie(mnemonic, start, i);
        indices.push(index);
      }
      start = i + 1;
    }
  ...
  if (start < mnemonic.length) {
    const index = getIndexFromTrie(mnemonic, start, mnemonic.length);
    indices.push(index);
  }
...
export function convertEnglishWordlistIndicesToCodepoints(
  ...
  const output = new Array(totalSize);
  let offset = 0;
