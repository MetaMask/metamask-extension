...
function createMockMessenger() {
  ...
  return {
    ...
    unsubscribe: jest.fn((event: string, handler: (...args: unknown[]) => void) => {
...
function createMockController(
  ...
  metadata?: Record<
    string,
    { persist?: boolean; includeInDebugSnapshot?: boolean; includeInStateLogs?: boolean; usedInUi?: boolean }
  >,
) {
  const fullMetadata: Record<string, {
    persist: boolean;
  ...
  for (const key of Object.keys(state)) {
    fullMetadata[key] = {
      ...
      includeInDebugSnapshot: metadata?.[key]?.includeInDebugSnapshot ?? true,
...
describe('ControllerRegistry', () => {
  const makeRegistry = (
    ...
    const registry = new ControllerRegistry(messenger as any, uiConfig, persistConfig);
  ...
  describe('subscribeAll', () => {
    ...
    it('returns unsubscribe functions that remove handlers', () => {
      ...
      expect(messenger.listenerCount('TokensController:stateChange')).toBe(1);

      unsubs.forEach((fn) => fn());

      expect(messenger.unsubscribe).toHaveBeenCalledTimes(2);
      expect(messenger.listenerCount('TokensController:stateChange')).toBe(0);
      expect(messenger.listenerCount('NetworkController:stateChange')).toBe(0);
    });
  });
