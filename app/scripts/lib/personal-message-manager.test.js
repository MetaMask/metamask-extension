...
describe('Personal Message Manager', () => {
  ...
  describe('#addUnapprovedMessage', () => {
    const selectedAddress = '0xFb2C15004343904e5f4082578c4e8e11105cF7e3';
    ...
    it('should detect SIWE messages', () => {
      ...
      const msgId = messageManager.addUnapprovedMessage(msgParams, request);
      const {
        msgParams: { siwe },
      } = messageManager.getMsg(msgId);
      expect(siwe.isSIWEMessage).toStrictEqual(true);
    });

    it("should throw an error if the SIWE message's domain doesn't match", () => {
      const request = { origin: 'https://mismatched-domain.com' };
      const expectedError = `SIWE domain is not valid: "${request.origin}" !== "${origin}"`;
      expect(() => {
