...
/**
 * Buffers Immer patches keyed by controller name.
 *
 ...
 *
 * This class only buffers — scheduling and transport are the caller's
 * responsibility.
 */
export class PatchBuffer {
  ...
  /**
   * Drain all buffered patches and return them as a keyed record.
   * Returns `null` if nothing has been buffered since the last flush.
   *
  ...
  /**
   * Whether the buffer has pending patches.
   */
  ...
  /**
   * Discard all buffered patches without returning them.
   */
  destroy(): void {
    this.#pending.clear();
  }
