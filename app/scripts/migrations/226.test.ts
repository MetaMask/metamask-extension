...
describe(`migration #${VERSION}`, () => {
  ...
  it('deletes TokenListController state', async () => {
    const oldStorage: VersionedData = {
      meta: { version: PREVIOUS_VERSION },
      data: {
        TokenListController: {
    ...
    expect(versionedData.data).not.toHaveProperty('TokenListController');
  ...
  it('does nothing if TokenListController state is missing', async () => {
    const oldStorage: VersionedData = {
      meta: { version: PREVIOUS_VERSION },
      data: {
        OtherController: { foo: 'bar' },
    ...
    expect(changedControllers.has('TokenListController')).toBe(false);
  });
});
