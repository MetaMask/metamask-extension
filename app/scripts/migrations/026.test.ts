...
type MigrationInput = Parameters<typeof migration26.migrate>[0];
...
describe('migration #26', () => {
  it('should move the identities from KeyringController', async () => {
    const newStorage = await migration26.migrate(oldStorage as MigrationInput);
    const newState = newStorage.data as Record<
      'PreferencesController',
      {
        identities?: Record<
          string,
    ...
    const { identities } = newState.PreferencesController;
    ...
    expect(newState.KeyringController.walletNicknames).toBeUndefined();
  });
