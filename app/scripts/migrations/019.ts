...
function transformState(state: LegacyState): LegacyState {
  ...
  if (TransactionController && TransactionController.transactions) {
    ...
    TransactionController.transactions = transactions.map(
      (txMeta, _, txList) => {
...
function getHighestContinuousFrom(
  txList: LegacyTransaction[],
  startPoint: number,
): number {
...
function getHighestNonce(txList: LegacyTransaction[]): number {
  const nonces = txList.map((txMeta) => {
    const { nonce } = txMeta.txParams;
