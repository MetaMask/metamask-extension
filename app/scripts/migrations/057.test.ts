...
describe('migration #57', () => {
  ...
  it('should not blow up if transactions are not an array', async () => {
    ...
    const result1 = await migration57.migrate(
      storageWithTransactionsAsString as unknown as MigrationInput,
    );

    const result2 = await migration57.migrate(
      storageWithTransactionsAsArrayOfString as unknown as MigrationInput,
    );
