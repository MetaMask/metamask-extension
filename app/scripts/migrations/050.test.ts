...
type MigrationInput = Parameters<typeof migration50.migrate>[0];
...
describe('migration #50', () => {
  let mockLocalStorageRemoveItem: jest.SpiedFunction<
  ...
  it('should update the version metadata', async () => {
    ...
    const newStorage = await migration50.migrate(
  ...
  it('should call window.localStorage.removeItem for each legacy key', async () => {
    ...
    await migration50.migrate(oldStorage as unknown as MigrationInput);
