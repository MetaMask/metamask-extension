...
describe('RewardsDataService', () => {
  ...
  beforeEach(() => {
    ...
    process.env.REWARDS_API_URL = AppConstants.REWARDS_API_URL.DEV;
  ...
  describe('constructor', () => {
    ...
    it('uses REWARDS_API_URL from environment variable when set', () => {
      process.env.REWARDS_API_URL = 'https://custom.api.url';
      service = createService();
      expect(service.name).toBe('RewardsDataService');
    });
  ...
  describe('locale handling', () => {
    ...
    it('normalizes locale from underscore to hyphen format', async () => {
    ...
    it('falls back to en-US when PreferencesController call fails', async () => {
      // Create a new messenger that throws an error
      const customBaseMessenger = new Messenger<
        PreferencesControllerGetStateAction,
        never
      >();
  ...
  describe('API URL configuration', () => {
    it('prioritizes REWARDS_API_URL environment variable', async () => {
      const customUrl = 'https://custom.rewards.api';
      process.env.REWARDS_API_URL = customUrl;
      service = createService();
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        `${customUrl}/referral/validate?code=TEST`,
        expect.any(Object),
      );
    });
