...
const MOCK_SEASON_STATUS: SeasonStatusDto = {
  season: MOCK_SEASON_METADATA,
  balance: {
    total: 250,
    updatedAt: new Date('2024-06-01T00:00:00.000Z'),
  },
...
async function withController<ReturnValue>(
  ...
  const { state, rewardsEnabled = true } = options;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const globalMessenger = new Messenger<any, any>();

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const restrictedMessenger: any = globalMessenger.getRestricted({
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  jest
    .spyOn(restrictedMessenger, 'call')
    .mockImplementation(mockMessengerCall as any);
...
describe('RewardsController', () => {
  describe('constructor', () => {
    it('should initialize with default state when rewards are disabled', async () => {
      await withController({ rewardsEnabled: false }, ({ controller }) => {
    ...
    it('should initialize with default state when rewards are enabled', async () => {
      await withController({ rewardsEnabled: true }, ({ controller }) => {
        expect(controller.state).toMatchObject({
          ...getRewardsControllerDefaultState(),
          rewardsEnabled: true,
        });
    ...
    it('should initialize with provided state', async () => {
      ...
      await withController(
        { state: customState, rewardsEnabled: true },
        ({ controller }) => {
          expect(controller.state.accounts).toEqual(customState.accounts);
        },
      );
    });
    ...
    it('should register action handlers', async () => {
      await withController(
        { rewardsEnabled: true },
        ({ controller, mockMessengerCall }) => {
          // Verify controller is instantiated
          expect(controller).toBeDefined();
          expect(controller.state.rewardsEnabled).toBe(true);
        },
      );
    });
  });
  ...
  describe('performSilentAuth', () => {
    it('should set activeAccount to null when no internal account provided', async () => {
      await withController({ rewardsEnabled: true }, async ({ controller }) => {
    ...
    it('should successfully perform silent auth for EVM account', async () => {
      await withController(
        ...
        async ({ controller, mockMessengerCall }) => {
          ...
          expect(result).toBe(MOCK_SUBSCRIPTION_ID);
          expect(controller.state.activeAccount).toMatchObject({
            account: MOCK_CAIP_ACCOUNT,
            hasOptedIn: true,
            subscriptionId: MOCK_SUBSCRIPTION_ID,
          });
    ...
    it('should opt in account successfully', async () => {
      await withController(
        ...
        async ({ controller, mockMessengerCall }) => {
          ...
          expect(result).toBe(MOCK_SUBSCRIPTION_ID);
          expect(controller.state.accounts[MOCK_CAIP_ACCOUNT]).toMatchObject({
            hasOptedIn: true,
            subscriptionId: MOCK_SUBSCRIPTION_ID,
          });
        },
  ...
  describe('invalidateSubscriptionCache', () => {
    it('should invalidate cache for specific season', async () => {
      ...
      await withController(
        { state, rewardsEnabled: true },
        ({ controller }) => {
          expect(controller.state.seasonStatuses[compositeKey]).toBeDefined();
          ...
          expect(controller.state.seasonStatuses[compositeKey]).toBeUndefined();
        },
      );
    });
    ...
    it('should invalidate cache for all seasons', async () => {
      ...
      await withController(
        { state, rewardsEnabled: true },
        ({ controller }) => {
          expect(controller.state.seasonStatuses[compositeKey1]).toBeDefined();
          expect(controller.state.seasonStatuses[compositeKey2]).toBeDefined();

          controller.invalidateSubscriptionCache(MOCK_SUBSCRIPTION_ID);

          expect(
            controller.state.seasonStatuses[compositeKey1],
          ).toBeUndefined();
          expect(
            controller.state.seasonStatuses[compositeKey2],
          ).toBeUndefined();
        },
      );
    });
    ...
    it('should handle authentication trigger when rewards are disabled', async () => {
      await withController(
        ...
        async ({ controller }) => {
          ...
          // Should not throw and should set activeAccount to null
          expect(controller.state.activeAccount).toBeNull();
        },
      );
    });
