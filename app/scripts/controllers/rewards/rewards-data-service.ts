...
export class RewardsDataService {
   ...
   *
   * @returns The normalized locale string (e.g., 'en-US', 'fr-CA')
   */
  private getLocale(): string {
    try {
      // @ts-expect-error - PreferencesController:getState is allowed via messenger configuration
      const preferencesState = this.#messenger.call(
        'PreferencesController:getState',
      ) as { currentLocale?: string };
      const currentLocale = preferencesState?.currentLocale;
      return getNormalisedLocale(currentLocale || 'en-US');
    } catch (error) {
