...
module.exports = class SignController extends EventEmitter {

  /**
   * Controller in charge of managing - storing, adding, removing, updating - Messages.
   *
   * @typedef {Object} SignController
   ...
   * @property {array} messages Holds all messages that have been created by this SignController
   *
   */
  constructor (opts) {
    super()
   ...
   * @throws A 'SignController - Message not found for id: "${msgId}".' if there is no Message in this.messages with an
  ...
  _setMsgStatus (msgId, status) {
    const msg = this.getMsg(msgId)
    if (!msg) throw new Error('SignController - Message not found for id: "${msgId}".')
    msg.status = status
    this._updateMsg(msg)
