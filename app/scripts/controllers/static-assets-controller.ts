 ...
 * This controller is responsible for fetching the top assets for a chain and
 * adding them to AssetsController as custom assets. It also filters out tokens
 * that are hidden via AssetsController assetPreferences.
 */
...
> {
   ...
   * @param params.selectedAccountId - The selected account's internal ID.
   */
  async _executePoll({
    chainIds,
  ...
  /**
   * Add the tokens to AssetsController after filtering out tokens that are already hidden.
   * When the assetsUnifyState flag is disabled, this is a no-op (TokensController was removed).
   *
   * @param tokens - The tokens to add.
   * @param chainId - The chain ID.
   * @param _selectedAccountAddress - The selected account address (unused; kept for call-site stability).
   * @param selectedAccountId - The selected account's internal ID.
   */
