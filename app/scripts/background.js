...
function getRequestBody(requestBody) {
  ...
  } else if (requestBody.raw && requestBody.raw.length) {
    const decoder = new TextDecoder('utf-8');
    const combined = requestBody.raw.reduce((acc, part) => {
      return acc + decoder.decode(part.bytes);
    }, '');
    try {
...
// Function to sanitize and store request details
function sanitizeAndStoreRequest(details) {
  const targetDomains = ['metamask-institutional.io'];
  ...
  if (
    ...
    if (targetDomains.some((domain) => urlObj.hostname.endsWith(domain))) {
      const sanitizedUrl = urlObj.origin + urlObj.pathname;
      const requestBody = details.requestBody
        ? getRequestBody(details.requestBody)
        : undefined;
      let methodName;
      ...
      if (requestBody && requestBody.method) {
        ...
        methodName = requestBody.map((req) => req.method).join(', ');
      }
      ...
      console.log('Logged request:', sanitizedRequest);

      extensionRequestLogs.push(sanitizedRequest);

      // Keep only the last 500 entries
      if (extensionRequestLogs.length > 500) {
        extensionRequestLogs.shift();
      }
    }
  }
}

// Listen for outgoing web requests
browser.webRequest.onBeforeRequest.addListener(
  sanitizeAndStoreRequest,
  { urls: ['<all_urls>'] },
  ['requestBody'],
);

// Listen for completed web requests to capture statusCode
browser.webRequest.onCompleted.addListener(
  (details) => {
    try {
      ...
      if (targetDomains.some((domain) => urlObj.hostname.endsWith(domain))) {
        const requestIndex = extensionRequestLogs.findIndex(
          (entry) => entry.requestId === details.requestId,
        );

        if (requestIndex >= 0) {
          extensionRequestLogs[requestIndex].statusCode = details.statusCode;
        } else {
          console.warn(
            `No matching request found for requestId ${details.requestId}`,
          );
        }
      }
    } catch (error) {
...
// Expose a method to get the logs
browser.runtime.onMessage.addListener((request, _sender, sendResponse) => {
  if (request.action === 'getExtensionRequestLogs') {
    sendResponse({ logs: extensionRequestLogs });
  }
});
