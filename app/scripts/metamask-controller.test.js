...
describe('MetaMaskController', () => {
  ...
  describe('MetaMaskController Behaviour', () => {
    ...
    describe('network connection banner lifecycle', () => {
      /**
       * Route `KeyringController:getState` to a fixed unlock state and swallow
       * the banner controller lifecycle calls so assertions are deterministic.
       *
       * @param {boolean} isUnlocked - The keyring unlock state to report.
       * @returns {jest.SpyInstance} The `controllerMessenger.call` spy.
       */
      function stubBannerLifecycleCalls(isUnlocked) {
        ...
        return jest
          ...
          .mockImplementation((method, ...args) => {
            ...
            if (method.startsWith('NetworkConnectionBannerController:')) {
              return undefined;
            }
      ...
      it('starts the banner controller when a UI connects while unlocked', () => {
        const callSpy = stubBannerLifecycleCalls(true);

        metamaskController.activeControllerConnections = 1;
        metamaskController.emit('controllerConnectionChanged', 1);
      ...
      it('stops the banner controller when a UI connects while locked', () => {
        const callSpy = stubBannerLifecycleCalls(false);
      ...
      it('stops the banner controller when the last UI disconnects', () => {
        const callSpy = stubBannerLifecycleCalls(true);

        metamaskController.activeControllerConnections = 0;
        metamaskController.emit('controllerConnectionChanged', 0);
      ...
      it('starts the banner controller on unlock while a UI is connected', () => {
        const callSpy = stubBannerLifecycleCalls(true);

        metamaskController.activeControllerConnections = 1;
        metamaskController.emit('unlock');
      ...
      it('stops the banner controller on lock', () => {
        const callSpy = stubBannerLifecycleCalls(false);

        metamaskController.activeControllerConnections = 1;
        metamaskController.emit('lock');
      ...
      it('does not propagate banner lifecycle errors to UI connection setup', () => {
        const originalCall = metamaskController.controllerMessenger.call.bind(
          metamaskController.controllerMessenger,
        );
        jest
          ...
          .mockImplementation((method, ...args) => {
            ...
            if (method === 'NetworkConnectionBannerController:start') {
              throw new Error('banner start failed');
            }
            return originalCall(method, ...args);
          });
