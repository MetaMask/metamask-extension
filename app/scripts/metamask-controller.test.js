...
describe('MetaMaskController', () => {
  ...
  describe('MetaMaskController Behaviour', () => {
    ...
    describe('#addToken', () => {
      ...
      const symbol = 'LINK';
      const decimals = 18;
      const networkClientId = NETWORK_CONFIGURATION_ID_1;

      it('delegates to TokensController.addToken when assets-unify state is off', async () => {
      ...
      describe('with assets-unify state enabled', () => {
        ...
        beforeEach(() => {
          jest
            .spyOn(environment, 'getIsAssetsUnifiedStateIncludedInBuild')
          ...
          jest
            ...
            .mockReturnValue({
              configuration: { chainId: MAINNET_CHAIN_ID },
            });
          jest
            .spyOn(unifyMetamaskController.assetsController, 'addCustomAsset')
            .mockResolvedValue(undefined);
          jest
            .spyOn(unifyMetamaskController.tokensController, 'addToken')
            .mockResolvedValue(undefined);
        });

        it('adds token via AssetsController.addCustomAsset', async () => {
          const image = 'https://example.com/icon.png';
          const expectedAssetId = toAssetId(address, MAINNET_CHAIN_ID);

          await unifyMetamaskController.getApi().addToken({
            address,
            symbol,
            decimals,
            image,
          ...
          expect(
            unifyMetamaskController.tokensController.addToken,
          ).not.toHaveBeenCalled();
          expect(
            unifyMetamaskController.assetsController.addCustomAsset,
          ).toHaveBeenCalledWith(
        ...
        it('omits iconUrl when image is not provided', async () => {
          const expectedAssetId = toAssetId(address, MAINNET_CHAIN_ID);
        ...
        it('throws when assetId cannot be built', async () => {
          unifyMetamaskController.networkController.getNetworkClientById.mockReturnValue(
            {
              configuration: {},
            },
          );
          ...
          await expect(
            unifyMetamaskController.getApi().addToken({
              address,
              symbol,
              decimals,
              networkClientId,
