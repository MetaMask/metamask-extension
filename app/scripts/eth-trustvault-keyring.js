...
export default class TrustvaultKeyring extends EventEmitter {
  ...
  async getAccounts() {
    ...
    if (
      ...
      this.dateAccountsWereLastFetched &&
      this.dateAccountsWereLastFetched + FIVE_MINUTES_IN_MILLISECONDS > Date.now()
    ) {
  ...
  async _signTransaction(address, tx, transactionDigest) {
    ...
    if (transactionId) {
      const {
        transaction: { v, r, s },
      } = await this._pollTransaction(transactionId);
      return { v, r, s };
    }
  }
