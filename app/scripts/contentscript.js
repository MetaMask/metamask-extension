...
function extensionStreamMessageListener(msg) {
  ...
  if (msg.data.method === 'metamask_chainChanged' && msg.data.params.nickname) {
    const toastDisplaySeconds = 5;
    const message = `Using <strong>${msg.data.params.nickname}</strong> for <strong>${window.location.host}</strong>`;
