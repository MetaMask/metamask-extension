...
export class LedgerDmkBridgeHandler {
   ...
   *
   * Logs every action's start, parameters, outcome, and duration so the
   * full DMK dispatch flow is observable from the offscreen DevTools console
  ...
  async handleAction(
    ...
    console.log('[LedgerDMK] → action start', action, params ?? {});
    try {
      const result = await this.dispatchAction(action, params);
      console.log(
        `[LedgerDMK] ✓ action success (${Math.round(performance.now() - startedAt)}ms)`,
        action,
        result,
      );
      return result;
      ...
      console.log(
        `[LedgerDMK] ✗ action failed (${Math.round(performance.now() - startedAt)}ms)`,
        action,
        error,
      );
      throw error;
  ...
  /**
   * Dispatches a Ledger action to the DMK bridge.
   *
   * @param action - The Ledger action to perform.
   * @param params - Optional parameters for the action.
   * @returns The result of the action.
