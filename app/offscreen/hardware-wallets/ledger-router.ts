...
function ensureMessageListener(): void {
  ...
  const listener: ChromeMessageListener = (message, _sender, sendResponse) => {
    if (
    ...
    // Keep the message channel open for the async sendResponse call.
    return true;
  };
...
async function performSwitch(mode: LedgerHandlerMode): Promise<void> {
  ...
  console.debug('[ledger-router] switched handler to', mode);
}
