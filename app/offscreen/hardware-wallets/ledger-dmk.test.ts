...
const mockOnSessionStateChangeSubject = new Subject<{ connected: boolean }>();

jest.mock('@metamask/eth-ledger-bridge-keyring', () => ({
  LedgerDMKBridge: jest.fn().mockImplementation(() => ({
...
describe('LedgerDMKBridgeHandler', () => {
  beforeEach(() => {
    ...
    mockBridgeDmkListenToAvailableDevices.mockReturnValue({
      subscribe: ({ next }: { next: (devices: unknown[]) => void }) => {
        next([{ name: 'MockLedgerDevice' }]);
