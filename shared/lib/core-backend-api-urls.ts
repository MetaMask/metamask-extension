...
/**
 * Whether bearer tokens must be omitted from core-backend REST requests.
 * Production tokens are rejected by non-production backends, so pointing the
 * URLs above at a dev/local backend usually requires disabling auth too.
 *
 * @returns True when `MM_BACKEND_DISABLE_AUTH` is set to `"true"`.
 */
export function isBackendAuthDisabled(): boolean {
  return process.env.MM_BACKEND_DISABLE_AUTH?.toString() === 'true';
}
