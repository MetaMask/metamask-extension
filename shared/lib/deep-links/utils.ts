...
export function getDeferredDeepLinkFromCookie(): Promise<DeferredDeepLink | null> {
  return new Promise((resolve) => {
    // Make sure that promise resolves after timeout in case extension API doesn't respond
    const timeoutId = setTimeout(() => {
      console.warn('Timeout retrieving deferred deeplink cookie');
      resolve(null);
    }, 5000);

    try {
      // TODO: Make sure this works in Firefox as well
      chrome.cookies.get(
        ...
        (cookie) => {
          ...
          try {
            ...
            if (
              ...
              console.warn('Invalid referringLink in deferred_deeplink cookie');
              resolve(null);
              return;
            }
            ...
            console.warn(
              `Failed to parse deferred_deeplink cookie: ${error instanceof Error ? error.message : 'Unknown error'}`,
            );
            resolve(null);
          }
        },
      ...
      console.warn(
        `Failed to use browser API for deferred deep link cookies: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      resolve(null);
    }
  });
