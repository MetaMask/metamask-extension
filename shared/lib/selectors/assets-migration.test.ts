...
const mockAccountAddressLowercase: Hex = "0xd8da6bf26964af9d7eed9e03e53415d37aa96045";
const mockAccountAddressChecksummed = toChecksumHexAddress(mockAccountAddressLowercase) as Hex;

const nativeEthAssetId = "eip155:1/slip44:60";
const erc20AssetAddressLowercase: Hex = "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48";
const erc20AssetAddressChecksummed = toChecksumHexAddress(erc20AssetAddressLowercase) as Hex;
...
const mockAccountAddressLowercase2: Hex = "0x1234567890abcdef1234567890abcdef12345678";
const enabledFlags = {
  remoteFeatureFlags: {
    [ASSETS_UNIFY_STATE_FLAG]: {
      enabled: true,
...
describe("getAccountTrackerControllerAccountsByChainId", () => {
  ...
  describe("edge cases when enabled", () => {
    ...
    it("handles balance with 0 decimals", () => {
      ...
      expect(result["0x2a"][mockAccountAddressChecksummed].balance).toBe("0x2a");
    });

    it("skips balance entries without metadata in assetsInfo", () => {
      const unknownAssetId = "eip155:1/erc20:0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef";
      const state = {
        metamask: {
          ...enabledFlags,
          accountsByChainId: {},
    ...
    it("skips non-native assets (ERC-20) from accountsByChainId", () => {
      ...
      expect(Object.keys(result["0x1"])).toStrictEqual([mockAccountAddressChecksummed]);
    });
    ...
    it('does not crash when amount is in scientific notation (e.g. "1e-18")', () => {
      ...
      expect(() => getAccountTrackerControllerAccountsByChainId(state)).not.toThrow();
      const result = getAccountTrackerControllerAccountsByChainId(state);
    ...
    it('correctly parses positive scientific notation (e.g. "1.5e2" with 2 decimals)', () => {
      ...
      expect(() => getAccountTrackerControllerAccountsByChainId(state)).not.toThrow();
      const result = getAccountTrackerControllerAccountsByChainId(state);
      expect(result["0x1"][mockAccountAddressChecksummed].balance).toBe("0x3a98");
    });
  });
});
...
describe("getTokenBalancesControllerTokenBalances", () => {
  ...
  describe("when assets unify state feature is enabled (happy path)", () => {
    ...
    it("adds zero-balance placeholder for custom EVM token not yet in assetsBalance", () => {
      const customTokenAddress: Hex = "0x4d5f47fa6a74757f35c14fd3a6ef8e3c9bc514e8";
      const customTokenAssetId = `eip155:1/erc20:${customTokenAddress}`;
      const customTokenAddressChecksummed = toChecksumHexAddress(customTokenAddress) as Hex;
  ...
  describe("edge cases when enabled", () => {
    it("skips balance entries without metadata in assetsInfo", () => {
      ...
      const nativeAddress = getNativeAssetForChainId("0x1").address;
      expect(Object.keys(result[mockAccountAddressLowercase]["0x1"])).toStrictEqual([
        nativeAddress,
      ]);
    });
    ...
    it("handles multiple EVM accounts", () => {
      ...
      expect(result[mockAccountAddressLowercase]["0x1"][erc20AssetAddressChecksummed]).toBe(
        "0x989680",
      ); // 10 * 10^6
      expect(result[mockAccountAddressLowercase2]["0x1"][erc20AssetAddressChecksummed]).toBe(
        "0x1312d00",
      ); // 20 * 10^6
    });
  });
    ...
    it("merges and deduplicates assetsBalance and customAssets", () => {
      ...
      const state = {
        metamask: {
          ...
          customAssets: {
            [mockAccountId2]: [solanaTokenAssetId as CaipAssetType, extraSolAssetId],
          },
          internalAccounts: {
            accounts: {
              [mockAccountId2]: {
...
describe("getTokenRatesControllerMarketData", () => {
  ...
  describe("edge cases when enabled", () => {
    it("skips non-EIP155 assets from marketData", () => {
      ...
      expect(result["0x1"]).toBeDefined();
      expect(Object.keys(result).every((key) => key.startsWith("0x"))).toBe(true);
    });
    ...
    it("places native slip44 asset at getNativeTokenAddress", () => {
      ...
      expect(result["0x1"][nativeTokenAddress].tokenAddress).toBe(nativeTokenAddress);
    ...
    it("does not use slip44 fallback for ERC-20 assets missing from assetsInfo", () => {
      const ethPrice = 2000;
      const nativeTokenAddress = "0x0000000000000000000000000000000000000000";
      const orphanErc20AssetId = "eip155:1/erc20:0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef";
      const state = {
        metamask: {
          ...enabledFlags,
          marketData: {},
...
describe("getRatesControllerRates", () => {
  const solanaNativeAssetId = "solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp/slip44:501";
...
describe("getIsAssetsUnifiedStateIncludedInBuild compile-time gate", () => {
  ...
  it("returns legacy accountsByChainId when the build excludes unified state even if the remote flag is enabled", () => {
    ...
    expect(getAccountTrackerControllerAccountsByChainId(state)).toBe(legacyAccountsByChainId);
  });
});
