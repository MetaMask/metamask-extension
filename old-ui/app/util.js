  ...
  if (decimalsToKeep === undefined) {
    if (beforeDecimal === '0') {
      if (afterDecimal !== '0') {
        ...
        formatted = '0.' + afterDecimal + ' AUX'
        // RvH Changed ETH to AUX
      }
    } else {
      formatted = beforeDecimal + '.' + afterDecimal.slice(0, 3) + ' AUX'
      // RvH Changed ETH to AUX
    }
  } else {
    afterDecimal += Array(decimalsToKeep).join('0')
    formatted = beforeDecimal + '.' + afterDecimal.slice(0, decimalsToKeep) + ' AUX'
    // RvH Changed ETH to AUX
  }
  return formatted
}
