const {selectFragmentBySuccessEvent, selectFragmentById, selectMatchingFragment, getLatestMetricsEventTimestamp} = require('.');
...
describe('selectMatchingFragment', () => {
    it('should find matching fragment in state by id', () => {
        ...
        const selected = selectMatchingFragment(state, {
            fragmentOptions: { successEvent: 'event name' },
            existingId: 'randomid'
        });
        expect(selected).toHaveProperty('id', 'randomid');
    });
    describe('getLatestMetricsEventTimestamp', () => {
        it('should find matching fragment in state by id', () => {
            const state = { metamask: { MetaMetricsController: { latestNonAnonymousEventTimestamp: 12345 } } };
