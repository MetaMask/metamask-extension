...
import { RWAFeatureFlag } from '../../../shared/lib/rwa-feature-flag';
import { selectRWAEnabledFlag } from './feature-flags';
...
type MockState = {
  metamask: {
    remoteFeatureFlags: {
      rwaEnabledVersion?: RWAFeatureFlag;
    };
  };
};

const getMockState = (rwaEnabledVersion?: RWAFeatureFlag): MockState => ({
  metamask: {
    remoteFeatureFlags: {
      rwaEnabledVersion,
    },
...
describe('RWA Feature Flags', () => {
  ...
  describe('selectRWAEnabledFlag', () => {
    describe('undefined or missing flag', () => {
      it('returns false when rwaEnabledVersion flag is undefined', () => {
        const state = getMockState(undefined);
        expect(selectRWAEnabledFlag(state)).toBe(false);
    ...
    describe('JSON flags with version gating', () => {
      it('returns true when enabled is true and version check passes', () => {
        ...
        expect(selectRWAEnabledFlag(state)).toBe(true);
