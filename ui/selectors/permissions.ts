...
export const getPermissionSubjectsDeepEqual = createDeepEqualSelector(
  (state: { metamask: Pick<BackgroundStateProxy, 'PermissionController'> }) =>
...
export const getSubjectMetadataDeepEqual = createDeepEqualSelector(
  (state: {
    metamask: Pick<BackgroundStateProxy, 'SubjectMetadataController'>;
 ...
 * @returns {object} The permissions subjects object.
 */
export function getPermissionSubjects(state: MultichainState) {
...
export function getPermittedAccounts(
  state: {
    metamask: Pick<BackgroundStateProxy, 'PermissionController'>;
  },
  origin: string,
) {
...
export function getPermittedAccountsForCurrentTab(
  state: {
    metamask: Pick<BackgroundStateProxy, 'PermissionController'>;
  } & Pick<MetaMaskReduxState, 'activeTab'>,
) {
 ...
 */
export function getPermittedAccountsByOrigin(state: MultichainState) {
...
export function getPermittedChainsByOrigin(state: MultichainState) {
  const subjects = getPermissionSubjects(state);
...
export function getSubjectMetadata(state: MultichainState) {
 ...
 */
export function getConnectedSubjectsForSelectedAddress(state: MultichainState) {
...
export function getAccountToConnectToActiveTab(
  state: {
    metamask: Pick<
      BackgroundStateProxy,
      'AccountsController' | 'PermissionController'
    >;
  } & Pick<MetaMaskReduxState, 'activeTab'>,
) {
  const selectedInternalAccount = getSelectedInternalAccount(state);
...
export function getPermissionsForActiveTab(
  state: {
    metamask: Pick<BackgroundStateProxy, 'PermissionController'>;
  } & Pick<MetaMaskReduxState, 'activeTab'>,
) {
...
export function activeTabHasPermissions(
  state: {
    metamask: Pick<BackgroundStateProxy, 'PermissionController'>;
  } & Pick<MetaMaskReduxState, 'activeTab'>,
) {
...
export function getLastConnectedInfo(state: {
  metamask: Pick<BackgroundStateProxy, 'PermissionLogController'>;
}) {
  const {
    PermissionLogController: { permissionHistory = {} },
  } = state.metamask;
...
export function getPermissionsRequests(state: ApprovalsMetaMaskState) {
...
export function getFirstPermissionRequest(state: ApprovalsMetaMaskState) {
  const requests = getPermissionsRequests(state);
  return requests && requests[0] ? requests[0] : null;
}

export function getPermissions(state: MultichainState, origin: string) {
