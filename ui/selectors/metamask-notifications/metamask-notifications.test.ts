...
import { selectIsMetamaskNotificationsEnabled, getMetamaskNotifications, getMetamaskNotificationsReadList, getMetamaskNotificationsUnreadCount, selectIsFeatureAnnouncementsEnabled, } from './metamask-notifications';
type Notification = NotificationServicesController.Types.INotification;
const mockNotifications: Notification[] = [
    NotificationServicesController.Processors.processNotification(NotificationServicesController.Mocks.createMockNotificationEthReceived()),
];
describe('Metamask Notifications Selectors', () => {
    ...
    it('should select the count of unread MetaMask notifications from state', () => {
        const expectedUnreadNotificationsCount = mockNotifications.filter((notification) => !notification.isRead).length;
        expect(getMetamaskNotificationsUnreadCount(mockState)).toEqual(expectedUnreadNotificationsCount);
    });
