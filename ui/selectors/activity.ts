...
import type { TransactionGroup } from './selectors.types';
...
function isFromSelectedAccount(tx: TransactionMeta, selectedAddress: string) {
  // Must be from the selected account (ported from selectedAddressTxListSelector)
  if (tx.txParams?.from?.toLowerCase() !== selectedAddress) {
    return false;
  }
  // Exclude certain transaction types (ported from txStatusSortedTransactionsSelector)
...
/**
 * Selects local pending transactions grouped by nonce for activity list rendering.
 *
 * Transaction lifecycle:
 * 1. User initiates tx → enters Redux with status 'unapproved'
 * 2. User approves → status changes to 'approved'
 * 3. Broadcast to network → status changes to 'submitted'
 * 4. Confirmed on-chain → status changes to 'confirmed'
 * 5. API indexes the tx → now available via API
 *
...
export const getPendingTransactionGroups = createSelector(
  ...
  (transactions, selectedAccount): TransactionGroup[] => {
    ...
    const filtered = transactions.filter((tx) => {
      ...
      // Include all transactions in pending states (ported from pendingStatusSortedTransactionsSelector)
...
export const getRecentTransactionGroups = createSelector(
  ...
  (transactions, selectedAccount): TransactionGroup[] => {
    ...
    const filtered = transactions.filter((tx) => {
      ...
      // Only completed (non-pending) transactions (inverse of pendingStatusSortedTransactionsSelector)
