...
import { MOCK_ACCOUNTS, MOCK_ACCOUNT_EOA, MOCK_ACCOUNT_ERC4337, MOCK_ACCOUNT_BIP122_P2WPKH, MOCK_ACCOUNT_BIP122_P2WPKH_TESTNET, } from '../../test/data/mock-accounts';
import mockState from '../../test/data/mock-state.json';
import { AccountsState, isSelectedInternalAccountEth, isSelectedInternalAccountBtc, hasCreatedBtcMainnetAccount, hasCreatedBtcTestnetAccount, getSelectedInternalAccount, getInternalAccounts, } from './accounts';
...
describe('Accounts Selectors', () => {
    describe('#getInternalAccounts', () => {
        it('returns a list of internal accounts', () => {
            expect(getInternalAccounts(mockState as AccountsState)).toStrictEqual(Object.values(mockState.metamask.internalAccounts.accounts));
        });
    });
    describe('#getSelectedInternalAccount', () => {
        it('returns selected internalAccount', () => {
            expect(getSelectedInternalAccount(mockState as AccountsState)).toStrictEqual({
        ...
        it('returns undefined if selectedAccount is undefined', () => {
            expect(getSelectedInternalAccount({
                AccountsController: {
                    internalAccounts: {
                        accounts: {},
                        selectedAccount: '',
    ...
    describe('isSelectedInternalAccountBtc', () => {
        ...
        ])('returns $isBtc if the account is: $type', ({ id, isBtc }: {
            id: string;
            isBtc: boolean;
        }) => {
            const state = MOCK_STATE;
                ...
                AccountsController: {
                    internalAccounts: {
                        ...
                        accounts: {
                            ...
                            mock_account_bip122_p2wpkh_testnet: MOCK_ACCOUNT_BIP122_P2WPKH_TESTNET,
                        },
                    }
                }
            };
