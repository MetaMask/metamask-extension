...
Children to the `SelectButton` component will redener below in the same content area as `label` and `description`, so you can still use `startAccessory` and `endAccessory`.
