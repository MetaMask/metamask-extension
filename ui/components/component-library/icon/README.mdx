...
Use the [IconSearch](/components-componentlibrary-icon--icon-search) story to find the icon you want to use.
...
### Adding a new icon component

To add a new icon component the only thing you need to do is add a new svg file to `app/images/icons` and run `yarn build:icons`. There are a few steps to ensure that the icon is added correctly.
...
Run `yarn build:icons` to generate the icon component. This will add the icon name to `./IconNames.js` and generate the correct styles that will be added to `icon.scss`
