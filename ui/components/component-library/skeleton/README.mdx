...
Use the `scaleY` prop to shrink the `Skeleton` component in the vertical direction when it is being shown.

<Canvas of={SkeletonStories.VerticalScaling} />

```jsx
import { Skeleton, Text, TextVariant } from '../../component-library';

<Skeleton width="max-content" scaleY={0.8}>
  <Text variant={TextVariant.headingMd}>Content that loads</Text>
</Skeleton>;
```
