...
`escKeyClose` is a callback function that is invoked when the 'Escape' key is pressed within the `Popover` component
...
<Popover escKeyClose={() => setIsOpen(false)}>Press esc key to close</Popover>;
```
