...
`BannerTip` is an inline notification that notifies users of important information & sometimes time-sensitive changes.
...
Use the `logoType` prop with the `BANNER_TIP_LOGOS` object from `../../component-library` to change the context of `BannerTip`.

Possible options:

- `BANNER_TIP_LOGOS.GREETING` Default
- `BANNER_TIP_LOGOS.CHAT`
...
<BannerTip logoType={BANNER_TIP_LOGOS.GREETING}>
  This is a demo of greeting.
</BannerTip>
<BannerTip logoType={BANNER_TIP_LOGOS.CHAT}>
