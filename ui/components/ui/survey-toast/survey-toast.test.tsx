...
const createStore = (options = { metametricsEnabled: true }) => mockStore({
...
const renderComponent = (options = { metametricsEnabled: true }) => renderWithProvider(<MetaMetricsContext.Provider>value, { mockTrackEvent } >
    <SurveyToast>/>
    < /MetaMetricsContext.Provider>,, createStore(options));
describe('SurveyToast', () => {
    ...
    it('renders the survey toast when a valid survey is available', async () => {
        ...
        await waitFor(() => {
            ...
            expect(screen.getByText(surveyData.valid.description)).toBeInTheDocument();
