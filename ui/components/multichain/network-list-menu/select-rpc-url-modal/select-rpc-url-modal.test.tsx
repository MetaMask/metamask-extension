...
import { updateNetwork, setActiveNetwork, setEditedNetwork, toggleNetworkMenu, } from '../../../../store/actions';
...
describe('SelectRpcUrlModal Component', () => {
    ...
    it('renders select rpc url', () => {
        const { container } = renderWithProvider(<SelectRpcUrlModal>networkConfiguration, { networkConfiguration } /  > , store);
        expect(container).toMatchSnapshot();
    });
    it('should render the component correctly with network image and name', () => {
        const { getByRole, getByText } = renderWithProvider(<SelectRpcUrlModal>networkConfiguration, { networkConfiguration } /  > , store);
        const imageSrc = CHAIN_ID_TO_NETWORK_IMAGE_URL_MAP[networkConfiguration.chainId as keyof typeof CHAIN_ID_TO_NETWORK_IMAGE_URL_MAP];
    ...
    it('should render all RPC endpoints and highlight the selected one', () => {
        const { getByText } = renderWithProvider(<SelectRpcUrlModal>networkConfiguration, { networkConfiguration } /  > , store);
        networkConfiguration.rpcEndpoints.forEach((rpcEndpoint) => {
            expect(getByText(stripProtocol(rpcEndpoint.url))).toBeInTheDocument();
        });
        const selectedItem = getByText(stripProtocol(networkConfiguration.rpcEndpoints[0].url)).closest('.select-rpc-url__item');
    ...
    it('should dispatch the correct actions when an RPC endpoint is clicked', () => {
        const { getByText } = renderWithProvider(<SelectRpcUrlModal>networkConfiguration, { networkConfiguration } /  > , store);
        const rpcEndpoint = getByText(stripProtocol(networkConfiguration.rpcEndpoints[1].url));
        fireEvent.click(rpcEndpoint);
        expect(mockDispatch).toHaveBeenCalledWith(updateNetwork({
    ...
    it('should render the selected indicator correctly for the default RPC', () => {
        const { container } = renderWithProvider(<SelectRpcUrlModal>networkConfiguration, { networkConfiguration } /  > , store);
        const selectedPill = container.querySelector('.select-rpc-url__item-selected-pill');
        expect(selectedPill).toBeInTheDocument();
    });
    it('should render the modal with a network image', () => {
        renderWithProvider(<SelectRpcUrlModal>networkConfiguration, { networkConfiguration } /  > , store);
        const networkImage = screen.getByRole('img');
        expect(networkImage).toBeInTheDocument();
        expect(networkImage).toHaveAttribute('src', CHAIN_ID_TO_NETWORK_IMAGE_URL_MAP[networkConfiguration.chainId as keyof typeof CHAIN_ID_TO_NETWORK_IMAGE_URL_MAP]);
    });
    it('should handle click on RPC URL and update the network', () => {
        renderWithProvider(<SelectRpcUrlModal>networkConfiguration, { networkConfiguration } /  > , store);
        fireEvent.click(screen.getByText(stripProtocol(networkConfiguration.rpcEndpoints[1].url)));
        expect(mockDispatch).toHaveBeenCalledWith(updateNetwork({
            ...networkConfiguration,
            defaultRpcEndpointIndex: 1,
        }));
        expect(mockDispatch).toHaveBeenCalledWith(setActiveNetwork(networkConfiguration.rpcEndpoints[1].networkClientId));
        expect(mockDispatch).toHaveBeenCalledWith(setEditedNetwork());
        expect(mockDispatch).toHaveBeenCalledWith(toggleNetworkMenu());
    });
});
