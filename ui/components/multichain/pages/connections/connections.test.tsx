...
describe('Connections Content', () => {
    ...
    it('should render correctly', () => {
        const { container, getByTestId } = renderWithProvider(<MemoryRouter>initialEntries, { ['/connect/https%3A%2F%2Fmetamask.github.io']:  }
            >
                <Route>path, "/connect/:origin" >
            <Connections>/>
            < /Route>
            < /MemoryRouter>,, connectedStore);
        expect(container).toMatchSnapshot();
        expect(getByTestId('connections-page')).toBeInTheDocument();
    });
    it('it should render Disconnect all Account button of the page', () => {
        const { getByText } = renderWithProvider(<MemoryRouter>initialEntries, { ['/connect/https%3A%2F%2Fmetamask.github.io']:  }
            >
                <Route>path, "/connect/:origin" >
            <Connections>/>
            < /Route>
    ...
    it('it should trigger disconnect all accounts modal flow when disconnect all accounts button is clicked', async () => {
        const { getByText, getByTestId } = renderWithProvider(<MemoryRouter>initialEntries, { ['/connect/https%3A%2F%2Fmetamask.github.io']:  }
            >
                <Route>path, "/connect/:origin" >
            <Connections>/>
            < /Route>
