...
import { INITIAL_SEND_STATE_FOR_EXISTING_DRAFT, createMockInternalAccount, } from '../../../../../../test/jest/mocks';
...
const render = (state: Partial<MetaMaskReduxState> = {}, props = {}, sendStage = SEND_STAGES.ADD_RECIPIENT) => {
...
describe('SendPageAccountPicker', () => {
    ...
    describe('actions', () => {
        it('opens account picker when clicked', () => {
            ...
            expect(document.querySelector('.multichain-account-menu-popover')).toBeInTheDocument();
        });
    });
    describe('Multichain', () => {
        it('cannot select BTC accounts', async () => {
            ...
            expect(accountListAddressesContent).toContain(shortenAddress(normalizeSafeAddress(mockAccount.address)));
            expect(accountListAddressesContent).not.toContain(shortenAddress(normalizeSafeAddress(mockBtcAccount.address)));
        });
    });
});
