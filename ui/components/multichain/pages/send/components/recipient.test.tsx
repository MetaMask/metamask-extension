...
import { CONFUSING_ENS_ERROR, ENS_UNKNOWN_ERROR, } from '../../../../../pages/confirmations/send/send.constants';
...
describe('SendPageRecipient', () => {
    describe('your accounts', () => {
        it('shows user accounts when there is no input', () => {
            ...
            expect(container.querySelector('.multichain-account-list-item')).toBeInTheDocument();
        });
        it('shows user accounts when there is no input', () => {
            const { container } = render({
                send: { recipientInput: 'd', draftTransactions: {} },
            });
            expect(container.querySelector('.multichain-account-list-item')).toBeNull();
        });
    });
    describe('address book', () => {
        it('renders the address book tab', () => {
            const { container } = render();
            expect(container.querySelector('[data-testid="send-contacts-tab"]')).toBeInTheDocument();
        });
    });
