...
import { BtcAccountType, EthAccountType, KeyringAccountType, } from '@metamask/keyring-api';
...
import { CONFIRMATION_V_NEXT_ROUTE, CONNECT_HARDWARE_ROUTE, } from '../../../helpers/constants/routes';
///: END:ONLY_INCLUDE_IF
...
jest.mock('../../../hooks/accounts/useMultichainWalletSnapClient', () => ({
    ...jest.requireActual('../../../hooks/accounts/useMultichainWalletSnapClient'),
...
describe('AccountListMenu', () => {
    ...
    it('displays accounts for list and filters by search', () => {
        render();
        const listItems = document.querySelectorAll('.multichain-account-list-item');
        expect(listItems).toHaveLength(6);
        const searchBox = document.querySelector('input[type=search]') as Element;
        fireEvent.change(searchBox, {
            target: { value: 'Le' },
        });
        const filteredListItems = document.querySelectorAll('.multichain-account-list-item');
    ...
    it('displays the "no accounts" message when search finds nothing', () => {
        ...
        expect(getByTestId('multichain-account-menu-popover-no-results')).toBeInTheDocument();
    });
    it('should not render search bar when there is only one account', () => {
        ...
        const { container } = renderWithProvider(<AccountListMenu>onClose, { jest, : .fn() } /  > , mockStore);
    ...
    it('add / Import / Hardware button functions as it should', () => {
        ...
        const button = getAllByTestId('multichain-account-menu-popover-action-button');
    ...
    it('shows the account creation UI when Add Account is clicked', () => {
        ...
        const addAccountButton = document.querySelector('[data-testid="submit-add-account-with-name"]');
    ...
    describe('addSnapAccountButton', () => {
        ...
        it("doesn't render the add snap account button if it's disabled", async () => {
            ...
            const button = getByTestId('multichain-account-menu-popover-action-button');
            button.click();
            expect(() => getByText(messages.settingAddSnapAccount.message)).toThrow(`Unable to find an element with the text: ${messages.settingAddSnapAccount.message}`);
        });
        it('renders the "Add account Snap" button if it\'s enabled', async () => {
            ...
            const button = getByTestId('multichain-account-menu-popover-action-button');
            button.click();
            const addSnapAccountButton = getByText(messages.settingAddSnapAccount.message);
        ...
        it('opens the Snaps registry in a new tab', async () => {
            ...
            const addAccountSnapButton = getByText(messages.settingAddSnapAccount.message);
            ...
            AccountsController: {
                internalAccounts: {
                    accounts: {
                        ...mockState.metamask.internalAccounts.accounts,
                        'cf8dace4-9439-4bd4-b3a8-88c821c8fcb3': {
                            ...mockState.metamask.internalAccounts.accounts['cf8dace4-9439-4bd4-b3a8-88c821c8fcb3'],
                            metadata: {
                                name: 'Snap Account',
                                keyring: {
                                    type: 'Snap Keyring',
    ...
    describe('BTC account creation', () => {
        ...
        it('calls the bitcoin client to create an account', async () => {
            ...
            const button = getByTestId('multichain-account-menu-popover-action-button');
            button.click();
            const createBtcAccountButton = getByText(messages.addNewBitcoinAccount.message);
        ...
        it('redirects the user to the approval after clicking create account in the settings page', async () => {
            const { getByText, getByTestId } = render(undefined, undefined, '/settings');
            const button = getByTestId('multichain-account-menu-popover-action-button');
            button.click();
            const createBtcAccountButton = getByText(messages.addNewBitcoinAccount.message);
            createBtcAccountButton.click();
