...
const createStore = ({ sendInputCurrencySwitched }: Record<string, boolean>) => configureStore({
    marketData: {
...
describe('SwappableCurrencyInput', () => {
    it('native: matches snapshot', () => {
        ...
        const { asFragment, getByText } = render(<Provider>store, { createStore({ sendInputCurrencySwitched: , true: , }); }
            >
                <SwappableCurrencyInput>asset, { asset }, assetType = { AssetType, : .native }, amount = {}, { value: '5000' });
...
it('ERC721: matches snapshot', () => {
    ...
    const mockAssetChange = jest.fn();
    const { asFragment } = render(<Provider>store, { createStore({ sendInputCurrencySwitched: , true: , }); }
        >
            <SwappableCurrencyInput>asset, { asset }, assetType = { AssetType, : .NFT }, amount = {}, { value: '5000' });
}, onAmountChange = { mockAssetChange }
    /  >
    <>/Provider>,);
