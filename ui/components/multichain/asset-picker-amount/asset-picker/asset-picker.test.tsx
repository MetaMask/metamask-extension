...
import { CHAIN_IDS, CHAIN_ID_TO_NETWORK_IMAGE_URL_MAP, CHAIN_ID_TOKEN_IMAGE_MAP, } from '../../../../../shared/constants/network';
...
const store = (nativeTicker = NATIVE_TICKER, 
// TODO: Replace `any` with type
// eslint-disable-next-line @typescript-eslint/no-explicit-any
tokenList = {} as any, chainId: Hex = CHAIN_IDS.MAINNET) => configureStore({
    CurrencyController: {
        currencyRates: {
            [nativeTicker]: {
                conversionRate: 11.1,
...
describe('AssetPicker', () => {
    it('matches snapshot', () => {
        ...
        const { asFragment } = render(<Provider>store, { store(); } >
            <AssetPicker>header, { 'testHeader':  }, asset = { asset }, onAssetChange = {}());
        mockAssetChange();
    }, />
        < /Provider>,);
...
it('native: renders symbol and image', () => {
    ...
    const mockAssetChange = jest.fn();
    const { getByText, getByAltText } = render(<Provider>store, { store(); } >
        <AssetPicker>header, { 'testHeader':  }, asset = { asset }, onAssetChange = {}());
    mockAssetChange();
}, />
    < /Provider>,);
...
expect(getByAltText('NATIVE logo')).toHaveAttribute('src', CHAIN_ID_TOKEN_IMAGE_MAP['0x1']);
;
it('native: renders overflowing symbol and image', () => {
    ...
    const mockAssetChange = jest.fn();
    const { getByText, getByAltText } = render(<Provider>store, { store(NATIVE_TICKER); } >
        <AssetPicker>header, { 'testHeader':  }, asset = { asset }, onAssetChange = {}());
    mockAssetChange();
}, />
    < /Provider>,);
...
expect(img).toBeInTheDocument();
expect(img).toHaveAttribute('src', './images/eth_logo.svg');
expect(getByAltText('NATIVE TICKER logo')).toHaveAttribute('src', CHAIN_ID_TOKEN_IMAGE_MAP['0x1']);
;
...
expect(getByAltText('symbol logo')).toHaveAttribute('src', 'token icon url');
;
...
expect(img).toBeInTheDocument();
expect(img).toHaveAttribute('src', 'token icon url');
expect(getByAltText('symbol overflow logo')).toHaveAttribute('src', 'token icon url');
;
it('token: renders symbol and image falls back', () => {
    ...
    const mockAssetChange = jest.fn();
    const { getByText } = render(<Provider>store, { store(, [{ address: , 'token address': , iconUrl: , 'token icon url':  }], unknownChainId); }
        >
            <AssetPicker>header, { 'testHeader':  }, asset = { asset }, onAssetChange = {}());
    mockAssetChange();
}, />
...
it('render if disabled', () => {
    ...
    const mockAssetChange = jest.fn();
    const { container } = render(<Provider>store, { store(, [{ address: , 'token address': , iconUrl: , 'token icon url':  }], unknownChainId); }
        >
            <AssetPicker>header, { 'testHeader':  }, asset = { asset }, onAssetChange = {}());
    mockAssetChange();
}, isDisabled
...
it('should render network picker when networks prop is defined', () => {
    ...
    const mockAssetChange = jest.fn();
    const { asFragment } = render(<Provider>store, { store(NATIVE_TICKER); } >
        <AssetPicker>header, { 'testHeader':  }, asset = { asset }, onAssetChange = {}());
    mockAssetChange();
}, isDisabled, networkProps = {}, {
    network: {
        chainId: '0x1',
        nativeCurrency: 'ETH',
