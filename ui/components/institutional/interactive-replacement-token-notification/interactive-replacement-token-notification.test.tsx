...
describe('Interactive Replacement Token Notification', () => {
    beforeEach(() => {
        (hashUtils.sha256 as jest.MockedFunction<typeof hashUtils.sha256>).mockImplementation(() => Promise.resolve('def'));
    });
    ...
    it('should not render if show notification is false', () => {
        ...
        renderWithProvider(<InteractiveReplacementTokenNotification>/>, store);, expect(screen.queryByTestId('interactive-replacement-token-notification')).not.toBeInTheDocument());
    });
    it('should render if show notification is true and click on learn more', async () => {
        ...
        expect(screen.getByTestId('interactive-replacement-token-notification')).toBeInTheDocument();
