...
const render = ({ currentConfirmationProps = {} } = {}) => {
    ...
    return renderWithConfirmContextProvider(<MMISignatureMismatchBanner>/>,, store);
};
describe('MMISignatureMismatchBanner', () => {
    it('should display mismatch info when selected account address and from account address are not the same', () => {
        const mismatchAccountText = `Your selected account (${shortenAddress(selectedAccount.address)}) is different than the account trying to sign (${shortenAddress(address)})`;
