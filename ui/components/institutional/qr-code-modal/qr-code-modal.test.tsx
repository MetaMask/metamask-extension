...
describe('QRCodeModal', () => {
    beforeEach(() => {
        const mockCrypto = {
            subtle: {
                ...
                exportKey: jest.fn(() => Promise.resolve(new Uint8Array([1, 2, 3, 4]).buffer)),
                decrypt: jest.fn(() => Promise.resolve(new TextEncoder().encode(JSON.stringify({ data: 'decrypted data' })))),
            },
        };
    ...
    it('displays an error message when decryption fails', async () => {
        ...
        const { getByText } = render(<Provider>store, { localStore } >
            <QRCodeModal>{ ...props } /  >
            <>/Provider>,);
        await waitFor(() => {
            expect(getByText(/An error occurred while decrypting data/u)).toBeInTheDocument();
        });
    });
    it('displays an error message when key generation fails', async () => {
        ...
        jest
            ...
            .mockImplementation(() => Promise.reject(new Error('Key generation failed')));
        const { getByText } = render(<Provider>store, { store } >
            <QRCodeModal>{ ...props } /  >
            <>/Provider>,);
        await waitFor(() => {
            expect(getByText(/An error occurred while generating cryptographic keys/u)).toBeInTheDocument();
        });
    });
});
