...
export const useNativeTokenBalance = (selectedChainId?: `0x${string}`) => {
  ...
  const { chainId: _chainId, ticker } = useSelector(
    getMultichainCurrentNetwork,
  );
  ...
  const balance = balancesAllChains[selectedChainId];
  ...
  const primaryBalance = secondaryCurrencyDisplay;

  const secondaryBalance = showFiat ? primaryCurrencyDisplay : undefined;
  ...
  console.log({ nativeTokenWithBalance });
