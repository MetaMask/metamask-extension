...
export const useNativeTokenBalanceForChain = (chain: MultichainNetwork) => {
  const showFiat = useSelector(getMultichainShouldShowFiat);
  const primaryTokenImage = useSelector((state) =>
    getMultichainCurrencyImageByChainId(state, chain.network.chainId),
  );
  const { showNativeTokenAsMainBalance } = useSelector(getPreferences);
  const { chainId, ticker, type, rpcUrl } = chain.network;

  // Adjustments for missing rpcUrl
  const rpcUrlOrDefault = chain.network.rpcUrl || ''; // Use empty string if rpcUrl is undefined
  ...
  const balancesAllChains: any = useSelector(
    getMultichainSelectedAccountCachedBalanceAllChains,
  );

  const balance = balancesAllChains[chain.network.chainId];
