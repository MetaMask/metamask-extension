...
import { BUY_GET_MUSD_CTA_VARIANT } from '../../../hooks/musd/useMusdCtaVisibility';
...
describe('MusdBuyGetCta', () => {
  ...
  describe('BUY variant', () => {
    it('renders "Buy mUSD" text for BUY variant', () => {
      ...
      expect(screen.getByText('Buy mUSD')).toBeInTheDocument();
    });

    it('navigates to buy screen when clicked for BUY variant', () => {
      const store = createMockStore();
      renderWithProvider(
        <MusdBuyGetCta
          variant={BUY_GET_MUSD_CTA_VARIANT.BUY}
  ...
  describe('network icon', () => {
    it('shows network icon when showNetworkIcon is true', () => {
      ...
      // Check for network icon element
      expect(
