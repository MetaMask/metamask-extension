...
/**
 * Low-level QR camera scanner used by `QRHardwarePopover` for both
 * wallet-import (PAIR) and transaction-signing (SIGN) flows.
 ...
 * and classifies the result into one of four `CameraReadyState` values.
 * 3. QR decoding - once the camera is ready, renders `EnhancedQrReader`,
 * which continuously feeds scanned payloads into a `URDecoder` until a
 * full UR is assembled and forwarded to `handleSuccess`.
 * 4. Error UI - displays recoverable camera / scan errors with retry.
 *
 * The component never polls for permission changes. Instead, it subscribes
 * to the `PermissionStatus.change` event so the scanner transitions to
 * READY automatically when the user grants camera access in browser settings.
 *
 * @param options0 - Component props.
 * @param options0.isReadingWallet - True when scanning a wallet sync QR code;
 * false for transaction signing.
 * @param options0.expectedUrTypes - UR types the consumer expects to receive.
...
const BaseQrReader = ({
  ...
  // ---- Error-active signal to parent ----------------------------------------
  // Tells the parent popover that BaseQrReader is showing error/permission UI
  // so it can hide the flow-specific title ("Scan QR code" / "Request signature").
  const isErrorActive =
    Boolean(scanError) ||
    Boolean(error) ||
  ...
  return (
    ...
    >
      <Box paddingLeft={5} paddingRight={5} className="qr-scanner__content">
        {readyState === CameraReadyState.Ready ? (
          <EnhancedQrReader onFrame={handleScan} />
        ) : null}
      </Box>
      {scanProgress > 0 && (
        <div
