...
/**
 * Creates the initial state for the BaseQrReader reducer.
 *
 * @returns A fresh {@link BaseQrReaderState} with camera in accessing mode,
 * no error, zero progress, and no loading indicator.
 */
export function getInitialState(): BaseQrReaderState {
  return {
...
/**
 * Pure reducer for the BaseQrReader component state machine.
 *
 ...
 */
export function baseQrReaderReducer(
  state: BaseQrReaderState,
  action: Action,
): BaseQrReaderState {
  switch (action.type) {
