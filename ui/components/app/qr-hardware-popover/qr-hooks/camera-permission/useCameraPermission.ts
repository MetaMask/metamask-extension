...
export function useCameraPermission(
   ...
   * to READY on success.
   *
   * Used as the `PermissionStatus.onchange` callback so the scanner
   * auto-recovers when the user grants camera access outside our UI.
   * Does a short `getUserMedia`/`stop` round-trip to confirm the camera
   * hardware is available before transitioning. On failure the UI stays
   * on the recovery screen and the user can retry.
   */
