...
const mockProps = {
  ...
  multichainNetwork: {
    ...
    network: {
      ...
      ticker: 'BTC',
      nickname: 'Bitcoin',
      isAddressCompatible: (address: string) => true,
      rpcPrefs: {
...
describe('MultichainTransactionDetailsModal', () => {
  ...
  it('renders the modal with transaction details', () => {
    ...
    expect(screen.getByTestId('transaction-amount')).toHaveTextContent('1.2 BTC');
  });
  ...
  it('shows transaction ID in shortened format', () => {
    ...
    const shortenedTxId = screen.getByText(txId.substring(0, 7) + '...' + txId.substring(txId.length - 5));
