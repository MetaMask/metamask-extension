...
import { useMusdCtaVisibility, BUY_GET_MUSD_CTA_VARIANT } from './useMusdCtaVisibility';
...
const createMockStore = (overrides = {}) => {
  ...
  return configureStore({
    reducer: {
      metamask: (state = defaultState.metamask) => state,
      musd: (state = defaultState.musd) => state,
    },
    preloadedState: defaultState,
  });
};

const wrapper = ({ children, store }: { children: React.ReactNode; store: ReturnType<typeof createMockStore> }) => (
...
describe('useMusdCtaVisibility', () => {
  ...
  describe('shouldShowBuyGetMusdCta', () => {
    ...
    it('returns GET variant when user has convertible tokens', () => {
      ...
      // Mock that user has convertible tokens
      const ctaState = result.current.shouldShowBuyGetMusdCta({
  ...
  describe('shouldShowAssetOverviewCta', () => {
    ...
    it('returns false when CTA has been dismissed for this token', () => {
      const store = createMockStore({
        musd: {
          dismissedCtaKeys: ['0x1-0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48'],
          educationSeen: false,
        },
      });
