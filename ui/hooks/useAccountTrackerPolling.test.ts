...
import { accountTrackerStartPolling, accountTrackerStopPollingByPollingToken, } from '../store/actions';
...
describe('useAccountTrackerPolling', () => {
    ...
    it('should poll account trackers for network client IDs when enabled and stop on dismount', async () => {
        ...
        const { unmount } = renderHookWithProvider(() => useAccountTrackerPolling(), state);
        // Should poll each client ID
        await Promise.all(mockPromises);
        expect(accountTrackerStartPolling).toHaveBeenCalledTimes(2);
        expect(accountTrackerStartPolling).toHaveBeenCalledWith('selectedNetworkClientId');
        expect(accountTrackerStartPolling).toHaveBeenCalledWith('selectedNetworkClientId2');
        // Stop polling on dismount
        unmount();
        expect(accountTrackerStopPollingByPollingToken).toHaveBeenCalledTimes(2);
        expect(accountTrackerStopPollingByPollingToken).toHaveBeenCalledWith('selectedNetworkClientId_tracking');
    });
