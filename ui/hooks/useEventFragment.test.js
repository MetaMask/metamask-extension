...
jest.mock('./useSegmentContext', () => ({ useSegmentContext: jest.fn(() => ({ page: '/' })) }));
...
describe('useEventFragment', () => {
    ...
    describe('return shape', () => {
        let value;
        beforeAll(async () => {
            useSelector.mockImplementation(selector => selector({ metamask: { MetaMetricsController: { fragments: { testid: { id: 'testid' } } } } }));
            createEventFragment.mockImplementation(() => Promise.resolve({ id: 'testid' }));
            const {result, waitForNextUpdate} = renderHook(() => useEventFragment(undefined, {
        ...
        it('should have fragment property', () => {
            ...
            expect(value.fragment).toMatchObject({ id: 'testid' });
        });
    });
    describe('identifying appropriate fragment', () => {
        ...
        it('should return matching fragment by successEvent when no id is provided, but persist is true', async () => {
            ...
            const {result} = renderHook(() => useEventFragment(undefined, {
                successEvent: 'track new event',
                persist: true
            }));
            const returnValue = result.current;
    ...
    describe('methods', () => {
        ...
        it('trackSuccess method should invoke the background finalizeEventFragment method', () => {
            value.trackSuccess();
            expect(finalizeEventFragment).toHaveBeenCalledWith('testid', { context: { page: '/' } });
        });
        ...
        it('updateEventFragment method should invoke the background updateEventFragment method', () => {
            ...
            expect(updateEventFragment).toHaveBeenCalledWith('testid', { properties: { count: 1 } });
        });
    });
});