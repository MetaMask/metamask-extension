...
import { tokenRatesStartPolling, tokenRatesStopPollingByPollingToken, } from '../store/actions';
...
describe('useTokenRatesPolling', () => {
    ...
    it('should poll token rates when enabled and stop on dismount', async () => {
        ...
        const { unmount } = renderHookWithProvider(() => useTokenRatesPolling(), state);
        ...
        expect(tokenRatesStopPollingByPollingToken).toHaveBeenCalledWith('0x1_rates');
        expect(tokenRatesStopPollingByPollingToken).toHaveBeenCalledWith('0x89_rates');
    });
