...
const wrapper: FC = ({ children }) => (<Provider>store) = { configureStore(mockStoreState); } > { children } < /Provider>;
;
describe('useRamps', () => {
    ...
    it('should use the correct metamask entry param when opening the buy crypto URL', () => {
        ...
        const mockBuyURI = `${process.env.PORTFOLIO_URL}/buy?metamaskEntry=${metaMaskEntry}&chainId=${mockChainId}&metametricsId=${mockedMetametricsId}&metricsEnabled=false`;
        const openTabSpy = jest.spyOn(global.platform, 'openTab');
        const { result } = renderHook(() => useRamps(RampsMetaMaskEntry.TokensBanner), { wrapper });
        result.current.openBuyCryptoInPdapp();
        expect(openTabSpy).toHaveBeenCalledWith({
            url: mockBuyURI,
        });
    ...
    // @ts-ignore
    it.each(['0x1', '0x38', '0xa'])('should open the buy crypto URL with the currently connected chain ID', (mockChainId: Hex) => {
        mockStoreState = {};
