...
function createMockStateWithPetnamesEnabled(petnamesEnabled: Test['petnamesEnabled']) {
...
describe('usePetnamesEnabled', () => {
    ...
    tests.forEach(({ petnamesEnabled, expectedResult }) => {
        it(`should return ${String(expectedResult)} when petnames preference is ${String(petnamesEnabled)}`, () => {
            const { result } = renderHookWithProvider(() => usePetnamesEnabled(), createMockStateWithPetnamesEnabled(petnamesEnabled));
