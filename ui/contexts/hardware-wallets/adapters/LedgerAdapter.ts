...
export class LedgerAdapter implements HardwareWalletAdapter {
  ...
  async connect(): Promise<void> {
    console.debug('[LedgerAdapter] connect() called', JSON.stringify({
    ...
    if (this.connected) {
      console.debug('[LedgerAdapter] connect() — already connected, returning early');
      return;
    }

    // Connection in progress - check if it's for the same device
    if (this.isConnecting && this.pendingConnection) {
      console.debug('[LedgerAdapter] connect() — connection already in progress, waiting for pending connection');
      ...
      try {
        ...
        console.debug('[LedgerAdapter] connect() — pending connection failed, will retry', JSON.stringify({
      ...
      if (this.connected) {
        console.debug('[LedgerAdapter] connect() — disconnecting before retrying new connection');
    ...
    this.pendingConnection = (async () => {
      try {
        ...
        console.debug('[LedgerAdapter] connect() Step 1: Checking WebHID availability...');
        const webHidAvailable = isWebHidAvailable();
        console.debug('[LedgerAdapter] connect() Step 1: WebHID availability result', JSON.stringify({
          webHidAvailable,
        }, null, '\t'));
        if (!webHidAvailable) {
        ...
        console.debug('[LedgerAdapter] connect() Step 2: Checking for physically connected Ledger device...');
        const connectedLedgerDevice = await this.checkDeviceConnected();
        console.debug('[LedgerAdapter] connect() Step 2: Device check result', JSON.stringify({
        ...
        console.debug('[LedgerAdapter] connect() Step 3: Checking device unlock status', JSON.stringify({
          productName,
          requiresUnlockCheck: ['Nano S', 'Nano X', 'Nano S Plus'].includes(productName),
        }, null, '\t'));
        if (['Nano S', 'Nano X', 'Nano S Plus'].includes(productName)) {
          const hdPath = await this.getHdPath();
          console.debug('[LedgerAdapter] connect() Step 3: Got HD path, requesting public key', JSON.stringify({
            hdPath,
          }, null, '\t'));
          await getLedgerPublicKey(hdPath);
          console.debug('[LedgerAdapter] connect() Step 3: Public key request succeeded — device is unlocked');
        }

        // Step 4: Attempt to create a transport for the device
        console.debug('[LedgerAdapter] connect() Step 4: Attempting Ledger transport creation...');
        await attemptLedgerTransportCreation();
        console.debug('[LedgerAdapter] connect() Step 4: Transport creation succeeded');

        // Mark as connected - device is present AND app is open
        this.connected = true;
        console.debug('[LedgerAdapter] connect() — connection complete', JSON.stringify({
          connected: this.connected,
          isConnecting: this.isConnecting,
        }, null, '\t'));
      } catch (error) {
        ...
        console.error('[LedgerAdapter] connect() — connection failed', JSON.stringify({
        ...
        console.debug('[LedgerAdapter] connect() — connection attempt finished (finally)', JSON.stringify({
          connected: this.connected,
          isConnecting: this.isConnecting,
          hasPendingConnection: this.pendingConnection !== null,
        }, null, '\t'));
  ...
  async ensureDeviceReady(
    ...
    console.debug('[LedgerAdapter] ensureDeviceReady() called', JSON.stringify({
      options: options ?? {},
      requireBlindSigning,
    ...
    if (!this.isConnected()) {
      console.debug('[LedgerAdapter] ensureDeviceReady() — not connected, calling connect() first');
      await this.connect();
      console.debug('[LedgerAdapter] ensureDeviceReady() — connect() completed, now connected:', this.connected);
    }

    try {
      // Get the app name and version from the Ledger device
      console.debug('[LedgerAdapter] ensureDeviceReady() — getting app name and version...');
      const appInfo = await getAppNameAndVersion();
      console.debug('[LedgerAdapter] ensureDeviceReady() — app info received', JSON.stringify({
        appName: appInfo.appName,
        appVersion: 'version' in appInfo ? (appInfo as Record<string, unknown>).version : undefined,
        fullAppInfo: appInfo,
      }, null, '\t'));
      if (appInfo.appName !== 'Ethereum') {
        console.warn('[LedgerAdapter] ensureDeviceReady() — Ethereum app is NOT open', JSON.stringify({
      ...
      console.debug('[LedgerAdapter] ensureDeviceReady() — Ethereum app confirmed open');
      ...
      if (requireBlindSigning) {
        console.debug('[LedgerAdapter] ensureDeviceReady() — checking blind signing status...');
        const appConfig = await getLedgerAppConfiguration();
        console.debug('[LedgerAdapter] ensureDeviceReady() — app configuration received', JSON.stringify({
          arbitraryDataEnabled: appConfig.arbitraryDataEnabled,
          fullAppConfig: appConfig,
        }, null, '\t'));
        if (appConfig.arbitraryDataEnabled !== 1) {
          console.warn('[LedgerAdapter] ensureDeviceReady() — blind signing is NOT enabled', JSON.stringify({
            arbitraryDataEnabled: appConfig.arbitraryDataEnabled,
          }, null, '\t'));
          throw createHardwareWalletError(
        ...
        console.debug('[LedgerAdapter] ensureDeviceReady() — blind signing is enabled');
      } else {
        console.debug('[LedgerAdapter] ensureDeviceReady() — skipping blind signing check (requireBlindSigning=false)');
      }
      ...
      const hwError = toHardwareWalletError(error, HardwareWalletType.Ledger);
      console.error('[LedgerAdapter] ensureDeviceReady() — readiness check failed', JSON.stringify({
        errorCode: hwError.code,
        errorMessage: hwError.message,
        errorSeverity: hwError.severity,
        errorCategory: hwError.category,
      ...
      console.debug('[LedgerAdapter] ensureDeviceReady() — emitting device event', JSON.stringify({
        deviceEvent,
        errorCode: hwError.code,
      }, null, '\t'));
      this.options.onDeviceEvent({
      ...
      if (shouldResetConnection || deviceEvent === DeviceEvent.Disconnected) {
        this.connected = false;
        console.debug('[LedgerAdapter] ensureDeviceReady() — connection state reset', JSON.stringify({
