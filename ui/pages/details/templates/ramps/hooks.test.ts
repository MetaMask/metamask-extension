...
describe('useRampsDetailsItem', () => {
  ...
  it('returns undefined when there is no identifier', () => {
    const { result } = renderHook(() =>
      useRampsDetailsItem(undefined, 'eip155:1'),
    );
  ...
  it('maps an order looked up by id with a CAIP chain fallback', () => {
    mockUseRampsOrders.mockReturnValue({
      orders: [],
    ...
    const { result } = renderHook(() =>
      useRampsDetailsItem('order-1', 'eip155:1'),
    );
  ...
  it('looks up by settlement hash when id lookup misses', () => {
    ...
    const { result } = renderHook(() =>
      useRampsDetailsItem('0xsettled', 'eip155:1'),
    );
  ...
  it('does not pass a non-CAIP chainId as the mapper fallback', () => {
    ...
    renderHook(() => useRampsDetailsItem('order-1', '1'));
