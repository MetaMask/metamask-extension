...
describe('PrepareBridgePage', () => {
  ...
  it('should show all native and non-native tokens in asset picker modal when isMultiselectEnabled is true', async () => {
    const mockStore = createBridgeMockStore({
      featureFlagOverrides: {
        extensionConfig: {
          support: true,
    ...
    const { container, getByTestId, getByRole } = renderWithProvider(
      <PrepareBridgePage />,
      configureStore(mockStore),
    );

    // Open the asset picker modal
    const bridgeDestinationButton = getByTestId('bridge-destination-button');
    await act(() => {
      userEvent.click(bridgeDestinationButton);
    });

    // Take a snapshot of the modal with all tokens visible
    expect(container).toMatchSnapshot();

    // Verify native token (ETH) is visible
    expect(getByRole('button', { name: /ETH/u })).toBeInTheDocument();

    // Verify non-native tokens are visible
    expect(getByRole('button', { name: /USDC/u })).toBeInTheDocument();
    expect(getByRole('button', { name: /SUSHI/u })).toBeInTheDocument();
  });
});
