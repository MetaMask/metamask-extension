...
export default function useHandleTx() {
  ...
  const handleSolanaTx = async ({
    ...
    if (typeof snapResponse === 'string') {
      ...
      if (snapResponse.result && typeof snapResponse.result === 'object') {
        ...
        signature = snapResponse.result.signature || 
    ...
    console.log('===SOLANA=== Creating bridge transaction meta with ALL fields:', {
      id: txMeta.id,
      hash: txMeta.hash,
  ...
  return {
    handleTx: async ({
      ...
      if (
        txType === TransactionType.bridge && 
        (typeof txParams === 'string' || fieldsToAddToTxMeta.isSolana)
      ) {
        console.log('Handling Solana bridge transaction');
        // Handle as Solana transaction
        return handleSolanaTx({ 
          txType, 
          txParams: typeof txParams === 'string' ? txParams : JSON.stringify(txParams), 
          fieldsToAddToTxMeta 
        });
      }
