...
import { BridgeAssetV2, getClientHeaders, MinimalAssetSchema, validateBridgeAsset } from '@metamask/bridge-controller';
...
export const fetchPopularTokens = async ({
  ...
  return tokens
    .map((token: unknown) => (validateBridgeAsset(token) ? token : null))
    .filter(Boolean);
};
...
export const fetchTokensBySearchQuery = async ({
  ...
  return {
    ...
    tokens: tokens
      .map((token: unknown) =>
        validateBridgeAsset(token) ? token : null,
      )
      .filter(Boolean),
  };
};
