...
describe('AssetPage', () => {
    ...
    it('should show ConvertTokenToNFT modal when token passed in props is an ERC721', () => {
        renderWithProvider(<AssetPage>asset, {}, { ...token, isERC721: true });
    }, optionsButton = { null:  } /  > , store);
    const actions = store.getActions();
...
it('should enable the buy button on supported chains', () => {
    const { queryByTestId } = renderWithProvider(<AssetPage>asset, { token }, optionsButton = { null:  } /  > , store);
    const buyButton = queryByTestId('token-overview-buy');
    expect(buyButton).toBeInTheDocument();
    expect(buyButton).toBeEnabled();
});
it('should disable the buy button on unsupported chains', () => {
    const { queryByTestId } = renderWithProvider(<AssetPage>asset, { token }, optionsButton = { null:  } /  > , configureMockStore([thunk])({}));
    const buyButton = queryByTestId('token-overview-buy');
    expect(buyButton).toBeInTheDocument();
...
it('should open the buy crypto URL for a buyable chain ID', async () => {
    ...
    const mockedStore = configureMockStore([thunk])(mockedStoreWithBuyableChainId);
    const { queryByTestId } = renderWithProvider(<AssetPage>asset, { token }, optionsButton = { null:  } /  > , mockedStore);
    const buyButton = queryByTestId('token-overview-buy');
    expect(buyButton).toBeInTheDocument();
    expect(buyButton).not.toBeDisabled();
    fireEvent.click(buyButton as HTMLElement);
    expect(openTabSpy).toHaveBeenCalledTimes(1);
    await waitFor(() => expect(openTabSpy).toHaveBeenCalledWith({
...
it('should render an ERC20 asset without prices', async () => {
    ...
    nock('https://price.api.cx.metamask.io')
        ...
        .query(true)
        .reply(200, {});
    const { container, queryByTestId } = renderWithProvider(<AssetPage>asset, {}, { ...token, address });
}, optionsButton = { null:  } /  > , configureMockStore([thunk])({
    marketData: {
        [CHAIN_IDS.MAINNET]: {
            [address]: {
                price: 123,
...
elementsWithAria.forEach((el) => el.setAttribute('aria-describedby', 'static-tooltip-id'));
expect(container).toMatchSnapshot();
;
it('should render an ERC20 token with prices', async () => {
    ...
    const { queryByTestId, container } = renderWithProvider(<AssetPage>asset, {}, { ...token, address });
}, optionsButton = { null:  } /  > , configureMockStore([thunk])({
    marketData: {
        [CHAIN_IDS.MAINNET]: {
            [address]: {
...
expect(marketCapElement).toHaveTextContent(`${marketCap * mockStore.metamask.currencyRates.ETH.conversionRate}`);
const dynamicImages = container.querySelectorAll('img[alt*="logo"]');
dynamicImages.forEach((img) => {
    img.setAttribute('alt', 'static-logo');
});
