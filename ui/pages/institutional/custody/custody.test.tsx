...
describe('CustodyPage', function () {
    ...
    it('renders PulseLoader when loading state is true', async () => {
        mockedGetCustodianToken.mockImplementation(() => new Promise((resolve) => resolve(null)));
        mockedGetCustodianAccounts.mockImplementation(() => new Promise((resolve) => resolve(null)));
        await act(async () => {
            renderWithProvider(<CustodyPage>/>, store);, expect(screen.getByTestId('pulse-loader')).toBeDefined());
        });
    });
    it('renders custody accounts list when I have accounts from connectRequest', async () => {
        mockedGetCustodianAccounts.mockImplementation(() => async (dispatch: Dispatch) => {
            const accounts = [
                {
        ...
        await waitFor(() => {
            expect(screen.getByTestId('select-all-accounts-selected-false')).toBeDefined();
    ...
    it('should select all accounts when "Select All Accounts" checkbox is clicked', async () => {
        ...
        expect(screen.getByTestId('select-all-accounts-selected-true')).toBeChecked();
    });
    it('handles connection errors correctly', async () => {
        ...
        await waitFor(() => {
            ...
            expect(screen.getByTestId('connect-error')).toHaveTextContent('Test Error');
        });
    });
    it('handles authentication errors correctly', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByTestId('connect-error')).toBeDefined();
            expect(screen.getByTestId('connect-error')).toHaveTextContent('Authentication error. Please ensure you have entered the correct token');
        });
    });
    it('does open confirm Connect Custodian modal when custodian display name is not gk8', async () => {
        ...
        expect(screen.queryByTestId('confirm-connect-custodian-modal')).toBeInTheDocument();
    });
    it('filters accounts based on search query', async () => {
        ...
        expect(Fuse).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
            keys: ['name', 'address'],
            tokenize: true,
            matchAllTokens: true,
            threshold: 0.0,
