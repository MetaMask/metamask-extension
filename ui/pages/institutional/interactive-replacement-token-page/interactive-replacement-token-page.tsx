...
import { Box, Button, ButtonLink, ButtonSize, ButtonVariant, Icon, IconName, IconSize, Label, Text, } from '../../../components/component-library';
import { BlockSize, Display, FlexDirection, IconColor, JustifyContent, OverflowWrap, TextColor, } from '../../../helpers/constants/design-system';
...
import { mmiActionsFactory, showInteractiveReplacementTokenBanner, } from '../../../store/institutional/institution-background';
...
const InteractiveReplacementTokenPage: React.FC = () => {
    ...
    const address = useSelector((state: State) => state.appState.modal.modalState.props?.address);
    const { custodyAccountDetails, interactiveReplacementToken, mmiConfiguration, } = useSelector((state: State) => state.metamask);
    const { address: selectedAddress } = useSelector(getSelectedInternalAccount);
    const { custodianName } = custodyAccountDetails[toChecksumHexAddress(address || selectedAddress)] ||
        {};
    const { url } = interactiveReplacementToken || {};
    const { custodians } = mmiConfiguration;
    const custodian: Custodian | undefined = custodians.find((item) => item.envName === custodianName);
    ...
    const { removeAddTokenConnectRequest, setCustodianNewRefreshToken, getCustodianAccounts, } = mmiActions;
    ...
    useEffect(() => {
        ...
        const getTokenAccounts = async () => {
            ...
            try {
                const custodianAccounts = (await dispatch(getCustodianAccounts(connectRequest.token, connectRequest.environment, connectRequest.service, false))) as unknown as TokenAccount[];
                const filteredAccounts = custodianAccounts.filter((account: TokenAccount) => 
                // @ts-expect-error metaMaskAccounts isn't a real type
                metaMaskAccounts[account.address.toLowerCase()]);
                const mappedAccounts = filteredAccounts.map((account: TokenAccount) => ({
    ...
    const onRemoveAddTokenConnectRequest = ({ origin, environment, token, }: ConnectRequest) => {
        dispatch(removeAddTokenConnectRequest({
            origin,
            environment,
            token,
        }));
    ...
    const handleApprove = async () => {
        ...
        setIsLoading(true);
        try {
            await Promise.all(tokenAccounts.map(async (account) => {
                await dispatch(setCustodianNewRefreshToken({
                    address: account.address,
    ...
    data - testid;
    "interactive-replacement-token" >
        <Box>className;
    {
        `page-container__header ${error && 'error'}`;
    }
     >
        <Box>className;
...
className = "interactive-replacement-token-page"
    >
        { error?(, Text, data) { } } - testid;
"connect-error-message";
overflowWrap = { OverflowWrap, : .BreakWord }
    >
        { t(, [custodian, displayName]) { } } || 'Custodian',
;
<>/Text>;
...
"interactive-replacement-token-page"
    >
        { tokenAccounts, : .map((account, idx) => {
                ...
                >
                    <Text>as, "span", data - testid, "account-name", overflowWrap = { OverflowWrap, : .BreakWord }
                >
                    { account, : .name }
                < /Text>
                < /Label>
                < Label, marginTop = { 1:  }, marginRight = { 2:  }, htmlFor = {} `address-${idx}`) }
    >
        <Text>as;
"span";
...
justifyContent = { JustifyContent, : .spaceBetween }
    >
        { account, : .labels && (<CustodyLabels>labels) };
{
    account.labels;
}
index = { idx, : .toString() };
...
onClick = { handleApprove }
    >
        { error, custodian, displayName } || 'Custodian';
t('approveButtonText');
<>/Button>
    < /Box>;
<>/Box>
