...
jest.mock('../../../store/institutional/institution-background', () => ({
    ...
    showInteractiveReplacementTokenBanner: () => mockedShowInteractiveReplacementTokenBanner,
}));
...
describe('Interactive Replacement Token Page', function () {
    ...
    it('should render all the accounts correctly', async () => {
        const expectedHref = `${CHAINID_DEFAULT_BLOCK_EXPLORER_URL_MAP[CHAIN_IDS.MAINNET]}address/${custodianAddress}`;
        await act(async () => {
            await render();
        });
        ...
        expect(screen.getByText(shortenAddress(custodianAddress))).toBeInTheDocument();
        expect(screen.getByText(custodianAccounts[1].labels[0].value)).toBeInTheDocument();
    });
    it('should not render if connectRequests is empty', async () => {
        ...
        expect(queryByTestId('interactive-replacement-token')).not.toBeInTheDocument();
    });
    ...
    it('should call onRemoveAddTokenConnectRequest, setCustodianNewRefreshToken, and dispatch showInteractiveReplacementTokenBanner when handleApprove is called', async () => {
        ...
        const mostRecentOverviewPageState = {
            description: 'You can now use your custodian accounts in MetaMask Institutional.',
            imgSrc: 'iconUrl',
