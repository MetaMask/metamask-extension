...
describe('ProfileSyncToggle', () => {
    it('renders correctly', () => {
        const { getByTestId } = render(<Redux.Provider>store, { mockStore(initialStore) { } }());
    },  >
        <MetamaskNotificationsProvider><ProfileSyncToggle>/>
...
it('calls enableProfileSyncing when toggle is turned on', () => {
    ...
    const { getByTestId } = render(<Redux.Provider>store, { mockStore(store); } >
        <ProfileSyncToggle>/>
        < /Redux.Provider>,);
