...
jest.mock('react-router-dom-v5-compat', () => ({
  ...
  useLocation: () => ({
    pathname: `/settings/contacts/edit/${MOCK_ADDRESS}`,
  }),
  useParams: () => ({}),
}));
...
describe('Edit Contact', () => {
  ...
  it('should render contact information correctly', () => {
    ...
    expect(getByTestId('address-book-edit-contact-name').value).toStrictEqual(
      MOCK_ADDRESS_NAME,
    );
    expect(
      getByTestId('address-book-edit-contact-address').value,
    ).toStrictEqual(MOCK_ADDRESS);
    expect(getByTestId('address-book-edit-contact-network')).toHaveTextContent(
      'Goerli',
    );
  });
});
