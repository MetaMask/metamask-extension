...
import { en as messages, renderWithProvider } from "../../../../test/lib/render-helpers-navigate";
import configureStore from "../../../store/store";
import mockState from "../../../../test/data/mock-state.json";
import { mockCryptoMarkets, mockHip3Markets } from "../../../components/app/perps/mocks";
...
jest.mock("../../../hooks/perps/usePerpsEventTracking", () => ({
  usePerpsEventTracking: (options?: unknown) => (options ? undefined : { track: mockTrack }),
}));
...
describe("MarketListView", () => {
  ...
  describe("search functionality", () => {
    ...
    it("shows empty state when no markets match search", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("perps-market-list-no-results")).toBeInTheDocument();
      });
    });
    ...
    it("hides filter row when searching", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("market-list-filter-sort-row")).toBeInTheDocument();
      });
      ...
      await waitFor(() => {
        expect(screen.queryByTestId("market-list-filter-sort-row")).not.toBeInTheDocument();
      });
    });
  });
  ...
  describe("filter functionality", () => {
    ...
    filterLabelCases.forEach(([filter, expectedLabel]) => {
      it(`shows a visible label for the ${filter} filter query param`, async () => {
        renderWithProvider(<MarketListView />, mockStore, `/perps/market-list?filter=${filter}`);

        await waitFor(() => {
          expect(screen.getByTestId("filter-select-button")).toHaveTextContent(expectedLabel);
        });
      });
    });
    ...
    it("shows only crypto markets on Crypto tab regardless of allowedHip3Sources", async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.queryByTestId("market-row-xyz-TSLA")).not.toBeInTheDocument();
      });
    });
  });
  ...
  describe("search funnel analytics", () => {
    ...
    it("emits search_result_tapped with the picked rank and no abandonment", () => {
      ...
      const [tapCall] = eventsNamed(MetaMetricsEventName.PerpsSearchResultTapped);
      expect(tapCall[1]).toEqual(expect.objectContaining({ search_query: "btc", result_rank: 1 }));
      // A tapped result resolves the search: it was not abandoned.
      expect(eventsNamed(MetaMetricsEventName.PerpsSearchAbandoned)).toHaveLength(0);
    });
    ...
    it("emits search_abandoned when the query is cleared without a tap", () => {
      ...
      const [abandonCall] = eventsNamed(MetaMetricsEventName.PerpsSearchAbandoned);
    ...
    it("emits the query then the tap when a result is picked inside the debounce window", () => {
      ...
      expect(eventsNamed(MetaMetricsEventName.PerpsSearchQuery)).toHaveLength(1);
      const [tapCall] = eventsNamed(MetaMetricsEventName.PerpsSearchResultTapped);
      expect(tapCall[1]).toEqual(expect.objectContaining({ search_query: "btc", result_rank: 1 }));
      expect(eventsNamed(MetaMetricsEventName.PerpsSearchAbandoned)).toHaveLength(0);
    });
    ...
    it("flushes a pending query when the box is cleared inside the debounce window", () => {
      ...
      const [queryCall] = eventsNamed(MetaMetricsEventName.PerpsSearchQuery);
      expect(queryCall?.[1]).toEqual(expect.objectContaining({ search_query: "btc" }));
      const [abandonCall] = eventsNamed(MetaMetricsEventName.PerpsSearchAbandoned);
      expect(abandonCall?.[1]).toEqual(
        expect.objectContaining({ search_query: "btc", query_count: 1 }),
      );
    ...
    it("abandons and resets the session when the query is backspaced to empty", () => {
      ...
      expect(eventsNamed(MetaMetricsEventName.PerpsSearchResultTapped)).toHaveLength(0);
    });
  });
});
