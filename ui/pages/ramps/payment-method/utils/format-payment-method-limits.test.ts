...
import { formatPaymentMethodLimits, getProviderBuyLimit } from "./format-payment-method-limits";
...
describe("getProviderBuyLimit", () => {
  it("returns the matching fiat/payment-method limit", () => {
    ...
    expect(getProviderBuyLimit(provider, "USD", "debit-credit-card")).toMatchSnapshot();
  });

  it("returns undefined when limits are unavailable", () => {
    expect(getProviderBuyLimit(null, "USD", "debit-credit-card")).toBeUndefined();
  });
});
...
describe("formatPaymentMethodLimits", () => {
  it("matches snapshot for min/max formatting", () => {
    expect({
      both: formatPaymentMethodLimits({ minAmount: 25, maxAmount: 2000 }, formatFiat, t),
      minOnly: formatPaymentMethodLimits({ minAmount: 25, maxAmount: Number.NaN }, formatFiat, t),
