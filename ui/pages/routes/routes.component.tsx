...
export default function Routes() {
  ...
  useEffect(() => {
    // In v6, we track page changes via location.pathname changes
    // This replaces the v5 history.listen approach and will trigger on all navigation
    // (PUSH, POP, REPLACE) rather than just PUSH actions, but this is acceptable
    // for the pageChanged action which tracks the most recent overview page
  ...
  const renderRoutes = useCallback(() => {
    ...
    return routes;
  }, [forgottenPassword, isSendRedesignEnabled, autoLockTimeLimit, dispatch]);
