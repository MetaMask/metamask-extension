...
import { LedgerTransportTypes, HardwareDeviceNames, } from '../../../../shared/constants/hardware-wallets';
...
describe('ConnectHardwareForm', () => {
    ...
    it('should match snapshot', () => {
        const { container } = renderWithProvider(<ConnectHardwareForm>{ ...mockProps } /  > , mockStore);
        expect(container).toMatchSnapshot();
    });
    it('should close the form when close button is clicked', () => {
        const { getByTestId } = renderWithProvider(<ConnectHardwareForm>{ ...mockProps } /  > , mockStore);
    ...
    describe('U2F Error', () => {
        it('should render a U2F error', async () => {
            ...
            const { getByText, getByLabelText, queryByText } = renderWithProvider(<ConnectHardwareForm>{ ...mockProps } /  > , mockStoreWithU2F);
            const ledgerButton = getByLabelText('Ledger');
            const continueButton = getByText('Continue');
            fireEvent.click(ledgerButton);
            fireEvent.click(continueButton);
            await waitFor(() => {
                expect(getByText('We had trouble connecting to your ledger, try reviewing', {
                    exact: false,
                })).toBeInTheDocument();
                const firefoxError = queryByText("If you're on the latest version of Firefox, you might be experiencing an issue related to Firefox dropping U2F support. Learn how to fix this issue", { exact: false });
        ...
        it('should render a different U2F error for firefox', async () => {
            jest
                ...
                .mockReturnValue('Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:95.0) Gecko/20100101 Firefox/95.0');
            mockConnectHardware.mockRejectedValue(new Error('U2F Error'));
            const mockStateWithU2F = Object.assign(mockState, {});
            mockStateWithU2F.appState.ledgerTransportType = LedgerTransportTypes.u2f;
            const mockStoreWithU2F = configureMockStore([thunk])(mockStateWithU2F);
            const { getByText, getByLabelText } = renderWithProvider(<ConnectHardwareForm>{ ...mockProps } /  > , mockStoreWithU2F);
            const ledgerButton = getByLabelText('Ledger');
            const continueButton = getByText('Continue');
            fireEvent.click(ledgerButton);
            fireEvent.click(continueButton);
            await waitFor(() => {
                expect(getByText("If you're on the latest version of Firefox, you might be experiencing an issue related to Firefox dropping U2F support. Learn how to fix this issue", { exact: false })).toBeInTheDocument();
            });
        });
    });
    describe('QR Hardware Wallet Steps', () => {
        it('should render the QR hardware wallet steps', async () => {
            const { getByText, getByLabelText } = renderWithProvider(<ConnectHardwareForm>{ ...mockProps } /  > , mockStore);
    ...
    describe('Select Hardware', () => {
        it('should check link buttons for Ngrave Zero brand', async () => {
            window.open = jest.fn();
            const { getByLabelText, getByTestId } = renderWithProvider(<ConnectHardwareForm>{ ...mockProps } /  > , mockStore);
            const qrButton = getByLabelText('QRCode');
            fireEvent.click(qrButton);
