...
describe("SelectHardware", () => {
  ...
  it("renders all hardware wallet options", () => {
    ...
    expect(screen.getByTestId("connect-hardware-wallet-ledger")).toBeInTheDocument();
    expect(screen.getByTestId("connect-hardware-wallet-keystone")).toBeInTheDocument();
    expect(screen.getByTestId("connect-hardware-wallet-trezor")).toBeInTheDocument();
    expect(screen.getByTestId("connect-hardware-wallet-onekey")).toBeInTheDocument();
    expect(screen.getByTestId("connect-hardware-wallet-lattice")).toBeInTheDocument();
    expect(screen.getByTestId("connect-hardware-wallet-other-qr")).toBeInTheDocument();
  });
  ...
  describe("wallet selection", () => {
    it("connects to Ledger", () => {
      ...
      expect(mockConnectToHardwareWallet).toHaveBeenCalledWith(HardwareDeviceNames.ledger);
    });

    it("connects to Keystone via QR device", () => {
      render();
      fireEvent.click(screen.getByTestId("connect-hardware-wallet-keystone"));
      expect(mockConnectToHardwareWallet).toHaveBeenCalledWith(HardwareDeviceNames.qr);
    });

    it("connects to Trezor", () => {
      render();
      fireEvent.click(screen.getByTestId("connect-hardware-wallet-trezor"));
      expect(mockConnectToHardwareWallet).toHaveBeenCalledWith(HardwareDeviceNames.trezor);
    });
    ...
    it("connects to Lattice", () => {
      ...
      expect(mockConnectToHardwareWallet).toHaveBeenCalledWith(HardwareDeviceNames.lattice);
    });
  ...
  describe("unsupported browser", () => {
    ...
    it("does not render wallet options", () => {
      ...
      expect(screen.queryByTestId("connect-hardware-wallet-ledger")).not.toBeInTheDocument();
    });
  });
  ...
  describe("Firefox Ledger warning", () => {
    it("shows warning when clicking Ledger on Firefox", () => {
      ...
      expect(screen.getByText(tEn("ledgerFirefoxNotSupportedTitle"))).toBeInTheDocument();
    ...
    it("dismisses warning when clicking a non-Ledger wallet", () => {
      ...
      fireEvent.click(screen.getByTestId("connect-hardware-wallet-trezor"));
      expect(screen.queryByText(tEn("ledgerFirefoxNotSupportedTitle"))).not.toBeInTheDocument();
    });
