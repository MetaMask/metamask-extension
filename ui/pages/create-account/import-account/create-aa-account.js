...
const factoryAbi = require('./MimoWalletFactory.json');

export default function NewAAAccountForm() {
  ...
  const [account, setAccount] = useState(Object.values(accounts)[0].address);

  async function createAAAccount() {
    ...
    const factory = new ethers.Contract(
      `${process.env.AA_FACTORY_ADDRESS}`,
      factoryAbi.abi,
      signer,
    );
    ...
    const expectedAddress = await factory.getAddress(account, salt);
    ...
    setAAAddress(expectedAddress);

    const tx = await factory.createAccount(account, salt, []);

    console.log(tx);

    importAccount('Smart Contract', [privateKey, expectedAddress, true]);
  }

  async function selectAccount(address) {
    // hack right now to get private key
    const privateKeySelected = await dispatch(
      actions.exportAccount('password', address),
    );
    console.log(privateKeySelected);
    setPrivateKey(privateKeySelected);
  }
    ...
    <>
      ...
      >
        <Text variant={TextVariant.headingLg}>
          Create A New Account Abstraction Account
        </Text>
        <Text variant={TextVariant.bodySm} marginTop={2}>
          {
            'Newly created Account Abstraction accounts won’t be associated with your MetaMask Secret Recovery Phrase. Learn more about imported accounts'
          }{' '}
          <ButtonLink
            size={Size.inherit}
        ...
        <Text>
          Please select the EOA account that will be used to create the new
          Account Abstraction account
        </Text>
      </Box>
