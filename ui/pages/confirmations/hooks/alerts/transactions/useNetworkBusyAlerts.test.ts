...
function runHook(state: Record<string, unknown>) {
    const response = renderHookWithConfirmContextProvider(useNetworkBusyAlerts, state);
...
describe('useNetworkBusyAlerts', () => {
    ...
    it('returns no alerts if network congestion less than threshold', () => {
        const alerts = runHook(getMockConfirmStateForTransaction(contractInteraction, {
            GasFeeController: {
                gasFeeEstimatesByChainId: {
                    '0x5': {
                        gasFeeEstimates: {
