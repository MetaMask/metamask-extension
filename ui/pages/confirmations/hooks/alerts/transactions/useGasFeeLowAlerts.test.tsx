...
import { AlertActionKey, RowAlertKey, } from '../../../../../components/app/confirm/info/row/constants';
...
function buildState({ currentConfirmation, transaction, }: {
...
function runHook(stateOptions?: Parameters<typeof buildState>[0]) {
    ...
    const response = renderHookWithConfirmContextProvider(useGasFeeLowAlerts, state, '/', ({ children }: {
...
describe('useGasFeeLowAlerts', () => {
    ...
    it.each(Object.values(PriorityLevels).filter((level) => level !== PriorityLevels.low))('returns no alerts if transaction has %s user fee level', (userFeeLevel: PriorityLevels) => {
        expect(runHook({
            currentConfirmation: CONFIRMATION_MOCK,
            transaction: {
                ...CONFIRMATION_MOCK,
