...
import { getMockConfirmStateForTransaction, getMockPersonalSignConfirmStateForRequest, } from '../../../../../test/data/confirmations/helper';
import { BlockaidResultType, SecurityProvider, } from '../../../../../shared/constants/security-provider';
...
describe('useBlockaidAlerts', () => {
    it('returns an empty array when there is no confirmation', () => {
        const { result } = renderHookWithConfirmContextProvider(() => useBlockaidAlert(), mockState);
        expect(result.current).toEqual([]);
    });
    it('returns alerts when there is a valid PersonalSign confirmation with a security alert response', () => {
        const mockCurrentState = getMockPersonalSignConfirmStateForRequest(currentConfirmationMock, {
        ...
        const { result } = renderHookWithConfirmContextProvider(() => useBlockaidAlert(), mockCurrentState);
