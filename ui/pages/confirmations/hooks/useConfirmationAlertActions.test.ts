...
describe('useConfirmationAlertActions', () => {
  it('calls processPersonalSignAction with the provided actionKey', () => {
    ...
    (usePersonalSignAlertActions as jest.Mock).mockReturnValue(processPersonalSignActionMock);

    const { result } = renderHookWithProvider(() => useConfirmationAlertAction(), {...mockState,});
