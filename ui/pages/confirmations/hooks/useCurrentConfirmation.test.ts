...
function buildState({ message, pendingApprovals, redesignedConfirmationsEnabled, redesignedTransactionsEnabled, transaction, isRedesignedConfirmationsDeveloperEnabled, }: {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    message?: Partial<AbstractMessage & {
        msgParams: any;
    }>;
    ...
    return {
        ...
        PreferencesController: {
            preferences: {
                ...
                isRedesignedConfirmationsDeveloperEnabled: isRedesignedConfirmationsDeveloperEnabled || false,
            }
        },
        TxController: {
...
function runHook(state: Parameters<typeof buildState>[0]) {
    const response = renderHookWithProvider(useCurrentConfirmation, buildState(state));
...
describe('useCurrentConfirmation', () => {
    ...
    it('return message matching latest pending approval ID', () => {
        ...
        expect(currentConfirmation).toStrictEqual(expect.objectContaining(MESSAGE_MOCK));
    });
    ...
    it('returns if message is SIWE', () => {
        ...
        expect(currentConfirmation).toStrictEqual(expect.objectContaining({
            id: APPROVAL_MOCK.id,
            msgParams: { siwe: { isSIWEMessage: true } },
        }));
    });
