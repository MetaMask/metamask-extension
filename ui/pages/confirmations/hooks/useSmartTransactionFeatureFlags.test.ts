...
import { fetchSmartTransactionsLiveness, setSwapsFeatureFlags, } from '../../../store/actions';
...
async function runHook({ smartTransactionsOptInStatus, chainId, confirmation, }: {
    smartTransactionsOptInStatus: boolean;
    chainId: Hex;
    confirmation?: Partial<TransactionMeta>;
}) {
    const transaction = (confirmation as TransactionMeta) ??
    ...
    renderHookWithConfirmContextProvider(() => useSmartTransactionFeatureFlags(), state);
    await act(async () => {
        // Intentionally empty
    });
}
describe('useSmartTransactionFeatureFlags', () => {
    ...
    const fetchSmartTransactionsLivenessMock = jest.mocked(fetchSmartTransactionsLiveness);
