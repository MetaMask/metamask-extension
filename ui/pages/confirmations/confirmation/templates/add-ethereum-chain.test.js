...
describe('add-ethereum-chain confirmation', () => {
  ...
  it('should show warning if RPC URL has special characters and display punycode encoding', async () => {
    ...
    await waitFor(() => {
      expect(
        getByText(
          "Attackers sometimes mimic sites by making small changes to the site address. Make sure you're interacting with the intended site before you continue. We replaced the original version with its Punycode encoded form. Original version: https://iոfura.io/gnosis",
        ),
      ).toBeInTheDocument();
