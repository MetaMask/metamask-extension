...
import { PERSONAL_SIGN_SENDER_ADDRESS, unapprovedPersonalSignMsg } from "../../../../../test/data/confirmations/personal_sign";
import { SignatureRequestType } from "../../types/confirm";
import { getConfirmationSender } from "./utils";

describe('getConfirmationSender()', () => {
  test('returns the sender address from a signature if it\'s passed', () => {
    const testCurrentConfirmation = genUnapprovedContractInteractionConfirmation() as TransactionMeta;
  ...
  test('returns the sender address from a transaction if it\'s passed', () => {
    const testCurrentConfirmation = unapprovedPersonalSignMsg as SignatureRequestType;
    const { from } = getConfirmationSender(testCurrentConfirmation);
