...
import { permitSignatureMsg, seaportSignatureMsg, } from '../../../../../../../../test/data/confirmations/typed_sign';
...
describe('PermitSimulation', () => {
    ...
    it('should not render default simulation if decodingLoading is true', async () => {
        ...
        const mockStore = configureMockStore([])(state);
        await act(async () => {
            const { queryByTestId } = renderWithConfirmContextProvider(<TypedSignV4Simulation>/>,, mockStore);
            await waitFor(() => {
                expect(queryByTestId('30')).not.toBeInTheDocument();
                expect(queryByTestId("You're giving the spender permission to spend this many tokens from your account.")).not.toBeInTheDocument();
            });
        });
    });
