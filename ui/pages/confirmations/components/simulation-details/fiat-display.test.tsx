...
const mockStoreWithShowingFiatOnTestnets = configureStore()(mockStateWithShowingFiatOnTestnets);
...
const mockStoreWithHidingFiatOnTestnets = configureStore()(mockStateWithHidingFiatOnTestnets);
...
describe('FiatDisplay', () => {
    ...
    describe('IndividualFiatDisplay', () => {
        ...
        it.each([
            [100, '$100'],
            [-100, '$100'],
        ])('when fiatAmount is %s it renders %s', (fiatAmount: number | null, expected: string) => {
            renderWithProvider(<IndividualFiatDisplay>fiatAmount, { fiatAmount } /  > , mockStoreWithShowingFiatOnTestnets);
            expect(screen.getByText(expected)).toBeInTheDocument();
        });
        it('does not render anything if user opted out to show fiat values on testnet', () => {
            const { queryByText } = renderWithProvider(<IndividualFiatDisplay>fiatAmount, { 100:  } /  > , mockStoreWithHidingFiatOnTestnets);
    ...
    describe('TotalFiatDisplay', () => {
        ...
        it.each([
            ...
            [[-100, -200, FIAT_UNAVAILABLE, -300], 'Total = $600'],
        ])('when fiatAmounts is %s it renders %s', (fiatAmounts: (number | null)[], expected: string) => {
            renderWithProvider(<TotalFiatDisplay>fiatAmounts, { fiatAmounts } /  > , mockStoreWithShowingFiatOnTestnets);
            expect(screen.getByText(expected)).toBeInTheDocument();
        });
        it('does not render anything if user opted out to show fiat values on testnet', () => {
            const { queryByText } = renderWithProvider(<IndividualFiatDisplay>fiatAmount, { 100:  } /  > , mockStoreWithHidingFiatOnTestnets);
