 ...
 * - Speed-up transactions (`type: retry`) classified using `originalType`, since `txParams` are otherwise unchanged from the transaction being sped up.
 * - Cancellations and retries of cancellations return no payees. Their original params, nested transactions, and swap-and-send recipient can remain on the metadata even though none of those sends executed.
