...
export default class ConfirmTransactionSwitch extends Component {
  ...
  redirectToTransaction () {
    ...
    if (type === TRANSACTION_TYPE_DEPLOY_CONTRACT) {
      const pathname = `${CONFIRM_TRANSACTION_ROUTE}/${id}${CONFIRM_DEPLOY_CONTRACT_PATH}`
      return <Redirect to={{ pathname }} />
    }

    if (type === TRANSACTION_TYPE_SENT_ETHER) {
    ...
    if (data) {
      switch (type) {
        case TRANSACTION_TYPE_TOKEN_METHOD_TRANSFER: {
          const pathname = `${CONFIRM_TRANSACTION_ROUTE}/${id}${CONFIRM_SEND_TOKEN_PATH}`
          return <Redirect to={{ pathname }} />
        }
        case TRANSACTION_TYPE_TOKEN_METHOD_APPROVE: {
          const pathname = `${CONFIRM_TRANSACTION_ROUTE}/${id}${CONFIRM_APPROVE_PATH}`
          return <Redirect to={{ pathname }} />
        }
        case TRANSACTION_TYPE_TOKEN_METHOD_TRANSFER_FROM: {
