...
import { BN, stripHexPrefix } from 'cfx-util'
...
const whenPredSetCRWithPropAndSetter = (pred, prop, setter) =>
  R.when(
    pred,
    R.converge(conversionRateLens, [R.pipe(R.prop(prop), setter), R.identity])
  )
...
const whenPredSetWithPropAndSetter = (pred, prop, setter) =>
  R.when(
    pred,
    R.converge(valuePropertyLens, [R.pipe(R.prop(prop), setter), R.identity])
  )
