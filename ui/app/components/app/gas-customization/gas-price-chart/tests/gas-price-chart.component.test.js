...
describe('GasPriceChart Component', function () {
  ...
  describe('render()', function () {
    ...
    it('should render the chart div', function () {
      ...
      assert.strictEqual(wrapper.childAt(0).props().id, 'chart')
    })
  })
  ...
  describe('componentDidUpdate', function () {
    it('should call handleChartUpdate if props.currentPrice has changed', function () {
      ...
      assert.strictEqual(gasPriceChartUtilsSpies.handleChartUpdate.callCount, 1)
    })
    ...
    it('should not call handleChartUpdate if props.currentPrice has not changed', function () {
      ...
      assert.strictEqual(gasPriceChartUtilsSpies.handleChartUpdate.callCount, 0)
    })
  })
  ...
  describe('renderChart', function () {
    it('should call setTickPosition 4 times, with the expected props', async function () {
      ...
      assert.strictEqual(gasPriceChartUtilsSpies.setTickPosition.callCount, 0)
    ...
    it('should add three events to the chart', async function () {
      ...
      assert.strictEqual(selectReturnSpies.on.callCount, 0)
      wrapper.instance().renderChart(testProps)
      await timeout(0)
      assert.strictEqual(selectReturnSpies.on.callCount, 3)

      const firstOnEventArgs = selectReturnSpies.on.getCall(0).args
      assert.strictEqual(firstOnEventArgs[0], 'mouseout')
      const secondOnEventArgs = selectReturnSpies.on.getCall(1).args
      assert.strictEqual(secondOnEventArgs[0], 'click')
      const thirdOnEventArgs = selectReturnSpies.on.getCall(2).args
      assert.strictEqual(thirdOnEventArgs[0], 'mousemove')
    })
    ...
    it('should updateCustomGasPrice on click', async function () {
      ...
      assert.strictEqual(propsMethodSpies.updateCustomGasPrice.callCount, 0)
      mouseoutEventArgs[1]()
      assert.strictEqual(propsMethodSpies.updateCustomGasPrice.callCount, 1)
      assert.strictEqual(
