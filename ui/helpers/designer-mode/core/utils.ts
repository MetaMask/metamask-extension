...
export function buildComponentInfo(el: HTMLElement, fields: ComponentInfoFields): ComponentInfo {
  return {
    ...
    testId: fields.testId === undefined ? (el.dataset.testid ?? el.getAttribute('data-testid') ?? null) : fields.testId,
...
export function buildDomPath(el: HTMLElement): string {
  ...
  while (current && current !== document.body) {
    ...
    if (node.classList.length > 0) {selector += `.${Array.from(node.classList).slice(0,2).join('.')}`;}
...
export function detectFramework(): Promise<import('./types').Framework> {
  return new Promise(resolve => {
    setTimeout(() => {
      ...
      if (win.__REACT_DEVTOOLS_GLOBAL_HOOK__) {return resolve('react');}
      if (win.__VUE__ || childGlobals.__vueParentComponent) {return resolve('vue');}
      if (ng?.getComponent || document.querySelector('[ng-version]')) {return resolve('angular');}
