...
export const exchangeRateFromMarketData = (
  genericChainId: Hex | ChainId | CaipChainId,
  tokenAddress: string,
  marketData?: Record<string, ContractMarketData>,
) => {
  if (isNonEvmChainId(genericChainId)) {
    return undefined;
  }
  const chainId = isStrictHexString(genericChainId)
    ? genericChainId
    : formatChainIdToHex(genericChainId);

  if (isNativeAddress(tokenAddress)) {
    // @ts-expect-error - hexChainId is a Hex string
    return marketData?.[chainId]?.[zeroAddress()]?.price ?? 1;
  }
  if (isStrictHexString(tokenAddress)) {
    return marketData?.[chainId]?.[tokenAddress]?.price ?? undefined;
  }
  if (isCaipAssetType(tokenAddress)) {
    const { assetReference } = parseCaipAssetType(tokenAddress);
    return isStrictHexString(assetReference)
      ? marketData?.[chainId]?.[assetReference]?.price
      : undefined;
  }
  return undefined;
};
...
export const toBridgeToken = (
  ...
  return {
    ...
    chainId,
    image: (payload.image || getAssetImageUrl(payload.assetId)) ?? '',
